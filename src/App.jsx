...
const DEFAULT_INPUTS = {
  ...
  fraisBancairesAchat: "2600", apport: "14000", tauxCredit: "4.2", dureeAnnees: "25",
  differeMois: "0",
...
function SimulateurExploitation() {
  var _i = React.useState({
    prixAchat: "175000", commune: "",
    taxeFonciere: "1200", chargesAn: "600", assurancePNO: "350",
    tauxCredit: "4.2", dureeCredit: "25", apport: "14000", fraisNotaire: "13300",
    travaux: "0", fraisBancaires: "2600",
  });
  var inputs = _i[0]; var setInputs = _i[1];
  var _lots = React.useState([
    { id: 1, nom: "Lot 1", surface: "45", nbChambres: "2", loyerNuMarche: "450", mode: "nu" },
  ]);
  ...
  var handleChange = function(e) { var n = e.target.name; var v = e.target.value; setInputs(function(p) { return Object.assign({}, p, { [n]: v }); }); };
  var fieldS = { width: "100%", background: "rgba(248,250,252,0.9)", border: "1px solid rgba(148,163,184,0.4)", borderRadius: 10, padding: "7px 10px", color: "#0f172a", fontSize: 13, outline: "none" };
  var labelS = { display: "block", fontSize: 11, fontWeight: 600, color: "#64748b", marginBottom: 3 };
  ...
  React.useEffect(function() {
    ...
    getCommunesCache().then(function(communes) {
      var q = communeSearch.toLowerCase();
      setSuggestions(communes.filter(function(c) { return c.nom.toLowerCase().includes(q); }).slice(0, 6));
    });
  }, [communeSearch]);

  var selectCommune = function(nom) {
    setCommuneSearch(nom);
    setSuggestions([]);
    setInputs(function(p) { return Object.assign({}, p, { commune: nom }); });
    getCommunesCache().then(function(communes) {
      var found = communes.find(function(c) { return c.nom === nom; });
      if (found) setDonneesCommune(found);
    });
  };

  // Données marché depuis commune
  var tauxEtudiants = donneesCommune && donneesCommune.demographie ? pf(donneesCommune.demographie.taux_etudiants_pct) : 0;
  var loyerMedianM2 = donneesCommune && donneesCommune.loyer ? pf(donneesCommune.loyer.appartement_m2) : 0;
  var population = donneesCommune ? pf(donneesCommune.population) : 0;

  // Lots management
  var addLot = function() {
    var newId = lots.length > 0 ? Math.max.apply(null, lots.map(function(l) { return l.id; })) + 1 : 1;
    setLots(function(p) { return p.concat({ id: newId, nom: "Lot " + newId, surface: "30", nbChambres: "1", loyerNuMarche: "400", mode: "nu" }); });
  };
  var removeLot = function(id) { if (lots.length <= 1) return; setLots(function(p) { return p.filter(function(l) { return l.id !== id; }); }); };
  var updateLot = function(id, field, value) { setLots(function(p) { return p.map(function(l) { return l.id === id ? Object.assign({}, l, { [field]: value }) : l; }); }); };
  ...
  var nMois = Math.max(1, pf(inputs.dureeCredit)) * 12;
  ...
  var MODES = {
    nu: { nom: "Location nue", icon: "🏠", color: "#6366f1", occup: 11.5, majorLoyer: 1, chargesSpecM2: 0, mobilierM2: 0, gliPct: 3, risque: 2, gestion: 1 },
    meuble: { nom: "Meublé LMNP", icon: "🛋️", color: "#0ea5e9", occup: 11, majorLoyer: 1.2, chargesSpecM2: 4, mobilierM2: 80, gliPct: 3.5, risque: 3, gestion: 2 },
    coloc: { nom: "Colocation", icon: "👥", color: "#22c55e", occup: 10.5, majorLoyer: 1, chargesSpecM2: 15, mobilierM2: 60, gliPct: 4, risque: 4, gestion: 3 },
    lcd: { nom: "Courte durée", icon: "✈️", color: "#f59e0b", occup: 8, majorLoyer: 2.2, chargesSpecM2: 30, mobilierM2: 120, gliPct: 0, risque: 5, gestion: 5 },
  };

  // Calculer par lot
  var lotsResults = lots.map(function(lot) {
    var mode = MODES[lot.mode] || MODES.nu;
    var surface = pf(lot.surface);
    var nbCh = Math.max(1, Math.round(pf(lot.nbChambres)));
    var loyerNu = pf(lot.loyerNuMarche);

    var loyerMois;
    if (lot.mode === "coloc") {
      loyerMois = Math.round(nbCh * loyerNu * 0.55);
    } else if (lot.mode === "lcd") {
      // Estimation LCD : surface × 0.8€/nuit × 22 nuits × taux occup relatif
      loyerMois = Math.round(surface * 0.8 * 22);
    } else {
      loyerMois = Math.round(loyerNu * mode.majorLoyer);
    }

    var loyerAn = loyerMois * mode.occup;
    var chargesSpecAn = surface * mode.chargesSpecM2;
    var gliAn = loyerAn * mode.gliPct / 100;
    var totalChargesLot = chargesSpecAn + gliAn;
    var mobilierInit = Math.round(surface * mode.mobilierM2);

    return Object.assign({}, lot, {
      modeInfo: mode, loyerMois: loyerMois, loyerAn: loyerAn,
      chargesSpecAn: chargesSpecAn, gliAn: gliAn, totalChargesLot: totalChargesLot,
      mobilierInit: mobilierInit,
    });
  });

  // Totaux
  var totalLoyerAn = lotsResults.reduce(function(s, l) { return s + l.loyerAn; }, 0);
  var totalChargesLotsAn = lotsResults.reduce(function(s, l) { return s + l.totalChargesLot; }, 0);
  var totalChargesAn = chargesBaseAn + totalChargesLotsAn;
  var cashflowAn = totalLoyerAn - totalChargesAn - creditAn;
  var cashflowMois = cashflowAn / 12;
  var rendBrut = depenseNette > 0 ? (totalLoyerAn / depenseNette * 100) : 0;
  var rendNet = depenseNette > 0 ? (cashflowAn / depenseNette * 100) : 0;
  var totalMobilier = lotsResults.reduce(function(s, l) { return s + l.mobilierInit; }, 0);
  var totalSurface = lotsResults.reduce(function(s, l) { return s + pf(l.surface); }, 0);

  var modeColors = { nu: "#6366f1", meuble: "#0ea5e9", coloc: "#22c55e", lcd: "#f59e0b" };

  return (
    <div style={{ display: "flex", flexDirection: "column", gap: 14 }}>

      {/* KPIs synthèse immeuble */}
      <div style={{ display: "grid", gridTemplateColumns: "repeat(auto-fit, minmax(130px, 1fr))", gap: 10 }}>
        {[
          { label: "Loyers/mois", value: fmtEur(Math.round(totalLoyerAn / 12)), icon: "💶", color: "#16a34a" },
          { label: "Cash-flow/mois", value: (cashflowMois >= 0 ? "+" : "") + fmtEur(Math.round(cashflowMois)), icon: "💰", color: cashflowMois >= 0 ? "#16a34a" : "#dc2626" },
          { label: "Rendement brut", value: fmtPct(rendBrut), icon: "📊", color: "#6366f1" },
          { label: "Rendement net", value: fmtPct(rendNet), icon: "📈", color: rendNet >= 0 ? "#16a34a" : "#dc2626" },
          { label: "Mobilier total", value: fmtEur(totalMobilier), icon: "🛒", color: "#0ea5e9" },
          { label: "Surface totale", value: fmt(totalSurface, 0) + " m²", icon: "📐", color: "#64748b" },
        ].map(function(k) {
          return (<div key={k.label} style={{ background: "rgba(255,255,255,0.8)", borderRadius: 14, padding: "10px 12px", border: "1px solid rgba(148,163,184,0.2)", textAlign: "center" }}>
      ...
      {/* Paramètres globaux + Commune */}
      <div style={{ display: "grid", gridTemplateColumns: "1fr 1fr", gap: 14 }}>
        <div style={SECTION}>
          <SectionHeader icon="🏠" title="Bien immobilier" />
          <div style={{ display: "grid", gridTemplateColumns: "1fr 1fr", gap: 8 }}>
            {[
              { label: "Prix d'achat FAI", name: "prixAchat" }, { label: "Frais notaire", name: "fraisNotaire" },
              { label: "Travaux", name: "travaux" }, { label: "Frais bancaires", name: "fraisBancaires" },
              { label: "Apport", name: "apport" }, { label: "Taxe foncière/an", name: "taxeFonciere" },
              { label: "Charges copro/an", name: "chargesAn" }, { label: "Assurance PNO/an", name: "assurancePNO" },
            ].map(function(f) {
              return (<div key={f.name}><label style={labelS}>{f.label}</label><input type="number" name={f.name} value={inputs[f.name]} onChange={handleChange} step="100" style={fieldS} /></div>);
            })}
            <div><label style={labelS}>Taux crédit</label><div style={{ display: "flex", gap: 4 }}><input type="number" name="tauxCredit" value={inputs.tauxCredit} onChange={handleChange} step="0.05" style={fieldS} /><span style={{ fontSize: 10, color: "#94a3b8", alignSelf: "center" }}>%</span></div></div>
            <div><label style={labelS}>Durée crédit</label><div style={{ display: "flex", gap: 4 }}><input type="number" name="dureeCredit" value={inputs.dureeCredit} onChange={handleChange} step="1" style={fieldS} /><span style={{ fontSize: 10, color: "#94a3b8", alignSelf: "center" }}>ans</span></div></div>
          </div>
        </div>
        <div style={SECTION}>
          <SectionHeader icon="📍" title="Données marché" />
          {/* Recherche commune */}
          <div style={{ marginBottom: 10, position: "relative" }}>
            <label style={labelS}>Commune</label>
            <input type="text" value={communeSearch} onChange={function(e) { setCommuneSearch(e.target.value); }}
              placeholder="Rechercher une commune..." style={fieldS} />
            {suggestions.length > 0 && (
              <div style={{ position: "absolute", top: "100%", left: 0, right: 0, background: "#fff", borderRadius: 10, boxShadow: "0 4px 20px rgba(0,0,0,0.15)", zIndex: 20, maxHeight: 180, overflowY: "auto", border: "1px solid rgba(148,163,184,0.3)" }}>
                {suggestions.map(function(c) {
                  return (<div key={c.nom} onClick={function() { selectCommune(c.nom); }}
                    style={{ padding: "8px 12px", cursor: "pointer", fontSize: 12, borderBottom: "1px solid rgba(148,163,184,0.1)" }}
                    onMouseEnter={function(e) { e.currentTarget.style.background = "rgba(99,102,241,0.06)"; }}
                    onMouseLeave={function(e) { e.currentTarget.style.background = ""; }}>
                    <span style={{ fontWeight: 600 }}>{c.nom}</span>
                    {c.scores && <span style={{ color: "#94a3b8", marginLeft: 8 }}>Score: {c.scores.global ? c.scores.global.toFixed(1) : "—"}/10</span>}
                  </div>);
                })}
              </div>
            )}
          ...
          {donneesCommune ? (
            <div style={{ display: "grid", gridTemplateColumns: "1fr 1fr", gap: 8 }}>
              {[
                { label: "Population", value: fmtEur(population).replace(" €", " hab."), color: "#0f172a" },
                { label: "Taux d'étudiants", value: tauxEtudiants > 0 ? tauxEtudiants + " %" : "—", color: tauxEtudiants > 5 ? "#16a34a" : "#64748b" },
                { label: "Loyer médian", value: loyerMedianM2 > 0 ? fmt(loyerMedianM2, 1) + " €/m²" : "—", color: "#0ea5e9" },
                { label: "Tension locative", value: donneesCommune.demographie && donneesCommune.demographie.tension_locative_pct != null ? donneesCommune.demographie.tension_locative_pct + " %" : "—", color: "#6366f1" },
                { label: "Vacance", value: donneesCommune.demographie && donneesCommune.demographie.vacance_pct != null ? donneesCommune.demographie.vacance_pct + " %" : "—", color: "#dc2626" },
                { label: "Score global", value: donneesCommune.scores ? donneesCommune.scores.global.toFixed(1) + "/10" : "—", color: "#4338ca" },
              ].map(function(k) {
                return (<div key={k.label} style={{ background: "rgba(248,250,252,0.8)", borderRadius: 10, padding: "8px 10px" }}>
                  <div style={{ fontSize: 10, color: "#94a3b8" }}>{k.label}</div>
                  <div style={{ fontSize: 14, fontWeight: 700, color: k.color }}>{k.value}</div>
                </div>);
              })}
              {tauxEtudiants > 5 && <div style={{ gridColumn: "1 / -1", background: "rgba(22,163,74,0.08)", borderRadius: 8, padding: "6px 10px", fontSize: 11, color: "#16a34a", fontWeight: 500 }}>
                ✓ Taux d'étudiants élevé ({tauxEtudiants}%) — favorable à la colocation
              </div>}
              {population < 2000 && <div style={{ gridColumn: "1 / -1", background: "rgba(220,38,38,0.08)", borderRadius: 8, padding: "6px 10px", fontSize: 11, color: "#dc2626", fontWeight: 500 }}>
                ⚠ Petite commune ({fmt(population, 0)} hab.) — courte durée risquée
              </div>}
            </div>
          ) : (
            <div style={{ textAlign: "center", padding: 20, color: "#94a3b8", fontSize: 12 }}>Renseigne une commune pour voir les données marché</div>
          )}
        </div>
      </div>

      {/* Lots avec mode d'exploitation */}
      <div style={SECTION}>
        <div style={{ display: "flex", justifyContent: "space-between", alignItems: "center", marginBottom: 12 }}>
          <SectionHeader icon="🏘️" title={"Lots / Logements (" + lots.length + ")"} />
          <button onClick={addLot} style={{ background: "linear-gradient(135deg,#6366f1,#38bdf8)", border: "none", borderRadius: 8, padding: "6px 14px", color: "#fff", cursor: "pointer", fontSize: 12, fontWeight: 600 }}>+ Ajouter un lot</button>
        </div>
        <div style={{ display: "flex", flexDirection: "column", gap: 10 }}>
          {lotsResults.map(function(lot, idx) {
            var mode = MODES[lot.mode] || MODES.nu;
            return (
              <div key={lot.id} style={{ background: "rgba(248,250,252,0.9)", borderRadius: 14, padding: "12px 14px", border: "2px solid " + mode.color + "30" }}>
                {/* Header lot */}
                <div style={{ display: "flex", justifyContent: "space-between", alignItems: "center", marginBottom: 8 }}>
                  <div style={{ display: "flex", alignItems: "center", gap: 8 }}>
                    <div style={{ width: 24, height: 24, borderRadius: 8, background: mode.color, display: "flex", alignItems: "center", justifyContent: "center", fontSize: 12, color: "#fff", fontWeight: 700 }}>#{idx + 1}</div>
                    <input value={lot.nom} onChange={function(e) { updateLot(lot.id, "nom", e.target.value); }}
                      style={{ fontWeight: 600, fontSize: 13, color: "#0f172a", background: "transparent", border: "none", outline: "none", borderBottom: "1px dashed " + mode.color, maxWidth: 120 }} />
                    <span style={{ fontSize: 12 }}>{mode.icon}</span>
                    <span style={{ fontSize: 10, fontWeight: 600, color: mode.color, background: mode.color + "15", padding: "2px 8px", borderRadius: 6 }}>{mode.nom}</span>
                  </div>
                  <div style={{ display: "flex", alignItems: "center", gap: 6 }}>
                    <span style={{ fontSize: 14, fontWeight: 800, color: lot.loyerMois > 0 ? "#16a34a" : "#94a3b8" }}>{fmtEur(lot.loyerMois)}/mois</span>
                    <button onClick={function() { removeLot(lot.id); }} style={{ background: "rgba(220,38,38,0.08)", border: "none", borderRadius: 6, padding: "3px 8px", color: "#dc2626", cursor: lots.length > 1 ? "pointer" : "not-allowed", fontSize: 11, opacity: lots.length > 1 ? 1 : 0.3 }}>✕</button>
                  </div>
                </div>
                {/* Mode selector */}
                <div style={{ display: "flex", gap: 4, marginBottom: 8 }}>
                  {Object.entries(MODES).map(function(entry) {
                    ...
                    var isActive = lot.mode === mId;
                    return (<button key={mId} onClick={function() { updateLot(lot.id, "mode", mId); }}
                      style={{ flex: 1, padding: "6px 4px", borderRadius: 8, border: isActive ? "2px solid " + m.color : "1px solid rgba(148,163,184,0.25)", background: isActive ? m.color + "12" : "transparent", cursor: "pointer", fontSize: 11, fontWeight: 600, color: isActive ? m.color : "#94a3b8", display: "flex", alignItems: "center", justifyContent: "center", gap: 4 }}>
                      <span>{m.icon}</span> {m.nom}
                    </button>);
                  })}
                </div>
                {/* Champs lot */}
                <div style={{ display: "grid", gridTemplateColumns: "1fr 1fr 1fr 1fr", gap: 8 }}>
                  <div><label style={labelS}>Surface</label><div style={{ display: "flex", gap: 3 }}><input type="number" value={lot.surface} step="1" onChange={function(e) { updateLot(lot.id, "surface", e.target.value); }} style={fieldS} /><span style={{ fontSize: 10, color: "#94a3b8", alignSelf: "center" }}>m²</span></div></div>
                  <div><label style={labelS}>{lot.mode === "coloc" ? "Nb chambres" : "Pièces"}</label><input type="number" value={lot.nbChambres} step="1" onChange={function(e) { updateLot(lot.id, "nbChambres", e.target.value); }} style={fieldS} /></div>
                  <div><label style={labelS}>Loyer nu marché</label><div style={{ display: "flex", gap: 3 }}><input type="number" value={lot.loyerNuMarche} step="50" onChange={function(e) { updateLot(lot.id, "loyerNuMarche", e.target.value); }} style={fieldS} /><span style={{ fontSize: 10, color: "#94a3b8", alignSelf: "center" }}>€</span></div></div>
                  <div style={{ display: "flex", flexDirection: "column", justifyContent: "center", gap: 3, background: mode.color + "08", borderRadius: 8, padding: "4px 8px" }}>
                    <div style={{ fontSize: 10, color: "#94a3b8" }}>Loyer estimé</div>
                    <div style={{ fontSize: 15, fontWeight: 800, color: mode.color }}>{fmtEur(lot.loyerMois)}/mois</div>
                    <div style={{ fontSize: 9, color: "#94a3b8" }}>Occup. {mode.occup} mois · {lot.mobilierInit > 0 ? "Mobilier " + fmtEur(lot.mobilierInit) : "Pas de mobilier"}</div>
                  </div>
                </div>
              </div>
            );
      ...
      {/* Synthèse immeuble */}
      <div style={SECTION}>
        <SectionHeader icon="📋" title="Synthèse de l'immeuble" />
        <div style={{ overflowX: "auto" }}>
          <table style={{ width: "100%", borderCollapse: "collapse", fontSize: 12 }}>
            <thead>
              <tr style={{ borderBottom: "2px solid rgba(148,163,184,0.2)" }}>
                {["Lot", "Mode", "Surface", "Loyer/mois", "Loyer/an", "Charges spec.", "Mobilier", ""].map(function(h) {
                  return <th key={h} style={{ padding: "8px 8px", textAlign: h === "Lot" ? "left" : "center", fontSize: 10, fontWeight: 600, color: "#64748b", textTransform: "uppercase" }}>{h}</th>;
                })}
              </tr>
            </thead>
            <tbody>
              {lotsResults.map(function(lot, idx) {
                var m = MODES[lot.mode];
                return (<tr key={lot.id} style={{ borderBottom: "1px solid rgba(148,163,184,0.1)", background: idx % 2 === 0 ? "rgba(248,250,252,0.5)" : "transparent" }}>
                  <td style={{ padding: "7px 8px", fontWeight: 600, color: "#334155" }}>{lot.nom}</td>
                  <td style={{ padding: "7px 8px", textAlign: "center" }}><span style={{ fontSize: 10, fontWeight: 600, color: m.color, background: m.color + "15", padding: "2px 6px", borderRadius: 4 }}>{m.icon} {m.nom}</span></td>
                  <td style={{ padding: "7px 8px", textAlign: "center" }}>{lot.surface} m²</td>
                  <td style={{ padding: "7px 8px", textAlign: "center", fontWeight: 600, color: "#16a34a" }}>{fmtEur(lot.loyerMois)}</td>
                  <td style={{ padding: "7px 8px", textAlign: "center" }}>{fmtEur(Math.round(lot.loyerAn))}</td>
                  <td style={{ padding: "7px 8px", textAlign: "center", color: "#dc2626" }}>{fmtEur(Math.round(lot.totalChargesLot))}</td>
                  <td style={{ padding: "7px 8px", textAlign: "center" }}>{lot.mobilierInit > 0 ? fmtEur(lot.mobilierInit) : "—"}</td>
                </tr>);
              })}
              {/* Totaux */}
              <tr style={{ borderTop: "2px solid rgba(148,163,184,0.3)", background: "rgba(99,102,241,0.04)" }}>
                <td style={{ padding: "8px", fontWeight: 700, color: "#4338ca" }}>TOTAL</td>
                <td style={{ padding: "8px", textAlign: "center", fontSize: 10, color: "#64748b" }}>{lots.length} lots</td>
                <td style={{ padding: "8px", textAlign: "center", fontWeight: 600 }}>{fmt(totalSurface, 0)} m²</td>
                <td style={{ padding: "8px", textAlign: "center", fontWeight: 700, color: "#16a34a" }}>{fmtEur(Math.round(totalLoyerAn / 12))}</td>
                <td style={{ padding: "8px", textAlign: "center", fontWeight: 700 }}>{fmtEur(Math.round(totalLoyerAn))}</td>
                <td style={{ padding: "8px", textAlign: "center", fontWeight: 600, color: "#dc2626" }}>{fmtEur(Math.round(totalChargesLotsAn))}</td>
                <td style={{ padding: "8px", textAlign: "center", fontWeight: 600 }}>{totalMobilier > 0 ? fmtEur(totalMobilier) : "—"}</td>
              </tr>
            </tbody>
          </table>
        </div>
        ...
        <div style={{ display: "grid", gridTemplateColumns: "repeat(auto-fit, minmax(140px, 1fr))", gap: 10, marginTop: 14 }}>
          {[
            { label: "Loyers annuels", value: fmtEur(Math.round(totalLoyerAn)), color: "#16a34a" },
            { label: "Charges annuelles", value: "−" + fmtEur(Math.round(totalChargesAn)), color: "#dc2626" },
            { label: "Crédit annuel", value: "−" + fmtEur(Math.round(creditAn)), color: "#d97706" },
            { label: "Cash-flow annuel", value: (cashflowAn >= 0 ? "+" : "") + fmtEur(Math.round(cashflowAn)), color: cashflowAn >= 0 ? "#16a34a" : "#dc2626" },
            { label: "Cash-flow mensuel", value: (cashflowMois >= 0 ? "+" : "") + fmtEur(Math.round(cashflowMois)), color: cashflowMois >= 0 ? "#16a34a" : "#dc2626" },
            { label: "Rendement brut", value: fmtPct(rendBrut), color: "#6366f1" },
          ].map(function(k) {
            return (<div key={k.label} style={{ background: "rgba(255,255,255,0.8)", borderRadius: 12, padding: "10px 12px", border: "1px solid rgba(148,163,184,0.15)", textAlign: "center" }}>
              <div style={{ fontSize: 10, color: "#94a3b8" }}>{k.label}</div>
        ...
        <div style={{ display: "flex", gap: 6, marginTop: 10, flexWrap: "wrap" }}>
          {Object.entries(MODES).map(function(entry) {
            var mId = entry[0]; var m = entry[1];
            var count = lots.filter(function(l) { return l.mode === mId; }).length;
            if (count === 0) return null;
            return (<span key={mId} style={{ fontSize: 11, fontWeight: 600, color: m.color, background: m.color + "12", padding: "4px 10px", borderRadius: 8, border: "1px solid " + m.color + "25" }}>
              {m.icon} {m.nom} : {count} lot{count > 1 ? "s" : ""}
            </span>);
          })}
        </div>
      </div>
  ...
  return (
    <div style={{ display: "flex", minHeight: "100vh", background: "radial-gradient(ellipse at top left,#bfdbfe 0,transparent 50%),radial-gradient(ellipse at top right,#fce7f3 0,transparent 50%),radial-gradient(ellipse at bottom center,#d1fae5 0,#f1f5f9 60%)", fontFamily: "-apple-system,BlinkMacSystemFont,system-ui,sans-serif" }}>
      ...
      <main style={{ flex: 1, padding: 20, overflowY: "auto", minWidth: 0 }}>
        ...
        {onglet === "exploitation" && <SimulateurExploitation />}
