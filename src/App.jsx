  ...
  for (var y = 1; y <= dur + 5; y++) {
    ...
    if (useLots) {
      lotsEffectifs.forEach(function(lot) {
        ...
        var moisActifs = Math.max(0, Math.min(moisFinAn, moisFinAn) - debut);
        moisActifs = Math.min(moisActifs, 12);
        // Proratiser le taux d'occupation par lot (proportionnel au poids du lot)
        var poidsLot = lotLoyer / totalLoyerLots;
        var occupLot = Math.min(tauxOccup * poidsLot, moisActifs);
          ...
          return (
            <div key={lot.id} style={{ background: "rgba(248,250,252,0.9)", borderRadius: 12, padding: "10px 12px", border: "1px solid rgba(148,163,184,0.25)" }}>
              ...
              <div style={{ display: "grid", gridTemplateColumns: "1fr 1fr 1fr 1fr 1fr", gap: 6 }}>
                {[{ label: "Surface (m²)", field: "surface", unit: "m²", step: "1" }, { label: "Loyer HC/mois", field: "loyer", unit: "€", step: "50" }, { label: "Travaux", field: "travaux", unit: "€", step: "500" }, { label: "Charges/an", field: "charges", unit: "€", step: "100" }, { label: "Début loyer", field: "debutLoyerMois", unit: "mois", step: "1" }].map(function(col) {
