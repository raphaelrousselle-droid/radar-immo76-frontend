...
function SimulationProjet({ photos, setPhotos, projets, setProjets, projetACharger, onProjetCharge }) {
  ...
  return (
    <div style={{ display: "flex", flexDirection: "column", gap: 14 }}>
      ...
      {activeTab === "dvf" && (
        <DVFPanel commune={inputs.commune} />
      )}
    </div>
  );
}
...
function DVFPanel({ commune }) {
  ...
  var _t = React.useState(""); var dvfType = _t[0]; var setDvfType = _t[1];
  var chartEvolRef = React.useRef(null);
  var chartTypesRef = React.useRef(null);
  var chartEvolInst = React.useRef(null);
  var chartTypesInst = React.useRef(null);

  // Recherche du code INSEE à partir du nom de commune
  var COMMUNES_76 = [
  ...
  function fmtD(n) { if (n == null || isNaN(n)) return "—"; return Math.round(n).toLocaleString("fr-FR"); }
  function fmtPriceD(n) { if (n == null || isNaN(n)) return "—"; if (n >= 1000000) return (n/1000000).toFixed(1).replace(".",",") + " M€"; return fmtD(n) + " €"; }

  function analyserMutations(mutations) {
    var avecSurf = mutations.filter(function(m) { return m.surface_reelle_bati > 0 && m.valeur_fonciere > 0; });
    var prixM2List = avecSurf.map(function(m) { return m.valeur_fonciere / m.surface_reelle_bati; });
    var prixM2Moy = prixM2List.length ? prixM2List.reduce(function(a,b){return a+b;},0)/prixM2List.length : null;
    var sorted = [...prixM2List].sort(function(a,b){return a-b;});
    var prixM2Med = sorted.length ? sorted[Math.floor(sorted.length/2)] : null;
    var prixMoyList = mutations.filter(function(m){return m.valeur_fonciere>0;}).map(function(m){return m.valeur_fonciere;});
    var prixMoyMoy = prixMoyList.length ? prixMoyList.reduce(function(a,b){return a+b;},0)/prixMoyList.length : null;
    var parAnnee = {};
    mutations.forEach(function(m) {
      if (!m.date_mutation) return;
      var a = m.date_mutation.slice(0,4);
      if (!parAnnee[a]) parAnnee[a] = { nb: 0, prixM2: [] };
      parAnnee[a].nb++;
      if (m.surface_reelle_bati>0 && m.valeur_fonciere>0) parAnnee[a].prixM2.push(m.valeur_fonciere/m.surface_reelle_bati);
    });
    var annees = Object.keys(parAnnee).sort();
    var evolPrix = annees.map(function(a){ var l=parAnnee[a].prixM2; return l.length ? Math.round(l.reduce(function(s,v){return s+v;},0)/l.length) : null; });
    var types = {};
    mutations.forEach(function(m) { var t = m.type_local||"Inconnu"; types[t]=(types[t]||0)+1; });
    return { prixM2Moy: prixM2Moy, prixM2Med: prixM2Med, total: mutations.length, prixMoyMoy: prixMoyMoy, annees: annees, evolPrix: evolPrix, types: types };
  }

  async function lancerRecherche(codeInsee, typeFiltre) {
    setDvfStatus("loading"); setDvfData(null); setDvfError(null);
    try {
      // API DVF publique — api.cquest.org (données DGFiP open data)
      var params = new URLSearchParams({ code_commune: codeInsee, nature_mutation: "Vente" });
      if (typeFiltre) params.set("type_local", typeFiltre);
      var url = "/api/dvf?" + params.toString(); // proxy Vercel - évite les blocages CORS
      var resp = await fetch(url);
      if (!resp.ok) throw new Error("Erreur API (" + resp.status + ")");
      var data = await resp.json();
      // Normaliser la réponse (format JSON ou GeoJSON)
      var results = [];
      if (Array.isArray(data.resultats)) results = data.resultats;
      else if (Array.isArray(data.features)) results = data.features.map(function(f){ return f.properties||f; });
      else if (Array.isArray(data.mutations)) results = data.mutations;
      else if (Array.isArray(data)) results = data;
      if (!results.length) { setDvfStatus("empty"); return; }
      setDvfData({ mutations: results, stats: analyserMutations(results) });
      setDvfStatus("done");
    } catch(e) {
  ...
  // Auto-lancer si commune connue
  React.useEffect(function() {
    if (!commune) return;
    var lower = commune.toLowerCase();
    var found = COMMUNES_76.find(function(c){ return c.nom.toLowerCase() === lower || c.nom.toLowerCase().includes(lower); });
    if (found) lancerRecherche(found.code, dvfType);
  }, [commune]);

  // Charger Chart.js si besoin
  React.useEffect(function() {
    if (!window.Chart) {
      var s = document.createElement("script");
  ...
  // Dessiner les graphiques quand les données arrivent
  React.useEffect(function() {
    if (dvfStatus !== "done" || !dvfData) return;
    var stats = dvfData.stats;
    var tryDraw = function() {
      if (!window.Chart) { setTimeout(tryDraw, 200); return; }
      // Graphique évolution
      if (chartEvolRef.current) {
        if (chartEvolInst.current) chartEvolInst.current.destroy();
        chartEvolInst.current = new window.Chart(chartEvolRef.current, {
          type: "line",
          data: {
            labels: stats.annees,
            datasets: [{ label: "€/m²", data: stats.evolPrix, borderColor: "#6366f1", backgroundColor: "rgba(99,102,241,0.07)", borderWidth: 2, pointRadius: 4, pointBackgroundColor: "#6366f1", tension: 0.3, fill: true, spanGaps: true }]
          },
          options: { responsive: true, maintainAspectRatio: false, plugins: { legend: { display: false }, tooltip: { callbacks: { label: function(ctx) { return fmtD(ctx.parsed.y) + " €/m²"; } } } }, scales: { x: { ticks: { color: "#94a3b8", font: { size: 10 } }, grid: { color: "rgba(148,163,184,0.1)" } }, y: { ticks: { color: "#94a3b8", font: { size: 10 }, callback: function(v) { return fmtD(v) + " €"; } }, grid: { color: "rgba(148,163,184,0.08)" } } } }
        });
      }
      // Graphique types
      if (chartTypesRef.current) {
        if (chartTypesInst.current) chartTypesInst.current.destroy();
        var labels = Object.keys(stats.types).slice(0,6);
        var values = labels.map(function(l){return stats.types[l];});
        var colors = ["#6366f1","#38bdf8","#16a34a","#f59e0b","#f97316","#dc2626"];
        chartTypesInst.current = new window.Chart(chartTypesRef.current, {
          type: "doughnut",
          data: { labels: labels, datasets: [{ data: values, backgroundColor: colors.slice(0,labels.length), borderColor: "#fff", borderWidth: 2 }] },
          options: { responsive: true, maintainAspectRatio: false, plugins: { legend: { position: "bottom", labels: { color: "#64748b", font: { size: 10 }, boxWidth: 10, padding: 8 } } } }
        });
      }
    };
    setTimeout(tryDraw, 80);
    return function() {
      if (chartEvolInst.current) { chartEvolInst.current.destroy(); chartEvolInst.current = null; }
      if (chartTypesInst.current) { chartTypesInst.current.destroy(); chartTypesInst.current = null; }
    };
  }, [dvfStatus, dvfData]);
  ...
  var found = commune ? COMMUNES_76.find(function(c){ return c.nom.toLowerCase() === commune.toLowerCase() || c.nom.toLowerCase().includes(commune.toLowerCase()); }) : null;
  ...
  return (
    <div style={{ display: "flex", flexDirection: "column", gap: 14 }}>
      <div style={SECTION}>
        ...
        {/* Filtre type + bouton relancer */}
        <div style={{ display: "flex", gap: 10, alignItems: "center", flexWrap: "wrap", marginBottom: 14 }}>
          {!commune && (
            <div style={{ fontSize: 13, color: "#94a3b8", fontStyle: "italic" }}>
              💡 Renseigne d'abord la commune dans l'onglet <strong>Paramètres</strong> pour charger les données DVF.
            </div>
          )}
          {commune && !found && (
            <div style={{ fontSize: 13, color: "#f97316" }}>
              ⚠️ Commune « {commune} » non trouvée dans la liste DVF 76. Vérifie l'orthographe.
            </div>
          )}
          {found && (
            <>
              ...
              <select value={dvfType} onChange={function(e){setDvfType(e.target.value);}}
                style={{ background: "rgba(248,250,252,0.9)", border: "1px solid rgba(148,163,184,0.4)", borderRadius: 8, padding: "5px 10px", fontSize: 12, color: "#0f172a", outline: "none" }}>
                <option value="">Tous types</option>
                <option value="Appartement">Appartements</option>
                <option value="Maison">Maisons</option>
                <option value="Local industriel. commercial ou assimilé">Locaux commerciaux</option>
              </select>
              <button onClick={function(){lancerRecherche(found.code, dvfType);}}
        ...
        {dvfStatus === "loading" && (
          <div style={{ textAlign: "center", padding: "40px 20px", color: "#94a3b8" }}>
            ...
            <div style={{ fontSize: 13 }}>Interrogation de l'API DVF Cerema…</div>
          </div>
        )}
        {dvfStatus === "error" && (
          <div style={{ background: "rgba(220,38,38,0.07)", border: "1px solid rgba(220,38,38,0.2)", borderRadius: 10, padding: "10px 14px", fontSize: 13, color: "#dc2626" }}>
            ⚠️ Erreur : {dvfError}. Vérifie ta connexion et réessaie.
          </div>
        )}
        {dvfStatus === "empty" && (
          <div style={{ textAlign: "center", padding: "30px 20px", color: "#94a3b8", fontSize: 13 }}>
            🔍 Aucune transaction DVF trouvée pour cette commune avec ce filtre.
          </div>
        )}

        {dvfStatus === "done" && dvfData && (function() {
          var stats = dvfData.stats;
          var recent = [...dvfData.mutations].sort(function(a,b){ return (b.date_mutation||"").localeCompare(a.date_mutation||""); }).slice(0,20);
          var getType = function(type) {
            if (!type) return { label:"?", bg:"rgba(148,163,184,0.1)", color:"#94a3b8" };
            var t = type.toLowerCase();
            if (t.includes("appartement")) return { label:"APT", bg:"rgba(99,102,241,0.1)", color:"#6366f1" };
            if (t.includes("maison")) return { label:"MAI", bg:"rgba(217,119,6,0.1)", color:"#b45309" };
            if (t.includes("local")) return { label:"LOC", bg:"rgba(22,163,74,0.1)", color:"#15803d" };
            if (t.includes("terrain")) return { label:"TER", bg:"rgba(14,165,233,0.1)", color:"#0369a1" };
            return { label:type.slice(0,3).toUpperCase(), bg:"rgba(148,163,184,0.1)", color:"#64748b" };
          };
          return (
            <div>
              {/* KPIs */}
              <div style={{ display: "grid", gridTemplateColumns: "repeat(auto-fit, minmax(140px, 1fr))", gap: 10, marginBottom: 16 }}>
                {[
                  { label: "Prix moyen/m²", value: fmtD(stats.prixM2Moy) + " €/m²", color: "#6366f1", icon: "📐" },
                  { label: "Prix médian/m²", value: fmtD(stats.prixM2Med) + " €/m²", color: "#0ea5e9", icon: "⚖️" },
                  { label: "Transactions", value: fmtD(stats.total), color: "#16a34a", icon: "📋" },
                  { label: "Prix vente moyen", value: fmtPriceD(stats.prixMoyMoy), color: "#d97706", icon: "💶" },
                ].map(function(k) {
                  return (
                    <div key={k.label} style={Object.assign({}, cardS, { textAlign: "center", borderTop: "3px solid " + k.color })}>
              ...
              <div style={{ display: "grid", gridTemplateColumns: "1fr 1fr", gap: 12, marginBottom: 16 }}>
                <div style={cardS}>
                  <div style={{ fontSize: 11, fontWeight: 600, color: "#64748b", textTransform: "uppercase", letterSpacing: "0.06em", marginBottom: 10 }}>Évolution prix/m² par année</div>
                  <div style={{ height: 180, position: "relative" }}>
                    <canvas ref={chartEvolRef}></canvas>
                  </div>
                </div>
                <div style={cardS}>
                  <div style={{ fontSize: 11, fontWeight: 600, color: "#64748b", textTransform: "uppercase", letterSpacing: "0.06em", marginBottom: 10 }}>Types de biens vendus</div>
                  <div style={{ height: 180, position: "relative" }}>
                    <canvas ref={chartTypesRef}></canvas>
                  </div>
                </div>
              </div>

              {/* Tableau 20 dernières ventes */}
              <div style={{ borderRadius: 12, overflow: "hidden", border: "1px solid rgba(148,163,184,0.18)" }}>
                <div style={{ background: "rgba(241,245,249,0.8)", padding: "8px 14px", fontSize: 11, fontWeight: 700, color: "#64748b", textTransform: "uppercase", letterSpacing: "0.06em", display: "flex", justifyContent: "space-between" }}>
                  <span>20 dernières ventes enregistrées</span>
                  <span style={{ color: "#94a3b8", fontWeight: 400 }}>Source : DVF Cerema 2014-2025</span>
                </div>
                <div style={{ overflowX: "auto" }}>
                  <table style={{ width: "100%", borderCollapse: "collapse", fontSize: 12 }}>
                    <thead>
                      <tr style={{ background: "rgba(248,250,252,0.9)" }}>
                        {["Date","Type","Surface","Prix","Prix/m²","Pièces"].map(function(h) {
                          return <th key={h} style={{ padding: "7px 12px", textAlign: "left", fontSize: 10, fontWeight: 700, color: "#94a3b8", textTransform: "uppercase", letterSpacing: "0.05em", whiteSpace: "nowrap", borderBottom: "1px solid rgba(148,163,184,0.15)" }}>{h}</th>;
                        })}
                      </tr>
                    </thead>
                    <tbody>
                      {recent.map(function(m, i) {
                        var t = getType(m.type_local);
                        var prixM2 = (m.surface_reelle_bati>0 && m.valeur_fonciere>0) ? fmtD(m.valeur_fonciere/m.surface_reelle_bati) + " €/m²" : "—";
                        var surf = m.surface_reelle_bati>0 ? fmtD(m.surface_reelle_bati) + " m²" : "—";
                        var date = m.date_mutation ? m.date_mutation.split("T")[0] : "—";
                        return (
                          <tr key={i} style={{ borderBottom: "1px solid rgba(148,163,184,0.08)" }}>
                            <td style={{ padding: "7px 12px", color: "#94a3b8", fontFamily: "monospace", fontSize: 11 }}>{date}</td>
                            <td style={{ padding: "7px 12px" }}>
                              <span style={{ fontSize: 10, fontWeight: 700, padding: "2px 7px", borderRadius: 5, background: t.bg, color: t.color }}>{t.label}</span>
                            </td>
                            <td style={{ padding: "7px 12px", color: "#334155" }}>{surf}</td>
                            <td style={{ padding: "7px 12px", fontWeight: 700, color: "#0f172a" }}>{fmtPriceD(m.valeur_fonciere)}</td>
                            <td style={{ padding: "7px 12px", color: "#6366f1", fontWeight: 600 }}>{prixM2}</td>
                            <td style={{ padding: "7px 12px", color: "#94a3b8" }}>{m.nombre_pieces_principales || "—"}</td>
                          </tr>
                        );
                      })}
                    </tbody>
              ...
              <div style={{ marginTop: 10, fontSize: 10, color: "#94a3b8", textAlign: "center" }}>
                Données DVF officielles — DGFiP / Cerema open data · Ventes à titre onéreux hors successions et donations
              </div>
            </div>
          );
        })()}
