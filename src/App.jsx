...
function SimulateurSCI() {
  ...
  const calc = useMemo(function() {
    ...
    const chargesTotal = pf(sciVals.chargesAn) + pf(sciVals.taxeFonciereAn) + assurancePNO + gestionAn + pf(sciVals.provisionTravauxAn) + pf(sciVals.expertComptableAn) + pf(sciVals.fraisBancairesAn);

    // Calcul intérêts année 1
    ...
    // Résultat fiscal IS
    const resultatFiscal = Math.max(0, loyersAn - chargesTotal - interetsAn1 - amortTotalAn);
    const tIS = pf(sciVals.tauxIS) / 100;
    const isAn = resultatFiscal * tIS;

    // Trésorerie
    const tresoAn = loyersAn - chargesTotal - remboursementAn - isAn;
    const tresoMois = tresoAn / 12;

    // Résultat comptable avant IS (pour dividendes)
    const resultatAvtIS = Math.max(0, loyersAn - chargesTotal - interetsAn1 - amortTotalAn);
    const resultatApresIS = resultatAvtIS - isAn;
  ...
  return (
    <div style={{ display: "flex", flexDirection: "column", gap: 14 }}>
      ...
      <div style={{ background: "linear-gradient(135deg,rgba(99,102,241,0.08),rgba(56,189,248,0.08))", borderRadius: 14, padding: "12px 16px", border: "1px solid rgba(99,102,241,0.2)", fontSize: 12, color: "#334155" }}>
        <strong>SCI à l'IS</strong> — Société Civile Immobilière soumise à l'Impôt sur les Sociétés. Les loyers sont imposés après déduction des amortissements (bâtiment + travaux), ce qui réduit fortement la base imposable. Les bénéfices sont distribués sous forme de dividendes (PFU 30%).
      </div>
      ...
      <div style={{ display: "grid", gridTemplateColumns: "repeat(auto-fit, minmax(280px, 1fr))", gap: 14 }}>
        ...
        <div style={{ display: "flex", flexDirection: "column", gap: 14 }}>
          ...
          <div style={SECTION}>
            ...
            <div style={{ display: "flex", flexDirection: "column", gap: 10 }}>
              ...
              <SCIField sciVals={sciVals} onChange={handleSciChange} field="fraisAchat" label="Frais de notaire + agence" unit="€" step="500" />
          ...
          <div style={SECTION}>
            ...
            <div style={{ display: "flex", flexDirection: "column", gap: 10 }}>
              ...
              <SCIField sciVals={sciVals} onChange={handleSciChange} field="assurancePNOAn" label="Assurance PNO" unit="€/an" hint="0 = calculé auto (0.12% du prix)" />
              <SCIField sciVals={sciVals} onChange={handleSciChange} field="gestionPct" label="Gestion locative" unit="%/an" step="0.5" />
              <SCIField sciVals={sciVals} onChange={handleSciChange} field="provisionTravauxAn" label="Provision travaux" unit="€/an" />
          ...
          <div style={SECTION}>
            ...
            <div style={{ display: "flex", flexDirection: "column", gap: 10 }}>
              ...
              <SCIField sciVals={sciVals} onChange={handleSciChange} field="partTerrain" label="Part du terrain (non amortissable)" unit="%" step="1" hint="Généralement 15-20% du prix" />
              ...
              <div>
                <label style={labelS}>Taux dividendes (PFU)</label>
                <div style={{ display: "flex", gap: 6 }}>
              ...
              <SCIField sciVals={sciVals} onChange={handleSciChange} field="nbParts" label="Nombre de parts sociales" unit="parts" step="1" hint="Pour calcul dividende par associé" />
        ...
        <div style={{ display: "flex", flexDirection: "column", gap: 14 }}>
          ...
          <div style={SECTION}>
            ...
            <StatRow label="Charges immeuble + TF + assurance" value={"– " + fmtEur(pf(sciVals.chargesAn) + pf(sciVals.taxeFonciereAn) + calc.assurancePNO)} color="#64748b" />
            <StatRow label="Gestion locative" value={"– " + fmtEur(calc.gestionAn)} color="#64748b" />
            <StatRow label="Provision travaux" value={"– " + fmtEur(pf(sciVals.provisionTravauxAn))} color="#64748b" />
            ...
            <StatRow label="Amortissement bâtiment" value={"– " + fmtEur(calc.amortBatimentAn)} color="#6366f1" />
            <StatRow label="Amortissement travaux" value={"– " + fmtEur(calc.amortTravauxAn)} color="#6366f1" />
            <div style={{ margin: "6px 0", height: 1, background: "rgba(148,163,184,0.15)" }} />
            <StatRow label="Résultat fiscal imposable" value={fmtEur(calc.resultatFiscal)} bold color={calc.resultatFiscal > 0 ? "#334155" : "#16a34a"} />
          ...
          <div style={SECTION}>
            ...
            <StatRow label="– Remboursement crédit (cap. + int.)" value={"– " + fmtEur(calc.remboursementAn)} color="#d97706" />
          ...
          <div style={SECTION}>
            ...
            <div style={{ marginTop: 8, background: "rgba(254,243,199,0.7)", borderRadius: 8, padding: "7px 10px", fontSize: 10, color: "#92400e", border: "1px solid rgba(251,191,36,0.3)" }}>
              ⚠ En SCI IS, les dividendes ne peuvent être distribués qu'après approbation des comptes. La réserve légale (10% du capital) doit être constituée en priorité.
            </div>
          </div>

          {/* Amortissements détail */}
          <div style={SECTION}>
            <SectionHeader icon="🏗️" title="Détail des amortissements" />
            ...
            <StatRow label={"Amort. bâtiment/an (" + sciVals.coefAmortBatiment + "%)"} value={fmtEur(calc.amortBatimentAn)} color="#6366f1" />
            <StatRow label={"Amort. travaux/an (" + sciVals.coefAmortTravaux + "%)"} value={fmtEur(calc.amortTravauxAn)} color="#6366f1" />
            <StatRow label="Amortissement total/an" value={fmtEur(calc.amortTotalAn)} bold color="#4338ca" border={false} />
            <div style={{ marginTop: 8, background: "rgba(99,102,241,0.07)", borderRadius: 8, padding: "7px 10px", fontSize: 10, color: "#4338ca" }}>
              💡 Sans amortissements, la base IS serait <strong>{fmtEur(Math.max(0, calc.loyersAn - calc.chargesTotal - calc.interetsAn1))}</strong> au lieu de <strong>{fmtEur(calc.resultatFiscal)}</strong>
            </div>
          </div>
      ...
      <div style={{ background: "rgba(241,245,249,0.8)", borderRadius: 12, padding: "10px 14px", fontSize: 11, color: "#64748b", border: "1px solid rgba(148,163,184,0.2)" }}>
        ℹ️ Calculs basés sur l'année 1 du crédit (intérêts maximum). Les intérêts diminuent chaque année, augmentant progressivement la base IS. Expert-comptable obligatoire en SCI IS (dépôt comptes au greffe). La SCI IS est conseillée pour des durées de détention longues (piège de la plus-value à la revente).
      </div>
    </div>
  );
}
