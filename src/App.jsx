...
function SimulationProjet({ photos, setPhotos, projets, setProjets, projetACharger, onProjetCharge }) {
  ...
  useEffect(function() {
    ...
    try {
      ...
      setCommuneSuggestions(cached.filter(function(c) {
  ...
  var fetchDonneesCommune = useCallback(function(nom) {
    if (!nom) return;
    setLoadingCommune(true); setDonneesCommune(null);
  ...
  useEffect(function() {
    if (inputs.commune) { setCommuneSearch(inputs.commune); fetchDonneesCommune(inputs.commune); }
  }, [inputs.commune]);

  // ── Commune + données marché ────────────────────────────────────────────────
  const [communeSearch, setCommuneSearch] = useState(inputs.commune || "");
  const [communeSuggestions, setCommuneSuggestions] = useState([]);
  const [donneesCommune, setDonneesCommune] = useState(null);
  const [loadingCommune, setLoadingCommune] = useState(false);
  const communeTimer = React.useRef(null);

  // Autocomplétion : cherche dans le cache sessionStorage des communes
  useEffect(function() {
    if (!communeSearch || communeSearch.length < 2) { setCommuneSuggestions([]); return; }
    try {
      var cached = JSON.parse(sessionStorage.getItem(CACHE_KEY)) || [];
          ...
          <div style={Object.assign({}, SECTION, { padding: "14px 20px" })}>
            ...
            <div style={{ position: "relative" }}>
              <input value={communeSearch}
                onChange={function(e) { setCommuneSearch(e.target.value); if (!e.target.value) { setInputs(function(p) { return Object.assign({}, p, { commune: "" }); }); setDonneesCommune(null); } }}
              ...
              {communeSuggestions.length > 0 && (
                <div style={{ position: "absolute", top: "100%", left: 0, right: 0, zIndex: 50, background: "#fff", border: "1px solid rgba(148,163,184,0.3)", borderRadius: 10, boxShadow: "0 8px 24px rgba(0,0,0,0.12)", marginTop: 4, overflow: "hidden" }}>
