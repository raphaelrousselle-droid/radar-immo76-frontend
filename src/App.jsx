...
function projeterCashFlow(inputs, regimeNom) {
  ...
  const debutLoyerMois = Math.max(0, Math.round(pf(inputs.debutLoyerMois)));
  var solde = sommeEmpruntee;
  var moisGlobal = 0; // compteur de mois depuis le début
  const data = [];
  for (var y = 1; y <= dur + 5; y++) {
    var creditAn = 0;
    var interetsAn = 0;
    // Mois par mois pour cette année
    for (var m = 0; m < 12; m++) {
      moisGlobal++;
      if (moisGlobal <= nMois && solde > 0) {
        if (tMensuel > 0) {
          ...
          if (moisGlobal <= differeMois) {
            // Différé : on ne paie que les intérêts, pas le capital
            creditAn += intM;
          } else {
            creditAn += mensualite;
    ...
    // Loyers : décalés de debutLoyerMois
    var moisLoyerActifs = Math.max(0, Math.min(12, (y * 12) - debutLoyerMois) - Math.max(0, ((y - 1) * 12) - debutLoyerMois));
    // Plus simple : sur les 12 mois de l'année y, combien sont après debutLoyerMois ?
    var moisDebutAn = (y - 1) * 12; // mois 0..11 de l'année y = moisDebutAn..moisDebutAn+11
    var moisFinAn = y * 12;
    var loyerDebut = Math.max(moisDebutAn, debutLoyerMois);
    var loyerFin = moisFinAn;
    moisLoyerActifs = Math.max(0, loyerFin - loyerDebut);
    moisLoyerActifs = Math.min(moisLoyerActifs, 12);
    // Appliquer le taux d'occupation (prorata sur les mois actifs)
    var tauxOccup = pf(inputs.tauxOccupation); // mois/an
    var occupMoisEffectif = Math.min(tauxOccup, moisLoyerActifs);
    const loyersAn = pf(inputs.loyerMensuelHC) * occupMoisEffectif * Math.pow(1.01, y - 1);
...
const DEFAULT_INPUTS = {
  ...
  differeMois: "0", debutLoyerMois: "0",
...
function SimulationProjet({ photos, setPhotos, projets, setProjets, projetACharger, onProjetCharge }) {
  ...
  const exportPDF = async function() {
    ...
    if (photos && photos[0] && photos[0].url) {
      try { doc.addImage(photos[0].url, "JPEG", 14, photoY, 80, 60, "", "MEDIUM"); photoY += 58; }
      catch(e) { photoY += 2; }
    }
          ...
          <div style={SECTION}>
            ...
            <div style={{ marginTop: 8, background: "rgba(241,245,249,0.8)", borderRadius: 12, padding: "10px 12px" }}>
              ...
              <div style={{ display: "grid", gridTemplateColumns: "1fr 1fr", gap: 8 }}>
                ...
                <InputField label="Début des loyers (mois)" name="debutLoyerMois" value={inputs.debutLoyerMois} onChange={handleChange} unit="mois" step="1" />
              </div>
