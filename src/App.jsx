...
function calcBienAnnee(bien, anneeReelle) {
  ...
  // Approche simplifiée : mensualité × nb mois actifs dans l'année (max 12)
  if (M > 0 && duree > 0) {
    // Mois de crédit total : de moisAchat à nMois
    // Mois dans cette année civile : de max(0, moisDebutAnnee) à min(moisFinAnnee, nMois)
    var debutEffectif = Math.max(0, moisDebutAnnee);
    var finEffectif = Math.min(moisFinAnnee, nMois);
    var moisCreditDansAnnee = Math.max(0, finEffectif - debutEffectif);
    moisCreditDansAnnee = Math.min(moisCreditDansAnnee, 12); // jamais plus de 12

    if (moisCreditDansAnnee > 0) {
      remboursementAn = mensualite * moisCreditDansAnnee;

      // Calcul des intérêts : on avance le solde jusqu'au début de l'année
      var solde = M;
      for (var mm = 0; mm < debutEffectif && solde > 0 && mm < nMois; mm++) {
        var intM2 = solde * tMensuel;
        solde = Math.max(0, solde - (mensualite - intM2));
      }
      // Puis on calcule les intérêts pour les mois de cette année
      for (var m = 0; m < moisCreditDansAnnee && solde > 0; m++) {
        var intM = solde * tMensuel;
        interetsAn += intM;
