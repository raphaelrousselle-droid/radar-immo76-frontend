...
const ICONS = {
  ...
  "ti-map": "M3 7l6-3 6 3 6-3v13l-6 3-6-3-6 3V7zm6-3v13m6-10v13",
  "ti-calculator": "M4 3h16a1 1 0 0 1 1 1v16a1 1 0 0 1-1 1H4a1 1 0 0 1-1-1V4a1 1 0 0 1 1-1zm3 4h10M7 12h2m4 0h2M7 16h2m4 0h2",
  "ti-users": "M17 21v-2a4 4 0 0 0-4-4H5a4 4 0 0 0-4 4v2M9 11a4 4 0 1 0 0-8 4 4 0 0 0 0 8zm14 10v-2a4 4 0 0 0-3-3.87M16 3.13a4 4 0 0 1 0 7.75",
};
...
function AppMain({ user, onLogout }) {
  ...
  const navItems = [
    ...
    { id: "emprunt",       label: "Capacité d'emprunt",      icon: "ti-calculator" },
    { id: "carte",         label: "Carte des communes",      icon: "ti-map" },
  ...
  const titres = {
    ...
    emprunt:       { h: "Calculateur de capacité d'emprunt", sub: "Simule ta capacité d'emprunt réelle en intégrant les revenus locatifs, co-emprunteur et charges existantes." },
    carte:         { h: "Carte des communes — Seine-Maritime", sub: "Scores d'investissement par commune, visualisés sur carte interactive." },
  ...
  return (
    <div style={{ display: "flex", minHeight: "100vh", background: "#FFF8F3", fontFamily: "'Plus Jakarta Sans',system-ui,sans-serif" }}>
      ...
      <main style={{ flex: 1, padding: 20, overflowY: "auto", minWidth: 0 }}>
        ...
        {onglet === "emprunt"   && <CalculateurEmprunt />}
        {onglet === "carte"     && <CarteCommunes />}
...
function ComparateurDPE() {
  ...
  // Impact loyer et valeur par classe (écart vs D = référence neutre)
  var DPE_LOYER   = { A:0.15, B:0.10, C:0.05, D:0, E:-0.05, F:-0.10, G:-0.15 };
  var DPE_VALEUR  = { A:0.10, B:0.07, C:0.03, D:0, E:-0.04, F:-0.10, G:-0.15 };

  // Postes de travaux avec coûts réels (€/m² ou €/logement)
  var POSTES = [
    { id:"iso_combles",  label:"Isolation combles perdus",        icon:"ti-home",             unite:"m²",      cout:{ G:25, F:22, E:18, D:12, C:0, B:0, A:0 }, desc:"Soufflage laine minérale — éligible MaPrimeRénov" },
    { id:"iso_murs_ext", label:"Isolation thermique extérieure",  icon:"ti-wind",             unite:"m²",      cout:{ G:200, F:180, E:150, D:100, C:0, B:0, A:0 }, desc:"ITE bardage ou enduit — subventionnable CEE" },
    { id:"iso_murs_int", label:"Isolation thermique intérieure",  icon:"ti-wind",             unite:"m²",      cout:{ G:80, F:70, E:55, D:35, C:0, B:0, A:0 }, desc:"ITI laine de verre — moins de surface habitable" },
    { id:"iso_plancher", label:"Isolation plancher bas",          icon:"ti-droplet",          unite:"m²",      cout:{ G:30, F:26, E:20, D:13, C:0, B:0, A:0 }, desc:"Sous-sol ou vide sanitaire" },
    { id:"fenetres",     label:"Remplacement fenêtres DV/TV",     icon:"ti-sun",              unite:"unité",   cout:{ G:1100, F:950, E:750, D:0, C:0, B:0, A:0 }, desc:"Double ou triple vitrage PVC/alu — NON éligible MaPrimeRénov seul" },
    { id:"pac_air_eau",  label:"Pompe à chaleur air/eau",         icon:"ti-thermometer",      unite:"logement",cout:{ G:14000, F:12000, E:10000, D:7000, C:0, B:0, A:0 }, desc:"Éligible MaPrimeRénov — jusqu'à 5000€ de prime" },
    { id:"pac_air_air",  label:"PAC air/air (climatisation réversible)", icon:"ti-thermometer", unite:"logement", cout:{ G:4000, F:3500, E:2800, D:0, C:0, B:0, A:0 }, desc:"Non éligible MaPrimeRénov" },
    { id:"chaudiere_gaz",label:"Chaudière gaz à condensation",   icon:"ti-flame",            unite:"logement",cout:{ G:5000, F:4500, E:3500, D:0, C:0, B:0, A:0 }, desc:"NON éligible MaPrimeRénov depuis 2024" },
    { id:"poele_granule",label:"Poêle ou insert granulés",        icon:"ti-flame",            unite:"logement",cout:{ G:5500, F:5000, E:4200, D:0, C:0, B:0, A:0 }, desc:"Éligible MaPrimeRénov — jusqu'à 2500€" },
    { id:"chauffe_eau",  label:"Chauffe-eau thermodynamique",     icon:"ti-thermometer",      unite:"logement",cout:{ G:3200, F:2800, E:2200, D:0, C:0, B:0, A:0 }, desc:"Éligible MaPrimeRénov — jusqu'à 1200€" },
    { id:"vmc_df",       label:"VMC double flux",                 icon:"ti-plug",             unite:"logement",cout:{ G:5000, F:4500, E:3500, D:0, C:0, B:0, A:0 }, desc:"Éligible CEE — améliore qualité air et DPE" },
  ];

  // Aides MaPrimeRénov par poste (taux modeste/intermédiaire/supérieur, hors ressources)
  var AIDES = {
    iso_combles:  { taux: 0.75, plafond: 75*70 },
    iso_murs_ext: { taux: 0.75, plafond: 150*70 },
    iso_murs_int: { taux: 0.75, plafond: 150*70 },
    iso_plancher: { taux: 0.75, plafond: 75*70 },
    fenetres:     { taux: 0,    plafond: 0 },      // non éligible
    pac_air_eau:  { taux: 0.50, plafond: 5000 },
    pac_air_air:  { taux: 0,    plafond: 0 },      // non éligible
    chaudiere_gaz:{ taux: 0,    plafond: 0 },      // non éligible depuis 2024
    poele_granule:{ taux: 0.50, plafond: 2500 },
    chauffe_eau:  { taux: 0.50, plafond: 1200 },
    vmc_df:       { taux: 0.35, plafond: 2000 },
  };

  // ── State ──────────────────────────────────────────────────────────────────
  var _addr   = React.useState(""); var adresse = _addr[0]; var setAdresse = _addr[1];
  var _sugg   = React.useState([]); var suggestions = _sugg[0]; var setSuggestions = _sugg[1];
  var _search = React.useState("idle"); var searchStatus = _search[0]; var setSearchStatus = _search[1];
  var _dpeData= React.useState(null); var dpeData = _dpeData[0]; var setDpeData = _dpeData[1];
  var _selDpe = React.useState(null); var selectedDpe = _selDpe[0]; var setSelectedDpe = _selDpe[1];
  var _manuel = React.useState(false); var modeManuel = _manuel[0]; var setModeManuel = _manuel[1];
  var _dpeM   = React.useState("F"); var dpeManuel = _dpeM[0]; var setDpeManuel = _dpeM[1];
  var _dpeCible= React.useState("D"); var dpeCible = _dpeCible[0]; var setDpeCible = _dpeCible[1];
  var _surf   = React.useState("70"); var surface = _surf[0]; var setSurface = _surf[1];
  var _loyer  = React.useState("700"); var loyer = _loyer[0]; var setLoyer = _loyer[1];
  var _valeur = React.useState("150000"); var valeur = _valeur[0]; var setValeur = _valeur[1];
  var _nb     = React.useState("1"); var nbLog = _nb[0]; var setNbLog = _nb[1];
  var _postesS= React.useState(function() {
    var s = {}; POSTES.forEach(function(p) { s[p.id] = { actif: false, qte: "" }; }); return s;
  }); var postesState = _postesS[0]; var setPostesState = _postesS[1];
  ...
  var dpeActuel = modeManuel ? dpeManuel : (selectedDpe ? selectedDpe.etiquette_dpe : null);

  // ── Recherche adresse ──────────────────────────────────────────────────────
  var timerRef = React.useRef(null);
  var handleAdresseChange = function(val) {
    setAdresse(val);
    clearTimeout(timerRef.current);
    if (val.length < 4) { setSuggestions([]); return; }
    timerRef.current = setTimeout(async function() {
      try {
        var r = await fetch("/api/dpe?action=geocode&q=" + encodeURIComponent(val + " Seine-Maritime"));
  ...
  var choisirAdresse = async function(feat) {
    setAdresse(feat.properties.label);
    setSuggestions([]);
    setSearchStatus("loading");
    setDpeData(null); setSelectedDpe(null);
    var [lon, lat] = feat.geometry.coordinates;
    try {
      // 1. Chercher par coordonnées GPS (plus fiable)
      var r = await fetch("/api/dpe?action=dpe_coords&lat=" + lat + "&lon=" + lon);
      var data = await r.json();
      if (data.results && data.results.length > 0) {
        setDpeData(data.results);
        setSelectedDpe(data.results[0]);
        var surf = pf(data.results[0].surface_habitable_logement);
        if (surf > 0) setSurface(String(Math.round(surf)));
        setSearchStatus("found");
      } else {
        // 2. Essayer par adresse texte
        var r2 = await fetch("/api/dpe?action=dpe_adresse&adresse_brut=" + encodeURIComponent(feat.properties.label));
        var data2 = await r2.json();
        if (data2.results && data2.results.length > 0) {
          setDpeData(data2.results);
          setSelectedDpe(data2.results[0]);
          var surf2 = pf(data2.results[0].surface_habitable_logement);
          if (surf2 > 0) setSurface(String(Math.round(surf2)));
          setSearchStatus("found");
        } else {
  ...
  // ── Calculs ────────────────────────────────────────────────────────────────
  var surf = pf(surface); var loyerN = pf(loyer); var valeurN = pf(valeur); var nb = Math.max(1,pf(nbLog));

  var coutBrut = POSTES.reduce(function(acc,p) {
    var ps = postesState[p.id];
    if (!ps.actif || !dpeActuel) return acc;
    var qte = pf(ps.qte) || (p.unite==="logement" ? nb : surf);
    return acc + (p.cout[dpeActuel]||0) * qte;
  }, 0);

  var aideTotal = POSTES.reduce(function(acc,p) {
    var ps = postesState[p.id];
    if (!ps.actif || !dpeActuel) return acc;
    var aide = AIDES[p.id];
    if (!aide || aide.taux===0) return acc;
    var qte = pf(ps.qte) || (p.unite==="logement" ? nb : surf);
    var coutPoste = (p.cout[dpeActuel]||0)*qte;
    return acc + Math.min(coutPoste*aide.taux, aide.plafond*nb);
  }, 0);

  var coutNet = coutBrut - aideTotal;
  var bonusLoyerA = DPE_LOYER[dpeActuel]||0;
  var bonusLoyerC = DPE_LOYER[dpeCible]||0;
  var nouveauLoyer = loyerN*(1+bonusLoyerC-bonusLoyerA);
  var gainLoyerMois = nouveauLoyer-loyerN;
  var gainLoyerAn = gainLoyerMois*12*nb;
  var bonusValA = DPE_VALEUR[dpeActuel]||0;
  var bonusValC = DPE_VALEUR[dpeCible]||0;
  var gainValeur = valeurN*(bonusValC-bonusValA);
  var retourInvest = coutNet>0 && gainLoyerAn>0 ? coutNet/gainLoyerAn : null;
  var gainNet10ans = gainValeur + gainLoyerAn*10 - coutNet;
  var estPassoire = dpeActuel && DPE_ILLEGAL.includes(dpeActuel);
  ...
  return (
    <div style={{ display:"flex", flexDirection:"column", gap:16 }}>
      ...
      {/* ── Recherche adresse ── */}
      <div style={SECTION}>
        <SectionHeader icon="ti-map-pin" title="Rechercher un bien par adresse" />
        <div style={{ position:"relative" }}>
          <div style={{ display:"flex", gap:10, alignItems:"center" }}>
            <div style={{ flex:1, position:"relative" }}>
              <input value={adresse} onChange={function(e){handleAdresseChange(e.target.value);}}
                placeholder="Ex : 12 rue de la République, Dieppe"
                style={Object.assign({},inputS,{paddingLeft:36})} />
              <div style={{ position:"absolute", left:10, top:"50%", transform:"translateY(-50%)" }}>
                <Icon name="ti-map-pin" size={16} color="#A8A29E" />
              </div>
            </div>
            <button onClick={function(){setModeManuel(!modeManuel);}}
              style={{ background:modeManuel?"#1C1917":"#FFF3EC", border:"1.5px solid "+(modeManuel?"#1C1917":"#FFE8D9"), borderRadius:10, padding:"8px 14px", color:modeManuel?"#fff":"#F97316", fontSize:13, fontWeight:600, cursor:"pointer", whiteSpace:"nowrap" }}>
              {modeManuel ? "← Retour recherche" : "Saisie manuelle"}
            </button>
          </div>
          {/* Suggestions */}
          {suggestions.length > 0 && (
            <div style={{ position:"absolute", top:"calc(100% + 6px)", left:0, right:0, background:"#fff", border:"1.5px solid #FFE8D9", borderRadius:12, overflow:"hidden", zIndex:50, boxShadow:"0 4px 20px rgba(249,115,22,0.1)" }}>
              {suggestions.map(function(f,i) {
                return (
                  <div key={i} onClick={function(){choisirAdresse(f);}}
                    style={{ padding:"10px 14px", cursor:"pointer", fontSize:13, borderBottom:i<suggestions.length-1?"1px solid #FFE8D9":"none", display:"flex", alignItems:"center", gap:8 }}
                    onMouseEnter={function(e){e.currentTarget.style.background="#FFF8F3";}}
                    onMouseLeave={function(e){e.currentTarget.style.background="#fff";}}>
                    <Icon name="ti-map-pin" size={14} color="#F97316" />
                    <span style={{ flex:1 }}>{f.properties.label}</span>
                    <span style={{ fontSize:11, color:"#A8A29E" }}>{f.properties.postcode}</span>
                  </div>
                );
              })}
            </div>
        ...
        {searchStatus === "loading" && (
          <div style={{ marginTop:12, display:"flex", alignItems:"center", gap:8, color:"#A8A29E", fontSize:13 }}>
            <div style={{ width:16, height:16, border:"2px solid #FFE8D9", borderTopColor:"#F97316", borderRadius:"50%", animation:"spin 0.8s linear infinite" }} />
            Interrogation de la base ADEME…
          </div>
        )}

        {searchStatus === "found" && dpeData && (
          <div style={{ marginTop:12 }}>
            <div style={{ fontSize:13, fontWeight:600, color:"#16a34a", marginBottom:8 }}>
              ✓ {dpeData.length} DPE trouvé{dpeData.length>1?"s":""} dans la base ADEME
            </div>
            <div style={{ display:"flex", flexDirection:"column", gap:6 }}>
              {dpeData.map(function(d,i) {
                var isSelected = selectedDpe && selectedDpe.numero_dpe === d.numero_dpe;
                return (
                  <div key={i} onClick={function(){setSelectedDpe(d); if(pf(d.surface_habitable_logement)>0) setSurface(String(Math.round(pf(d.surface_habitable_logement))));}}
                    style={{ background:isSelected?"#FFF3EC":"#FFF8F3", border:"1.5px solid "+(isSelected?"#F97316":"#FFE8D9"), borderRadius:10, padding:"10px 14px", cursor:"pointer", display:"flex", alignItems:"center", gap:12, transition:"all 0.15s" }}>
                    <DPEBadge dpe={d.etiquette_dpe} />
                    <div style={{ flex:1 }}>
                      <div style={{ fontSize:13, fontWeight:600, color:"#1C1917" }}>
                        DPE du {d.date_etablissement_dpe ? d.date_etablissement_dpe.split("T")[0] : "—"}
                      </div>
                      <div style={{ fontSize:11, color:"#A8A29E", marginTop:1 }}>
                        {d.surface_habitable_logement ? Math.round(pf(d.surface_habitable_logement))+" m²" : ""} · {d.type_batiment||""} · {d.consommation_energie ? Math.round(pf(d.consommation_energie))+" kWh/m²/an" : ""}
                      </div>
                      <div style={{ fontSize:11, color:"#A8A29E" }}>
                        {d.type_energie_principale_chauffage||""} · {d.type_vitrage||""}
                      </div>
                    </div>
                    <div style={{ textAlign:"right" }}>
                      <div style={{ fontSize:12, color:"#A8A29E" }}>GES</div>
                      <DPEBadge dpe={d.etiquette_ges||"—"} />
                    </div>
                  </div>
                );
              })}
        ...
        {searchStatus === "notfound" && (
          <div style={{ marginTop:12, background:"rgba(245,158,11,0.07)", border:"1.5px solid rgba(245,158,11,0.25)", borderRadius:10, padding:"10px 14px", fontSize:13, color:"#92400e" }}>
            ⚠ Aucun DPE trouvé dans la base ADEME pour cette adresse (diagnostic antérieur à juillet 2021 ou non enregistré). Utilise la saisie manuelle pour continuer.
          </div>
        )}

        {searchStatus === "error" && (
          <div style={{ marginTop:12, background:"rgba(220,38,38,0.07)", border:"1.5px solid rgba(220,38,38,0.25)", borderRadius:10, padding:"10px 14px", fontSize:13, color:"#dc2626" }}>
            Erreur de connexion à l'API ADEME. Réessaie ou utilise la saisie manuelle.
          </div>
        )}
      </div>

      {/* ── Saisie manuelle ou sélection DPE cible ── */}
      {(modeManuel || searchStatus==="notfound" || (searchStatus==="found" && selectedDpe)) && (
        <div style={SECTION}>
          <SectionHeader icon="ti-leaf" title="Paramètres" />
          <div style={{ display:"grid", gridTemplateColumns:"repeat(auto-fit, minmax(150px,1fr))", gap:12, marginBottom:16 }}>
            {[
              { label:"Surface (m²)", val:surface, set:setSurface },
              { label:"Loyer mensuel (€)", val:loyer, set:setLoyer },
              { label:"Valeur du bien (€)", val:valeur, set:setValeur },
              { label:"Nb de logements", val:nbLog, set:setNbLog },
            ].map(function(f) {
              return (
                <div key={f.label}>
                  <div style={{ fontSize:12, color:"#A8A29E", marginBottom:5, fontWeight:600 }}>{f.label}</div>
                  <input type="number" value={f.val} min="0" onChange={function(e){f.set(e.target.value);}} style={inputS} />
                </div>
              );
            })}
          </div>
          ...
          {/* DPE actuel (manuel) */}
          {(modeManuel || searchStatus==="notfound") && (
            <div style={{ marginBottom:16 }}>
              <div style={{ fontSize:12, color:"#A8A29E", marginBottom:8, fontWeight:600 }}>DPE ACTUEL (saisie manuelle)</div>
              <div style={{ display:"flex", gap:6 }}>
                {DPE_ORDER.map(function(d) {
                  return (
                    <button key={d} onClick={function(){setDpeManuel(d);}}
                      style={{ width:42, height:42, borderRadius:10, border:dpeManuel===d?"2.5px solid #1C1917":"2px solid transparent", background:DPE_COLORS[d], color:"#fff", fontWeight:800, fontSize:16, cursor:"pointer", opacity:dpeManuel===d?1:0.55, transform:dpeManuel===d?"scale(1.1)":"scale(1)", transition:"all 0.15s" }}>
                      {d}
                    </button>
                  );
                })}
          ...
          {dpeActuel && (
            <div>
              ...
              <div style={{ display:"flex", gap:6, alignItems:"center" }}>
                {DPE_ORDER.map(function(d) {
                  var isBetter = DPE_ORDER.indexOf(d) < DPE_ORDER.indexOf(dpeActuel);
                  return (
                    <button key={d} onClick={function(){if(isBetter)setDpeCible(d);}}
                      style={{ width:42, height:42, borderRadius:10, border:dpeCible===d?"2.5px solid #1C1917":"2px solid transparent", background:DPE_COLORS[d], color:"#fff", fontWeight:800, fontSize:16, cursor:isBetter?"pointer":"not-allowed", opacity:dpeCible===d?1:isBetter?0.65:0.2, transform:dpeCible===d?"scale(1.1)":"scale(1)", transition:"all 0.15s" }}>
                      {d}
                    </button>
                  );
                })}
                <div style={{ marginLeft:8, display:"flex", gap:8, alignItems:"center" }}>
                  <DPEBadge dpe={dpeActuel} lg={true} />
                  <span style={{ fontSize:18, color:"#A8A29E" }}>→</span>
                  <DPEBadge dpe={dpeCible} lg={true} />
                </div>
              </div>

              {/* Alerte passoire */}
              {estPassoire && (
                <div style={{ marginTop:10, background:"rgba(220,38,38,0.07)", border:"1.5px solid rgba(220,38,38,0.25)", borderRadius:10, padding:"10px 14px", display:"flex", gap:8, alignItems:"flex-start" }}>
                  <Icon name="ti-flame" size={18} color="#dc2626" />
                  <div style={{ fontSize:13 }}>
                    <strong style={{ color:"#dc2626" }}>Passoire thermique — Location interdite depuis 2025.</strong>
                    <span style={{ color:"#78716C" }}> Une rénovation est obligatoire pour louer légalement ce logement.</span>
                  </div>
                </div>
              )}
            </div>
        ...
        <div style={SECTION}>
          ...
          <div style={{ display:"flex", flexDirection:"column", gap:8 }}>
            {POSTES.map(function(p) {
              var ps = postesState[p.id];
              var cout = p.cout[dpeActuel]||0;
              var aide = AIDES[p.id];
              var qteDefaut = p.unite==="logement"?nb:surf;
              var qte = pf(ps.qte)||qteDefaut;
              var coutPoste = ps.actif ? cout*qte : 0;
              var aidePoste = ps.actif && aide && aide.taux>0 ? Math.min(coutPoste*aide.taux, aide.plafond*nb) : 0;
              var eligible = aide && aide.taux > 0;

              return (
                <div key={p.id} style={{ background:ps.actif?"#FFF3EC":"#FFF8F3", border:"1.5px solid "+(ps.actif?"#FFE8D9":"#F5F0EB"), borderRadius:12, padding:"12px 14px", transition:"all 0.15s", opacity:cout===0?0.5:1 }}>
                  <div style={{ display:"flex", alignItems:"flex-start", gap:10 }}>
                    <button onClick={function(){if(cout>0)updatePoste(p.id,"actif",!ps.actif);}}
                      style={{ width:22, height:22, borderRadius:6, border:"2px solid "+(ps.actif?"#F97316":"#D6D3D1"), background:ps.actif?"#F97316":"#fff", cursor:cout>0?"pointer":"not-allowed", display:"flex", alignItems:"center", justifyContent:"center", flexShrink:0, marginTop:2, transition:"all 0.15s" }}>
                      {ps.actif && <Icon name="ti-check-circle" size={13} color="#fff" />}
                    </button>
                    <Icon name={p.icon} size={18} color={ps.actif?"#F97316":"#A8A29E"} style={{ marginTop:2, flexShrink:0 }} />
                    <div style={{ flex:1 }}>
                      <div style={{ fontSize:14, fontWeight:600, color:"#1C1917" }}>{p.label}</div>
                      <div style={{ fontSize:11, color:"#A8A29E", marginTop:1 }}>{p.desc}</div>
                      {eligible && <div style={{ fontSize:11, color:"#16a34a", fontWeight:600, marginTop:2 }}>✓ Éligible MaPrimeRénov — jusqu'à {fmtEur(aide.plafond)} par logement</div>}
                      {!eligible && cout>0 && <div style={{ fontSize:11, color:"#dc2626", marginTop:2 }}>✗ Non éligible MaPrimeRénov</div>}
                    </div>
                    <div style={{ textAlign:"right", flexShrink:0 }}>
                      {cout>0 ? (
                        <>
                          ...
                          {ps.actif && aidePoste>0 && <div style={{ fontSize:11, color:"#16a34a" }}>−{fmtEur(aidePoste)} aide</div>}
                        </>
                      ) : (
                        <div style={{ fontSize:11, color:"#16a34a", fontWeight:600 }}>Déjà optimal</div>
                      )}
                    </div>
                    {ps.actif && p.unite!=="logement" && (
                      <div style={{ display:"flex", alignItems:"center", gap:5, flexShrink:0 }}>
                        <input type="number" value={ps.qte} placeholder={String(Math.round(qteDefaut))}
                          onChange={function(e){updatePoste(p.id,"qte",e.target.value);}}
                          style={{ width:64, background:"#fff", border:"1.5px solid #FFE8D9", borderRadius:8, padding:"4px 8px", fontSize:13, textAlign:"center", outline:"none" }} />
                        <span style={{ fontSize:11, color:"#A8A29E" }}>{p.unite}</span>
                      </div>
                    )}
                  </div>
                </div>
      ...
      {/* ── Résultats ── */}
      {dpeActuel && coutBrut > 0 && (
        <div style={{ display:"grid", gridTemplateColumns:"1fr 1fr", gap:12 }}>

          {/* Coûts */}
          <div style={{ display:"flex", flexDirection:"column", gap:12 }}>
            <div style={SECTION}>
              <SectionHeader icon="ti-coin" title="Budget travaux" />
              {[
                { label:"Coût brut total", val:fmtEur(coutBrut), color:"#1C1917", bold:true },
                { label:"Aides MaPrimeRénov estimées", val:"−"+fmtEur(aideTotal), color:"#16a34a", bold:false },
                { label:"Coût net après aides", val:fmtEur(coutNet), color:"#F97316", bold:true },
              ].map(function(r,i) {
                return (
                  <div key={i} style={{ display:"flex", justifyContent:"space-between", padding:"9px 0", borderBottom:i<2?"1px solid #FFE8D9":"none" }}>
                    <span style={{ fontSize:13, color:"#78716C" }}>{r.label}</span>
                    <span style={{ fontSize:r.bold?16:14, fontWeight:r.bold?800:600, color:r.color }}>{r.val}</span>
                  </div>
                );
              })}
              <div style={{ marginTop:10, background:"#FFF8F3", borderRadius:10, padding:"8px 12px", fontSize:11, color:"#78716C", lineHeight:1.6 }}>
                ℹ Les aides MaPrimeRénov affichées correspondent au plafond maximum. Le montant réel dépend de vos revenus et du type de travaux. Consulte maprimerenov.gouv.fr.
              </div>
            </div>
            ...
            <div style={SECTION}>
              ...
              <div style={{ display:"flex", alignItems:"center", justifyContent:"center", gap:20, padding:"16px 0" }}>
                <div style={{ textAlign:"center" }}>
                  <div style={{ fontSize:11, color:"#A8A29E", marginBottom:8, fontWeight:600 }}>ACTUEL</div>
                  <DPEBadge dpe={dpeActuel} lg={true} />
                  {estPassoire && <div style={{ fontSize:10, color:"#dc2626", fontWeight:700, marginTop:6 }}>INTERDIT</div>}
                </div>
                <Icon name="ti-trending-up" size={24} color="#A8A29E" />
                <div style={{ textAlign:"center" }}>
                  <div style={{ fontSize:11, color:"#A8A29E", marginBottom:8, fontWeight:600 }}>CIBLE</div>
                  <DPEBadge dpe={dpeCible} lg={true} />
                  {seraLegal && <div style={{ fontSize:10, color:"#16a34a", fontWeight:700, marginTop:6 }}>LÉGAL</div>}
                </div>
              </div>
              {/* Barre DPE */}
              <div style={{ display:"flex", gap:3, height:14, borderRadius:8, overflow:"hidden", marginTop:4 }}>
                {DPE_ORDER.map(function(d) {
                  var isA = d===dpeActuel; var isC = d===dpeCible;
                  return (
                    <div key={d} style={{ flex:1, background:DPE_COLORS[d], opacity:isA||isC?1:0.25, border:isA||isC?"2px solid #1C1917":"none", borderRadius:4, position:"relative" }}>
                      {(isA||isC) && <div style={{ position:"absolute", top:-18, left:"50%", transform:"translateX(-50%)", fontSize:9, fontWeight:800, color:DPE_COLORS[d] }}>{d}</div>}
                    </div>
                  );
                })}
              </div>
              {selectedDpe && selectedDpe.consommation_energie && (
                <div style={{ marginTop:12, fontSize:12, color:"#78716C" }}>
                  Consommation actuelle : <strong style={{ color:"#1C1917" }}>{Math.round(pf(selectedDpe.consommation_energie))} kWh/m²/an</strong>
                </div>
              )}
            </div>
          </div>
          ...
          <div style={{ display:"flex", flexDirection:"column", gap:12 }}>
            <div style={SECTION}>
              ...
              {[
                { label:"Loyer actuel ("+dpeActuel+")", val:fmtEur(loyerN)+"/mois", color:"#78716C" },
                { label:"Loyer estimé après ("+dpeCible+")", val:fmtEur(nouveauLoyer)+"/mois", color:"#1C1917" },
                { label:"Gain mensuel / logement", val:(gainLoyerMois>=0?"+":"")+fmtEur(gainLoyerMois)+"/mois", color:gainLoyerMois>=0?"#16a34a":"#dc2626" },
                { label:"Gain annuel ("+nb+" logement"+(nb>1?"s":"")+")", val:(gainLoyerAn>=0?"+":"")+fmtEur(gainLoyerAn)+"/an", color:gainLoyerAn>=0?"#16a34a":"#dc2626" },
              ].map(function(r,i) {
                return (
                  <div key={i} style={{ display:"flex", justifyContent:"space-between", padding:"9px 0", borderBottom:i<3?"1px solid #FFE8D9":"none" }}>
                    <span style={{ fontSize:13, color:"#78716C" }}>{r.label}</span>
            ...
            <div style={SECTION}>
              ...
              {[
                { label:"Valeur actuelle ("+dpeActuel+")", val:fmtEur(valeurN), color:"#78716C" },
                { label:"Valeur estimée ("+dpeCible+")", val:fmtEur(valeurN+gainValeur), color:"#1C1917" },
                { label:"Plus-value DPE estimée", val:(gainValeur>=0?"+":"")+fmtEur(gainValeur), color:gainValeur>=0?"#16a34a":"#dc2626" },
              ].map(function(r,i) {
                return (
                  <div key={i} style={{ display:"flex", justifyContent:"space-between", padding:"9px 0", borderBottom:i<2?"1px solid #FFE8D9":"none" }}>
                    <span style={{ fontSize:13, color:"#78716C" }}>{r.label}</span>
            ...
            <div style={Object.assign({},SECTION,{ background:gainNet10ans>=0?"rgba(22,163,74,0.04)":"rgba(220,38,38,0.04)", border:"1.5px solid "+(gainNet10ans>=0?"#BBF7D0":"rgba(220,38,38,0.2)") })}>
              ...
              {[
                ...
                return (
                  <div key={i} style={{ display:"flex", justifyContent:"space-between", alignItems:"center", padding:"9px 0", borderBottom:i<2?"1px solid #FFE8D9":"none" }}>
                    <span style={{ fontSize:13, color:"#78716C" }}>{r.label}</span>
                    <span style={{ fontSize:r.big?18:14, fontWeight:r.big?800:600, color:r.color }}>{r.val}</span>
                  </div>
                );
              })}
            </div>
      ...
      {dpeActuel && coutBrut === 0 && (
        <div style={{ textAlign:"center", padding:"40px 20px", color:"#A8A29E" }}>
          <Icon name="ti-hammer" size={48} color="#FFE8D9" />
      ...
      {!dpeActuel && searchStatus !== "loading" && (
        <div style={{ textAlign:"center", padding:"40px 20px", color:"#A8A29E" }}>
          <Icon name="ti-leaf" size={48} color="#FFE8D9" />
          <div style={{ fontSize:16, fontWeight:600, color:"#78716C", marginTop:12 }}>Recherche une adresse ou utilise la saisie manuelle</div>
          <div style={{ fontSize:13, marginTop:6 }}>L'outil interroge la base ADEME de plus de 12 millions de DPE</div>
        </div>
      )}
    </div>
  );
...
function CalculateurEmprunt() {
  var pf = function(v) { var n = parseFloat(String(v).replace(",",".")); return isNaN(n)?0:n; };
  var fmtEur = function(v) { return Math.round(v).toLocaleString("fr-FR")+" €"; };
  var fmt1 = function(v) { return v.toFixed(1).replace(".",","); };

  // ── State ──────────────────────────────────────────────────────────────────
  var _r1 = React.useState("4000"); var rev1 = _r1[0]; var setRev1 = _r1[1];
  var _r2 = React.useState("0");    var rev2 = _r2[0]; var setRev2 = _r2[1];
  var _rl = React.useState("0");    var revLoc = _rl[0]; var setRevLoc = _rl[1];
  var _c1 = React.useState("0");    var cred1 = _c1[0]; var setCred1 = _c1[1];
  var _c2 = React.useState("0");    var cred2 = _c2[0]; var setCred2 = _c2[1];
  var _ap = React.useState("20000");var apport = _ap[0]; var setApport = _ap[1];
  var _tx = React.useState("4.2");  var taux = _tx[0];   var setTaux = _tx[1];
  var _as = React.useState("0.30"); var assur = _as[0];  var setAssur = _as[1];
  var _du = React.useState("25");   var duree = _du[0];  var setDuree = _du[1];
  var _fn = React.useState("1500"); var fraisNot = _fn[0]; var setFraisNot = _fn[1];
  var _fc = React.useState("1000"); var fraisBanq = _fc[0]; var setFraisBanq = _fc[1];
  var _tauxLoc = React.useState("70"); var tauxLoc = _tauxLoc[0]; var setTauxLoc = _tauxLoc[1];
  var _mode = React.useState("locatif"); var mode = _mode[0]; var setMode = _mode[1];

  // ── Calculs ────────────────────────────────────────────────────────────────
  var R1 = pf(rev1), R2 = pf(rev2), RL = pf(revLoc);
  var C1 = pf(cred1), C2 = pf(cred2);
  var AP = pf(apport), TX = pf(taux), AS = pf(assur), DU = pf(duree);
  var FN = pf(fraisNot), FB = pf(fraisBanq), TL = pf(tauxLoc);

  // Revenus pris en compte selon mode
  // Locatif : banques retiennent 70% des revenus locatifs
  // Résidence principale : revenus locatifs non pris en compte
  var revLocRetenu = mode === "locatif" ? RL * (TL/100) : 0;
  var revTotalBrut = R1 + R2 + RL;
  var revTotalRetenu = R1 + R2 + revLocRetenu;

  // Charges existantes
  var chargesExist = C1 + C2;

  // Taux d'endettement max 35% (règle HCSF)
  var mensualiteMax = revTotalRetenu * 0.35 - chargesExist;
  var mensualiteMax30 = revTotalRetenu * 0.30 - chargesExist; // prudent

  // Calcul capacité d'emprunt depuis mensualité max
  var nMois = DU * 12;
  var tMens = TX / 100 / 12;
  var tAssurMens = AS / 100 / 12;

  var calcMontant = function(mensMax) {
    var mensCredit = mensMax - (0 * tAssurMens); // approx itérative
    if (tMens === 0) return mensMax * nMois;
    // M = mensualite_credit * (1 - (1+t)^-n) / t
    // montant * tAssurMens = mensualite_assur
    // mensMax = mensCredit + montant * tAssurMens
    // montant = mensCredit / (t / (1-(1+t)^-n)) = mensCredit * facteur
    var facteur = (1 - Math.pow(1+tMens, -nMois)) / tMens;
    // mensMax = montant/facteur + montant*tAssurMens
    // montant * (1/facteur + tAssurMens) = mensMax
    return mensMax / (1/facteur + tAssurMens);
  };

  var capaciteEmprunt35 = Math.max(0, calcMontant(mensualiteMax));
  var capaciteEmprunt30 = Math.max(0, calcMontant(mensualiteMax30));

  // Budget total d'achat
  var budgetTotal35 = capaciteEmprunt35 + AP;
  var budgetTotal30 = capaciteEmprunt30 + AP;

  // Prix du bien max (hors frais)
  var tauxFraisNotaire = 0.08; // 8% ancien
  var prixBienMax35 = budgetTotal35 / (1 + tauxFraisNotaire) - FB;
  var prixBienMax30 = budgetTotal30 / (1 + tauxFraisNotaire) - FB;

  // Mensualité réelle pour chaque scénario
  var calcMensualite = function(montant) {
    if (tMens === 0) return montant / nMois;
    var mens_credit = montant * tMens / (1 - Math.pow(1+tMens, -nMois));
    var mens_assur = montant * tAssurMens;
    return mens_credit + mens_assur;
  };

  var mens35 = calcMensualite(capaciteEmprunt35);
  var mens30 = calcMensualite(capaciteEmprunt30);

  var tauxEndetteReel35 = revTotalRetenu > 0 ? (chargesExist + mens35) / revTotalRetenu * 100 : 0;
  var tauxEndetteReel30 = revTotalRetenu > 0 ? (chargesExist + mens30) / revTotalRetenu * 100 : 0;

  // Coût total crédit
  var coutTotal35 = mens35 * nMois - capaciteEmprunt35;
  var coutTotal30 = mens30 * nMois - capaciteEmprunt30;
  ...
  var Row = function(props) {
    return (
      <div style={{ display:"flex", justifyContent:"space-between", alignItems:"center", padding:"8px 0", borderBottom:"1px solid #FFE8D9" }}>
        <span style={{ fontSize:13, color:"#78716C" }}>{props.label}</span>
        <span style={{ fontSize:props.big?17:14, fontWeight:props.big?800:600, color:props.color||"#1C1917" }}>{props.val}</span>
      </div>
    );
  };
  ...
  var KPI = function(props) {
    return (
      <div style={{ background:"#fff", border:"1.5px solid #FFE8D9", borderRadius:13, padding:"14px 16px" }}>
        <div style={{ display:"flex", alignItems:"center", gap:8, marginBottom:8 }}>
          <div style={{ width:30, height:30, borderRadius:8, background:"#FFF3EC", display:"flex", alignItems:"center", justifyContent:"center" }}>
            <Icon name={props.icon} size={16} color="#F97316" />
          </div>
          <span style={{ fontSize:11, color:"#A8A29E", textTransform:"uppercase", letterSpacing:"0.06em", fontWeight:600 }}>{props.label}</span>
        </div>
        <div style={{ fontSize:22, fontWeight:800, color:props.color||"#F97316", lineHeight:1 }}>{props.val}</div>
        {props.sub && <div style={{ fontSize:11, color:"#A8A29E", marginTop:4 }}>{props.sub}</div>}
      </div>
    );
  };
  ...
  return (
    <div style={{ display:"flex", flexDirection:"column", gap:16 }}>
      ...
      {/* Mode locatif / RP */}
      <div style={SECTION}>
        <SectionHeader icon="ti-calculator" title="Type de projet" />
        <div style={{ display:"flex", gap:8 }}>
          {[
            { id:"locatif", label:"Investissement locatif", desc:"Les loyers sont pris en compte à "+TL+"%" },
            { id:"rp",      label:"Résidence principale",   desc:"Revenus locatifs non comptabilisés" },
          ].map(function(m) {
            var isA = mode === m.id;
            return (
              <button key={m.id} onClick={function(){setMode(m.id);}}
                style={{ flex:1, background:isA?"#FFF3EC":"#FFF8F3", border:"1.5px solid "+(isA?"#F97316":"#FFE8D9"), borderRadius:12, padding:"12px 16px", cursor:"pointer", textAlign:"left" }}>
                <div style={{ fontSize:14, fontWeight:700, color:isA?"#F97316":"#1C1917" }}>{m.label}</div>
                <div style={{ fontSize:11, color:"#A8A29E", marginTop:3 }}>{m.desc}</div>
              </button>
            );
          })}
        </div>
      ...
      <div style={{ display:"grid", gridTemplateColumns:"1fr 1fr", gap:16 }}>
        ...
        {/* Revenus */}
        <div style={SECTION}>
          <SectionHeader icon="ti-trending-up" title="Revenus mensuels nets" />
          {[
            { label:"Emprunteur 1 (nets avant impôts)", val:rev1, set:setRev1 },
            { label:"Emprunteur 2 (co-emprunteur)", val:rev2, set:setRev2 },
            { label:"Revenus locatifs existants/mois", val:revLoc, set:setRevLoc },
          ].map(function(f) {
            return (
              <div key={f.label} style={{ marginBottom:10 }}>
                <div style={{ fontSize:12, color:"#A8A29E", marginBottom:4, fontWeight:600 }}>{f.label}</div>
                <div style={{ display:"flex", alignItems:"center", gap:6 }}>
                  <input type="number" value={f.val} min="0" onChange={function(e){f.set(e.target.value);}} style={inputS} />
          ...
          {mode === "locatif" && (
            <div style={{ marginTop:4 }}>
              <div style={{ fontSize:12, color:"#A8A29E", marginBottom:4, fontWeight:600 }}>Taux de prise en compte des loyers (%)</div>
              <div style={{ display:"flex", alignItems:"center", gap:8 }}>
                <input type="range" min="50" max="100" step="5" value={tauxLoc}
                  onChange={function(e){setTauxLoc(e.target.value);}}
                  style={{ flex:1, accentColor:"#F97316" }} />
                <span style={{ fontSize:14, fontWeight:700, color:"#F97316", minWidth:36 }}>{tauxLoc}%</span>
              </div>
              <div style={{ fontSize:11, color:"#A8A29E", marginTop:2 }}>Règle HCSF : 70% pour l'investissement locatif</div>
            </div>
          )}

          <div style={{ marginTop:12, background:"#FFF3EC", borderRadius:10, padding:"10px 12px" }}>
            <div style={{ fontSize:12, color:"#78716C" }}>Revenus retenus par la banque</div>
            <div style={{ fontSize:18, fontWeight:800, color:"#F97316" }}>{fmtEur(revTotalRetenu)}/mois</div>
            {revLocRetenu > 0 && <div style={{ fontSize:11, color:"#A8A29E", marginTop:2 }}>dont {fmtEur(revLocRetenu)} de loyers ({TL}%)</div>}
          </div>
        </div>

        {/* Charges et paramètres */}
        <div style={{ display:"flex", flexDirection:"column", gap:14 }}>
          <div style={SECTION}>
            <SectionHeader icon="ti-credit-card" title="Charges de crédit existantes" />
            {[
              { label:"Crédit en cours — Emprunteur 1 (€/mois)", val:cred1, set:setCred1 },
              { label:"Crédit en cours — Emprunteur 2 (€/mois)", val:cred2, set:setCred2 },
            ].map(function(f) {
              return (
                <div key={f.label} style={{ marginBottom:10 }}>
                  <div style={{ fontSize:12, color:"#A8A29E", marginBottom:4, fontWeight:600 }}>{f.label}</div>
            ...
            <Row label="Total charges mensuelles" val={fmtEur(chargesExist)+"/mois"} color="#dc2626" />
            <Row label="Mensualité max à 35% (HCSF)" val={fmtEur(Math.max(0,mensualiteMax))+"/mois"} color={mensualiteMax>0?"#16a34a":"#dc2626"} />
          </div>

          <div style={SECTION}>
            <SectionHeader icon="ti-building-bank" title="Paramètres du crédit" />
            {[
              { label:"Taux crédit (%)", val:taux, set:setTaux, step:"0.05" },
              { label:"Taux assurance (%)", val:assur, set:setAssur, step:"0.01" },
              { label:"Durée (ans)", val:duree, set:setDuree, step:"1", min:"5", max:"30" },
              { label:"Apport personnel (€)", val:apport, set:setApport, step:"1000" },
              { label:"Frais bancaires estimés (€)", val:fraisBanq, set:setFraisBanq, step:"500" },
            ].map(function(f) {
              return (
                <div key={f.label} style={{ marginBottom:10 }}>
                  <div style={{ fontSize:12, color:"#A8A29E", marginBottom:4, fontWeight:600 }}>{f.label}</div>
                  <input type="number" value={f.val} min={f.min||"0"} max={f.max} step={f.step||"1"}
                    onChange={function(e){f.set(e.target.value);}} style={inputS} />
                </div>
              );
            })}
          </div>
      ...
      {/* Résultats */}
      {revTotalRetenu > 0 && mensualiteMax > 0 && (
        <div style={{ display:"flex", flexDirection:"column", gap:14 }}>
          {/* KPIs */}
          <div style={{ display:"grid", gridTemplateColumns:"repeat(auto-fit, minmax(180px,1fr))", gap:10 }}>
            <KPI icon="ti-home" label="Capacité d'emprunt (35%)" val={fmtEur(capaciteEmprunt35)} sub={"Mensualité : "+fmtEur(mens35)+"/mois"} />
            <KPI icon="ti-coin" label="Budget total (35%)" val={fmtEur(budgetTotal35)} sub={"Apport "+fmtEur(AP)+" inclus"} color="#16a34a" />
            <KPI icon="ti-building-skyscraper" label="Prix bien max (35%)" val={fmtEur(prixBienMax35)} sub="Frais notaire 8% déduits" color="#1C1917" />
            <KPI icon="ti-scale" label="Taux endettement réel" val={fmt1(tauxEndetteReel35)+"%"} sub={tauxEndetteReel35<=35?"✓ Dans les clous HCSF":"⚠ Dépasse 35%"} color={tauxEndetteReel35<=35?"#16a34a":"#dc2626"} />
          </div>

          {/* Tableau comparatif 35% vs 30% */}
          <div style={SECTION}>
            <SectionHeader icon="ti-scale" title="Comparatif selon le taux d'endettement" />
            <div style={{ overflowX:"auto" }}>
              <table style={{ width:"100%", borderCollapse:"collapse", fontSize:13 }}>
                <thead>
                  <tr style={{ borderBottom:"2px solid #FFE8D9" }}>
                    <th style={{ textAlign:"left", padding:"8px 12px", fontSize:11, color:"#A8A29E", textTransform:"uppercase", letterSpacing:"0.06em" }}>Critère</th>
                    <th style={{ textAlign:"right", padding:"8px 12px", color:"#F97316", fontWeight:700 }}>35% — Max HCSF</th>
                    <th style={{ textAlign:"right", padding:"8px 12px", color:"#16a34a", fontWeight:700 }}>30% — Prudent</th>
                  </tr>
                </thead>
                <tbody>
                  {[
                    { label:"Mensualité max", v35:fmtEur(mensualiteMax)+"/mois", v30:fmtEur(mensualiteMax30)+"/mois" },
                    { label:"Capacité d'emprunt", v35:fmtEur(capaciteEmprunt35), v30:fmtEur(capaciteEmprunt30) },
                    { label:"Budget total (avec apport)", v35:fmtEur(budgetTotal35), v30:fmtEur(budgetTotal30) },
                    { label:"Prix bien max (frais inclus)", v35:fmtEur(prixBienMax35), v30:fmtEur(prixBienMax30), bold:true },
                    { label:"Mensualité réelle", v35:fmtEur(mens35)+"/mois", v30:fmtEur(mens30)+"/mois" },
                    { label:"Coût total du crédit", v35:fmtEur(coutTotal35), v30:fmtEur(coutTotal30) },
                    { label:"Taux d'endettement réel", v35:fmt1(tauxEndetteReel35)+"%", v30:fmt1(tauxEndetteReel30)+"%" },
                  ].map(function(r, i) {
                    return (
                      <tr key={i} style={{ borderBottom:"1px solid #FFE8D9", background:r.bold?"#FFF8F3":"transparent" }}>
                        <td style={{ padding:"9px 12px", color:"#78716C", fontWeight:r.bold?700:400 }}>{r.label}</td>
                        <td style={{ padding:"9px 12px", textAlign:"right", fontWeight:r.bold?800:600, color:"#F97316" }}>{r.v35}</td>
                        <td style={{ padding:"9px 12px", textAlign:"right", fontWeight:r.bold?800:600, color:"#16a34a" }}>{r.v30}</td>
                      </tr>
                    );
                  })}
                </tbody>
          ...
          {/* Conseils */}
          <div style={Object.assign({},SECTION,{background:"rgba(249,115,22,0.03)"})}>
            <SectionHeader icon="ti-rocket" title="Points clés pour la banque" />
            <div style={{ display:"flex", flexDirection:"column", gap:8 }}>
              {[
                { ok: chargesExist < revTotalRetenu * 0.15, msg: chargesExist < revTotalRetenu * 0.15 ? "Tes charges actuelles sont faibles — bon signal pour la banque." : "Tes charges actuelles sont élevées — elles réduisent ta capacité." },
                { ok: AP >= capaciteEmprunt35 * 0.10, msg: AP >= capaciteEmprunt35 * 0.10 ? "Ton apport représente "+fmt1(AP/capaciteEmprunt35*100)+"% du crédit — bien au-dessus des 10% recommandés." : "Ton apport est inférieur à 10% du crédit — pense à augmenter l'apport." },
                { ok: tauxEndetteReel35 <= 35, msg: tauxEndetteReel35 <= 35 ? "Taux d'endettement à "+fmt1(tauxEndetteReel35)+"% — dans les clous HCSF (max 35%)." : "Taux d'endettement à "+fmt1(tauxEndetteReel35)+"% — dépasse la règle HCSF de 35%." },
                { ok: mode === "locatif" && revLocRetenu > 0, msg: mode === "locatif" ? "En locatif, la banque retient "+TL+"% de tes loyers. Certaines banques vont jusqu'à 80%." : "En résidence principale, les loyers ne sont pas pris en compte par la banque." },
              ].map(function(c, i) {
                return (
                  <div key={i} style={{ display:"flex", alignItems:"flex-start", gap:10, padding:"8px 12px", background:c.ok?"#F0FDF4":"#FEF2F2", borderRadius:10, border:"1px solid "+(c.ok?"#BBF7D0":"#FECACA") }}>
                    <span style={{ fontSize:14, flexShrink:0 }}>{c.ok ? "✓" : "⚠"}</span>
                    <span style={{ fontSize:13, color:c.ok?"#16a34a":"#dc2626" }}>{c.msg}</span>
                  </div>
                );
              })}
            </div>
      ...
      {revTotalRetenu === 0 && (
        <div style={{ textAlign:"center", padding:"40px 20px", color:"#A8A29E" }}>
          <Icon name="ti-calculator" size={48} color="#FFE8D9" />
          <div style={{ fontSize:16, fontWeight:600, color:"#78716C", marginTop:12 }}>Renseigne tes revenus pour calculer ta capacité</div>
        </div>
      )}
    </div>
  );
...
function CarteCommunes() {
  var _sel = React.useState(null); var selected = _sel[0]; var setSelected = _sel[1];
  var _filtre = React.useState("score"); var filtre = _filtre[0]; var setFiltre = _filtre[1];
  var _search = React.useState(""); var search = _search[0]; var setSearch = _search[1];

  // Charger Leaflet dynamiquement
  var _leaflet = React.useState(false); var leafletReady = _leaflet[0]; var setLeafletReady = _leaflet[1];
  ...
  React.useEffect(function() {
    ...
    var map = L.map(mapRef.current, { center:[49.7, 1.0], zoom:9, zoomControl:true });
  ...
  React.useEffect(function() {
    if (!leafletReady || !mapInstance.current || communesData.length === 0) return;
    var L = window.L;
    if (!L) return;
    ...
    var filteredData = communesData.filter(function(c) {
      return !search || c.nom.toLowerCase().includes(search.toLowerCase());
    });

    filteredData.forEach(function(c) {
      var score = filtre === "score" ? c.score : filtre === "rendement" ? c.rendement : c.demo;
      var color = scoreColor(score);
      var size = Math.max(8, Math.min(24, 8 + score * 1.5));

      var icon = L.divIcon({
        className: "",
        html: '<div style="width:'+size+'px;height:'+size+'px;background:'+color+';border-radius:50%;border:2px solid #fff;box-shadow:0 2px 6px rgba(0,0,0,0.3);cursor:pointer;transition:transform 0.15s;" onmouseover="this.style.transform=\'scale(1.3)\'" onmouseout="this.style.transform=\'scale(1)\'"></div>',
        iconSize: [size, size],
      ...
      var marker = L.marker([c.lat, c.lon], { icon: icon })
        .addTo(map)
        .on("click", function() { setSelected(c); });

      markersRef.current.push(marker);
    });
  }, [leafletReady, communesData, filtre, search]);
  ...
  var topCommunes = [...communesData]
    .sort(function(a,b) {
      var sa = filtre==="score"?a.score:filtre==="rendement"?a.rendement:a.demo;
      var sb = filtre==="score"?b.score:filtre==="rendement"?b.rendement:b.demo;
      return sb-sa;
    })
    .slice(0,10);
  ...
  return (
    <div style={{ display:"flex", flexDirection:"column", gap:14 }}>
      ...
      {/* Filtres */}
      <div style={SECTION}>
        <SectionHeader icon="ti-map" title="Carte des communes — Seine-Maritime" />
        <div style={{ display:"flex", gap:10, alignItems:"center", flexWrap:"wrap" }}>
          <div style={{ display:"flex", gap:6 }}>
            {filtres.map(function(f) {
              var isA = filtre===f.id;
              return (
                <button key={f.id} onClick={function(){setFiltre(f.id);}}
                  style={{ padding:"6px 14px", borderRadius:8, border:"1.5px solid "+(isA?"#F97316":"#FFE8D9"), background:isA?"#FFF3EC":"#FFF8F3", color:isA?"#F97316":"#78716C", fontSize:13, fontWeight:isA?700:500, cursor:"pointer" }}>
                  {f.label}
                </button>
              );
            })}
          ...
          <input value={search} onChange={function(e){setSearch(e.target.value);}}
            ...
            style={{ flex:1, minWidth:180, background:"#FFF8F3", border:"1.5px solid #FFE8D9", borderRadius:10, padding:"7px 12px", fontSize:13, outline:"none", color:"#1C1917" }} />
          <div style={{ display:"flex", alignItems:"center", gap:8, fontSize:11, color:"#A8A29E" }}>
            <div style={{ display:"flex", gap:4, alignItems:"center" }}>
              {[["#16a34a","≥ 8"],["#F97316","≥ 6"],["#fbbf24","≥ 4"],["#dc2626","< 4"]].map(function(c) {
                return <span key={c[1]} style={{ display:"flex", alignItems:"center", gap:3 }}><span style={{ width:10, height:10, borderRadius:"50%", background:c[0], display:"inline-block" }}></span>{c[1]}</span>;
              })}
            </div>
          </div>
        </div>
      ...
      <div style={{ display:"grid", gridTemplateColumns:"1fr 320px", gap:14 }}>
        {/* Carte */}
        <div style={{ background:"#fff", border:"1.5px solid #FFE8D9", borderRadius:16, overflow:"hidden", height:500 }}>
          {!leafletReady && (
        ...
        <div style={{ display:"flex", flexDirection:"column", gap:10 }}>
          ...
          {selected && (
            <div style={{ background:"#fff", border:"1.5px solid #F97316", borderRadius:14, padding:14 }}>
              <div style={{ display:"flex", justifyContent:"space-between", alignItems:"flex-start", marginBottom:10 }}>
                <div>
                  <div style={{ fontSize:16, fontWeight:800, color:"#1C1917" }}>{selected.nom}</div>
                  <div style={{ fontSize:12, color:"#A8A29E" }}>{selected.pop ? selected.pop.toLocaleString("fr-FR")+" hab." : ""} · {selected.code}</div>
                </div>
                <div style={{ width:40, height:40, borderRadius:10, background:scoreColor(selected.score), display:"flex", alignItems:"center", justifyContent:"center", fontSize:16, fontWeight:800, color:"#fff" }}>
                  {selected.score.toFixed(1)}
                </div>
              </div>
              {[
                { label:"Rendement", val:selected.rendement, max:10 },
                { label:"Démographie", val:selected.demo, max:10 },
                { label:"Socio-éco", val:selected.socio, max:10 },
              ].map(function(r) {
                return (
                  <div key={r.label} style={{ marginBottom:8 }}>
                    <div style={{ display:"flex", justifyContent:"space-between", marginBottom:3 }}>
                      <span style={{ fontSize:12, color:"#78716C" }}>{r.label}</span>
                      <span style={{ fontSize:12, fontWeight:700, color:scoreColor(r.val) }}>{r.val.toFixed(1)}</span>
                    </div>
                    <div style={{ height:6, background:"#FFE8D9", borderRadius:999, overflow:"hidden" }}>
                      <div style={{ height:"100%", width:(r.val/r.max*100)+"%", background:scoreColor(r.val), borderRadius:999, transition:"width 0.4s" }} />
                    </div>
                  </div>
                );
              })}
              <button onClick={function(){}}
                style={{ width:"100%", marginTop:8, background:"#F97316", border:"none", borderRadius:10, padding:"8px", color:"#fff", fontSize:13, fontWeight:700, cursor:"pointer" }}>
                Voir le détail commune
              </button>
            </div>
          )}

          {/* Top 10 */}
          <div style={{ background:"#fff", border:"1.5px solid #FFE8D9", borderRadius:14, padding:14, flex:1, overflow:"auto" }}>
            <div style={{ fontSize:12, fontWeight:700, color:"#A8A29E", textTransform:"uppercase", letterSpacing:"0.06em", marginBottom:10 }}>
              Top 10 — {filtres.find(function(f){return f.id===filtre;})?.label}
            </div>
            {topCommunes.map(function(c, i) {
              var score = filtre==="score"?c.score:filtre==="rendement"?c.rendement:c.demo;
              var isSelected = selected && selected.code === c.code;
              return (
                <div key={c.code} onClick={function(){ setSelected(c); if(mapInstance.current) mapInstance.current.setView([c.lat,c.lon],13); }}
                  style={{ display:"flex", alignItems:"center", gap:8, padding:"7px 0", borderBottom:"1px solid #FFE8D9", cursor:"pointer", background:isSelected?"#FFF3EC":"transparent", borderRadius:6, paddingLeft:isSelected?8:0, transition:"all 0.15s" }}>
                  <span style={{ fontSize:11, fontWeight:800, color:"#D6D3D1", width:20 }}>{i+1}</span>
                  <div style={{ width:8, height:8, borderRadius:"50%", background:scoreColor(score), flexShrink:0 }} />
                  <span style={{ flex:1, fontSize:13, fontWeight:600, color:"#1C1917" }}>{c.nom}</span>
            ...
            {topCommunes.length === 0 && (
              <div style={{ textAlign:"center", padding:"20px", color:"#A8A29E", fontSize:13 }}>
                Charge d'abord les données dans Analyse communes
              </div>
            )}
          </div>
        </div>
