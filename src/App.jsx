...
function calculerSimulation(i) {
  ...
  const mkR = (impot) => {
    ...
    return { ebe: loyersAnnuels - totalFraisAnnuels, impot, tresorerie, rendBrut, rendNet, tri, regle70 };
  };
...
function CashFlowChart({ data }) {
  ...
  const allVals = data.flatMap(function(d) { return [d.loyers, -(d.frais + d.credit + d.impots), d.cashflow]; });
  const maxV = Math.max.apply(null, allVals.concat([0])) * 1.15;
  const minV = Math.min.apply(null, allVals.concat([0])) * 1.15;
    ...
    <div style={{ overflowX: "auto" }}>
      ...
      >
        ...
        {data.map(function(d, i) {
          ...
          return (
            <g key={i} opacity={opacity} onMouseEnter={function() { setHovered(i); }}>
        ...
        <polyline
          points={data.map(function(d, i) { return barCenterX(i) + "," + toY(d.cashflow); }).join(" ")}
          fill="none"
          stroke="#38bdf8"
          strokeWidth={2.5}
        ...
        {data.map(function(d, i) {
          if (i === 0 || (i + 1) % 5 === 0) {
            return (
              <text key={i} x={barCenterX(i)} y={H - padB + 16} textAnchor="middle" fontSize={10} fill="#64748b">
                {d.year}
              </text>
            );
          }
        ...
        {h != null && (
          <g>
            <rect x={ttX} y={padT + 4} width={TOOLTIP_W} height={132} rx={10} fill="rgba(255,255,255,0.96)" stroke="rgba(148,163,184,0.5)" strokeWidth={1} />
  ...
  return (
    <div style={{ marginBottom: 10 }}>
      ...
      <div style={{ display: "flex", alignItems: "center", gap: 6 }}>
        <input
          type="number" name={name} value={value} step={s} min={m} onChange={onChange}
          style={{ width: "100%", background: "rgba(255,255,255,0.75)", border: "1px solid rgba(148,163,184,0.5)", borderRadius: 999, padding: "7px 12px", color: "#0f172a", fontSize: 13, backdropFilter: "blur(10px)", outline: "none" }}
        />
    ...
    <div style={{ display: "flex", flexDirection: "column", gap: 16 }}>
            ...
            {projets.map(function(p) {
              return (
                <div key={p.id} style={{ display: "flex", alignItems: "center", gap: 6, background: "rgba(99,102,241,0.1)", border: "1px solid rgba(99,102,241,0.3)", borderRadius: 999, padding: "5px 10px" }}>
                  <button onClick={function() { charger(p); }} style={{ background: "none", border: "none", fontSize: 12, fontWeight: 600, color: "#4338ca", cursor: "pointer" }}>
                    📂 {p.nom}
                  </button>
        ...
        <div style={{ display: "flex", justifyContent: "space-between", alignItems: "center", marginBottom: 12, flexWrap: "wrap", gap: 8 }}>
          ...
          <div style={{ display: "flex", gap: 8, alignItems: "center" }}>
            <input
              ...
              style={{ background: "rgba(255,255,255,0.8)", border: "1px solid rgba(148,163,184,0.5)", borderRadius: 999, padding: "6px 12px", fontSize: 13, color: "#0f172a", width: 180, outline: "none" }}
            />
            <button
              onClick={sauvegarder}
      ...
      <div style={{ display: "flex", flexWrap: "wrap", gap: 8 }}>
        {Object.keys(result.regimes).map(function(r) {
          return (
            <button key={r} onClick={function() { setRegimeActif(r); }}
        ...
        <table style={{ width: "100%", borderCollapse: "collapse", fontSize: 12 }}>
          <thead>
            <tr style={{ color: "#64748b", borderBottom: "1px solid rgba(148,163,184,0.35)" }}>
              {["Régime", "Tréso/an", "Impôt/an", "Rdt net", "TRI", "Règle 70%"].map(function(h) {
          ...
          <tbody>
            {Object.entries(result.regimes).map(function(entry) {
              ...
              return (
                <tr key={nom} onClick={function() { setRegimeActif(nom); }}
...
function AnalyseCommunes() {
  ...
  const loadCommunes = useCallback(function(force) {
    if (!force) {
      ...
      if (cached) {
        try { setCommunes(JSON.parse(cached)); setLoading(false); return; } catch(e) {}
      }
    }
  ...
  const filtered = useMemo(function() {
    return communes
      ...
      .sort(function(a, b) {
        return (sn(b.scores && b.scores[sortKey]) || -1) - (sn(a.scores && a.scores[sortKey]) || -1);
      });
  ...
  const toggleCompare = function(c) {
    setCompareList(function(prev) {
      if (prev.find(function(x) { return x.nom === c.nom; })) {
        return prev.filter(function(x) { return x.nom !== c.nom; });
      }
    ...
    <div style={{ display: "flex", flexDirection: "column", gap: 16 }}>
      ...
      <div style={{ display: "flex", justifyContent: "center" }}>
        <div style={{ maxWidth: 820, flex: 1, display: "flex", alignItems: "center", gap: 10, padding: "8px 12px", borderRadius: 999, background: "rgba(255,255,255,0.75)", boxShadow: "0 8px 28px rgba(99,102,241,0.12), 0 0 0 1px rgba(148,163,184,0.3)", backdropFilter: "blur(18px)" }}>
          ...
          />
          <select value={sortKey} onChange={function(e) { setSortKey(e.target.value); }}
            style={{ background: "rgba(241,245,249,0.85)", border: "1px solid rgba(148,163,184,0.4)", borderRadius: 999, padding: "6px 10px", fontSize: 12, color: "#334155", outline: "none" }}>
            {scoreKeys.map(function(k) { return <option key={k.key} value={k.key}>{k.label}</option>; })}
          </select>
          <select value={filterMin} onChange={function(e) { setFilterMin(Number(e.target.value)); }}
            style={{ background: "rgba(241,245,249,0.85)", border: "1px solid rgba(148,163,184,0.4)", borderRadius: 999, padding: "6px 10px", fontSize: 12, color: "#334155", outline: "none" }}>
      ...
      <div style={{ display: "flex", justifyContent: "space-between", alignItems: "center", flexWrap: "wrap", gap: 8 }}>
        ...
        <div style={{ display: "flex", gap: 8 }}>
          <button onClick={function() { loadCommunes(true); }}
          ...
          {compareList.length > 0 && (
            <button onClick={function() { setShowCompare(true); }}
          ...
          {!loading && !error && (
            <div style={{ display: "grid", gridTemplateColumns: "repeat(auto-fill, minmax(230px, 1fr))", gap: 12 }}>
              {top10.map(function(c) {
                ...
                return (
                  <div key={c.nom}
                    onClick={function() { setSelected(isSelected ? null : c); }}
                    onContextMenu={function(e) { e.preventDefault(); toggleCompare(c); }}
                    style={{
            ...
            {detail && !detail.error && (
              <div>
                <div style={{ display: "flex", justifyContent: "space-between", marginBottom: 12, alignItems: "center" }}>
                  ...
                  <button onClick={function() { setSelected(null); setDetail(null); setOpenScore(null); }}
                  ...
                  {[
                    ...
                    return (
                      <div key={item.key} style={{ marginBottom: isOpen ? 4 : 6 }}>
                        <div onClick={function() { if (clickable) setOpenScore(isOpen ? null : item.key); }}
                ...
                <div style={{ fontSize: 11, color: "#94a3b8", marginTop: 4 }}>
                  Sources : {detail.prix && detail.prix.source} · {detail.loyer && detail.loyer.source}
                </div>
              </div>
            )}
          </div>
      ...
      {showCompare && compareList.length > 0 && (
        <div style={{ position: "fixed", inset: 0, background: "rgba(15,23,42,0.4)", zIndex: 50, display: "flex", alignItems: "center", justifyContent: "center" }}
          onClick={function() { setShowCompare(false); }}>
          <div style={{ background: "rgba(255,255,255,0.92)", borderRadius: 20, padding: 20, minWidth: 520, maxWidth: 840, maxHeight: "80vh", overflowY: "auto", boxShadow: "0 20px 50px rgba(15,23,42,0.2)", backdropFilter: "blur(24px)" }}
            ...
            <table style={{ width: "100%", borderCollapse: "collapse", fontSize: 12 }}>
              ...
              <tbody>
                {[
                  { label: "Score global", fn: function(c) { return sn(c.scores && c.scores.global) != null ? sn(c.scores.global).toFixed(1) : "—"; } },
                  { label: "Rendement", fn: function(c) { return sn(c.scores && c.scores.rendement) != null ? sn(c.scores.rendement).toFixed(1) : "—"; } },
                  { label: "Démographie", fn: function(c) { return sn(c.scores && c.scores.demographie) != null ? sn(c.scores.demographie).toFixed(1) : "—"; } },
                  { label: "Socio-éco", fn: function(c) { return sn(c.scores && c.scores.socio_eco) != null ? sn(c.scores.socio_eco).toFixed(1) : "—"; } },
        ...
        {navItems.map(function(n) {
          return (
            <button key={n.id} onClick={function() { setOnglet(n.id); }}
              style={{ display: "flex", alignItems: "center", gap: 10, padding: "8px 10px", borderRadius: 999, border: "none", cursor: "pointer", fontSize: 13, fontWeight: 500, textAlign: "left", width: "100%", background: onglet === n.id ? "linear-gradient(135deg,rgba(99,102,241,0.15),rgba(56,189,248,0.15))" : "transparent", color: onglet === n.id ? "#4338ca" : "#475569", boxShadow: onglet === n.id ? "0 4px 14px rgba(99,102,241,0.15)" : "none", borderLeft: onglet === n.id ? "3px solid #6366f1" : "3px solid transparent" }}>
              <span>{n.icon}</span>
              <span>{n.label}</span>
            </button>
          );
