...
function SimulationProjet({ photos, setPhotos, projets, setProjets, projetACharger, onProjetCharge }) {
  ...
  const communeInputRef = React.useRef(null);
  ...
  useEffect(function() {
    ...
    setInputs(projetACharger.inputs);
    setRegimeActif(projetACharger.regimeActif);
  ...
  const exportPDF = async function() {
    ...
    var W = 210; var margin = 14; var y = 0; var lineH = 6;

    // Couleurs
    var C_PURPLE = [99, 102, 241]; var C_BLUE = [56, 189, 248];
    var C_GREEN = [22, 163, 74]; var C_RED = [220, 38, 38];
    var C_DARK = [15, 23, 42]; var C_GRAY = [100, 116, 139];
    var C_LIGHT = [241, 245, 249];
    ...
    doc.text("Radar Immo 76", margin, 11);
    doc.setFontSize(10); doc.setFont("helvetica", "normal");
    doc.text("Dossier de présentation — Simulation d'investissement", margin, 18);
    ...
    doc.text(nomProjet || "Simulation sans nom", margin, y); y += 8;
    doc.setFontSize(9); doc.setFont("helvetica", "normal"); doc.setTextColor(...C_GRAY);
    doc.text("Régime fiscal : " + regimeActif + "   |   DPE : " + (inputs.noteDPE || "NC"), margin, y); y += 10;

    // ── Fonction bloc section ──────────────────────────────────────────────────
    function sectionTitle(title) {
      doc.setFillColor(...C_LIGHT);
      doc.rect(margin, y - 1, W - margin * 2, 7, "F");
      doc.setTextColor(...C_PURPLE); doc.setFontSize(9); doc.setFont("helvetica", "bold");
      doc.text(title.toUpperCase(), margin + 2, y + 4); y += 10;
      doc.setTextColor(...C_DARK);
    }

    function row2(label, value, colorVal) {
      doc.setFontSize(9); doc.setFont("helvetica", "normal"); doc.setTextColor(...C_GRAY);
      doc.text(label, margin + 2, y);
      doc.setFont("helvetica", "bold");
      if (colorVal) doc.setTextColor(...colorVal); else doc.setTextColor(...C_DARK);
    ...
    var r = result; var reg = r.regimes[regimeActif];

    // ── SECTION 1 : Acquisition ────────────────────────────────────────────────
    sectionTitle("1. Acquisition & Financement");
    row2("Prix de vente", fmtEur(pf(inputs.prixVente)));
    row2("Frais de notaire", fmtEur(pf(inputs.fraisNotaire)));
    row2("Travaux + Aménagements", fmtEur(pf(inputs.travaux) + pf(inputs.amenagements)));
    row2("Frais d'agence", fmtEur(pf(inputs.prixVente) * pf(inputs.fraisAgencePct) / 100));
    separator();
    row2("COÛT TOTAL", fmtEur(r.depenseNette), C_PURPLE);
    row2("Apport personnel", fmtEur(pf(inputs.apport)));
    row2("Montant emprunté", fmtEur(r.sommeEmpruntee), C_BLUE);
    row2("Mensualité crédit", fmtEur(r.mensualite), C_BLUE);
    row2("Durée / Taux", pf(inputs.dureeAnnees) + " ans à " + pf(inputs.tauxCredit) + " %");
    row2("Coût total du crédit", fmtEur(r.coutPretTotal));
    y += 4;

    // ── SECTION 2 : Revenus & Charges ─────────────────────────────────────────
    sectionTitle("2. Revenus & Charges annuels");
    row2("Loyer mensuel HC", fmtEur(pf(inputs.loyerMensuelHC)));
    row2("Taux d'occupation", pf(inputs.tauxOccupation) + " mois/an");
    row2("Loyers annuels", fmtEur(r.loyersAnnuels), C_GREEN);
    separator();
    row2("Charges immeuble", fmtEur(pf(inputs.chargesImmeubleAn)));
    row2("Taxe foncière", fmtEur(pf(inputs.taxeFonciereAn)));
    row2("Assurance PNO", fmtEur(pf(inputs.assurancePNOAn) || r.depenseNette * 0.0012));
    row2("Gestion locative", fmtEur(r.loyersAnnuels * pf(inputs.gestionLocativePct) / 100));
    row2("Total charges annuelles", fmtEur(r.totalFraisAnnuels), C_RED);
    y += 4;

    // ── SECTION 3 : Résultats ──────────────────────────────────────────────────
    sectionTitle("3. Résultats — Régime " + regimeActif);
    row2("Rendement brut", fmtPct(r.rendBrut), C_GREEN);
    row2("Rendement net", fmtPct(reg.rendNet), C_GREEN);
    row2("Fiscalité annuelle (" + regimeActif + ")", fmtEur(reg.impot), C_RED);
    separator();
    var tresoPMois = reg.tresorerie / 12;
    row2("CASH-FLOW NET / mois", fmt(tresoPMois, 0) + " €", tresoPMois >= 0 ? C_GREEN : C_RED);
    row2("Règle des 70 %", reg.regle70 != null ? fmt(reg.regle70, 2) : "—", reg.regle70 != null && reg.regle70 <= 0.7 ? C_GREEN : C_RED);
    row2("TRI estimé", reg.tri != null ? fmt(reg.tri, 1) + " ans" : "—");
    row2("Amortissement annuel", fmtEur(r.amortissement));
    y += 4;

    // ── SECTION 4 : Comparatif régimes (tableau) ──────────────────────────────
    // Nouvelle page si besoin
    if (y > 220) { doc.addPage(); y = 20; }
    sectionTitle("4. Comparatif des régimes fiscaux");

    var headers = ["Régime", "Tréso/an", "CF/mois", "Impôt/an", "Rdt net", "Note"];
    var colW = [48, 28, 22, 28, 22, 16];
    var xCols = [margin];
    for (var ci = 0; ci < colW.length - 1; ci++) xCols.push(xCols[ci] + colW[ci]);

    // En-tête tableau
    doc.setFillColor(...C_PURPLE);
    doc.rect(margin, y - 1, W - margin * 2, 7, "F");
    doc.setTextColor(255, 255, 255); doc.setFontSize(8); doc.setFont("helvetica", "bold");
    headers.forEach(function(h, i) { doc.text(h, xCols[i] + 1, y + 4); });
    y += 9;

    // Lignes tableau
    var regimes = Object.entries(r.regimes);
    regimes.forEach(function(entry, ri) {
      var nom = entry[0]; var reg2 = entry[1];
      var note = calculerNote(r, nom);
      var isActive = nom === regimeActif;
      if (isActive) { doc.setFillColor(245, 243, 255); doc.rect(margin, y - 1, W - margin * 2, 6.5, "F"); }
      doc.setTextColor(...C_DARK); doc.setFontSize(8);
      doc.setFont("helvetica", isActive ? "bold" : "normal");
      doc.text(nom, xCols[0] + 1, y + 4);
      doc.setFont("helvetica", "normal");
      var cf = reg2.tresorerie / 12;
      doc.setTextColor(reg2.tresorerie >= 0 ? 22 : 220, reg2.tresorerie >= 0 ? 163 : 38, reg2.tresorerie >= 0 ? 74 : 38);
      doc.text(fmtEur(reg2.tresorerie), xCols[1] + 1, y + 4);
      doc.text((cf >= 0 ? "+" : "") + fmt(cf, 0) + " €", xCols[2] + 1, y + 4);
      doc.setTextColor(...C_RED);
      doc.text(fmtEur(reg2.impot), xCols[3] + 1, y + 4);
      doc.setTextColor(...C_GREEN);
      doc.text(fmtPct(reg2.rendNet), xCols[4] + 1, y + 4);
      doc.setTextColor(...getNoteColor(note) === "#16a34a" ? C_GREEN : getNoteColor(note) === "#d97706" ? [217, 119, 6] : C_RED);
      doc.text(String(note) + "/100", xCols[5] + 1, y + 4);
      doc.setTextColor(...C_DARK);
    ...
    for (var pi = 1; pi <= totalPages; pi++) {
      ...
      doc.setTextColor(...C_GRAY); doc.setFontSize(7); doc.setFont("helvetica", "normal");
      doc.text("Radar Immo 76 — Document généré le " + dateStr + " — Simulation non contractuelle", margin, 293);
      doc.text("Page " + pi + "/" + totalPages, W - margin, 293, { align: "right" });
    }

    doc.save("simulation-" + (nomProjet || "projet") + ".pdf");
  };
            ...
            <div style={{ position: "relative" }}>
              <input value={communeSearch}
                ...
                onChange={function(e) {
                  ...
                  if (!e.target.value) { setInputs(function(p) { return Object.assign({}, p, { commune: "" }); }); setDonneesCommune(null); setCommuneSuggestions([]); }
                  if (communeInputRef.current) { setDropdownRect(communeInputRef.current.getBoundingClientRect()); }
                }}
                onFocus={function() { if (communeInputRef.current) setDropdownRect(communeInputRef.current.getBoundingClientRect()); }}
              ...
              {communeSuggestions.length > 0 && dropdownRect && (
                <div style={{ position: "fixed", zIndex: 9999, background: "#fff", border: "1px solid rgba(148,163,184,0.3)", borderRadius: 10, boxShadow: "0 8px 24px rgba(0,0,0,0.18)", overflow: "hidden", top: dropdownRect.bottom + 4, left: dropdownRect.left, width: dropdownRect.width }}>
                  {communeSuggestions.map(function(c) {
                    return <div key={c.nom}
                      onClick={function() { setCommuneSearch(c.nom); setCommuneSuggestions([]); setDropdownRect(null); setInputs(function(p) { return Object.assign({}, p, { commune: c.nom }); }); fetchDonneesCommune(c.nom); }}
