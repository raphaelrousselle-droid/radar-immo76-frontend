...
function SimulationProjet({ photos, setPhotos, projets, setProjets, projetACharger, onProjetCharge }) {
  ...
  const exportPDF = async function() {
    ...
    // Couleurs directes (pas de BG pour éviter conflits)
    const BG_RGB = [248, 250, 252];
    ...
    var cleanTitle = function(title) { return (title || "Projet").replace(/[\\W_]/g, " ").trim().substring(0, 25); };
    ...
    function hdr(title, subtitle) {
      ...
      doc.text(cleanTitle(title), W / 2, 14, { align: "center" });
      if (subtitle) { 
    ...
    function kpiRow(items, y) {
      ...
      items.forEach(function(item, i) {
        var x = 14 + i * colW;
        doc.setFillColor(248, 250, 252);  // ← DIRECT
    ...
    function pieChart(cx, cy, radius, data) {
      ...
      data.forEach(function(d) {
        ...
        doc.circle(cx + Math.cos(midAngle) * (radius * 0.7), cy + Math.sin(midAngle) * (radius * 0.7), radius * 0.25, "F");
        doc.setLineWidth(2);
        doc.setDrawColor(d.color[0], d.color[1], d.color[2]);
        doc.arc(cx, cy, radius, angle * 180 / Math.PI, (angle + slice) * 180 / Math.PI);
        angle += slice;
      });
    }
    function lineChart(cx, cy, w, h, data) {
      var maxY = Math.max(...data.map(function(d) { return d.cashflow; })) * 1.1;
      var minY = Math.min(...data.map(function(d) { return d.cashflow; })) * 1.1 || 0;
      var rangeY = maxY - minY;
      var pad = 20;
      var scaleX = (w - pad * 2) / Math.max(data.length - 1, 1);
      ...
      for (var gy = 0; gy <= 4; gy++) {
        doc.line(cx + pad, cy + pad + gy * (h - pad * 2) / 4, cx + w - pad, cy + pad + gy * (h - pad * 2) / 4);
      }
      ...
      for (var p = 0; p < data.length - 1; p++) {
        var x1 = cx + pad + p * scaleX;
        var y1 = cy + h - pad - (data[p].cashflow - minY) * scaleY;
        var x2 = cx + pad + (p + 1) * scaleX;
        var y2 = cy + h - pad - (data[p + 1].cashflow - minY) * scaleY;
      ...
      doc.line(cx + pad, cy + pad, cx + pad, cy + h - pad);
      doc.line(cx + pad, cy + h - pad, cx + w - pad, cy + h - pad);
    }
    ...
    if (photos && photos[0] && photos[0].url) {
      try { doc.addImage(photos[0].url, "JPEG", 14, photoY, 80, 60, "", "MEDIUM"); photoY += 65; }
      catch(e) { photoY += 2; }
    }
    ...
    doc.text(cleanTitle(nomProjet), W / 2, photoY + 7, { align: "center" });
    ...
    if (lots && lots.length > 0) {
      ...
      doc.text("Lot", 18, photoY); doc.text("Surface", 80, photoY); doc.text("Loyer/mois", 130, photoY); doc.text("Travaux", 160, photoY);
      photoY += 6; doc.setDrawColor(220, 220, 220); doc.line(16, photoY - 1, W - 16, photoY - 1); photoY += 4;
      lots.slice(0, 6).forEach(function(l) {
        doc.setFontSize(8.5); doc.setFont("helvetica", "normal"); doc.setTextColor(30, 41, 59);
        doc.text((l.nom || "Lot").substring(0, 15), 18, photoY);
        doc.text(l.surface ? fmt(pf(l.surface), 0) + " m²" : "—", 80, photoY, { align: "center" });
        doc.text(l.loyer ? fmt(pf(l.loyer), 0) + " €" : "—", 130, photoY, { align: "center" });
        doc.text(l.travaux ? fmt(pf(l.travaux), 0) + " €" : "—", 160, photoY, { align: "right" });
        photoY += 8;
      });
      photoY += 2;
    }
    ...
    doc.addPage();
    hdr("Analyse financière du projet", cleanTitle(nomProjet));
    var y = 28;
    y = kpiRow([
      { label: "Loyer mensuel HC", value: fmt(pf(inputs.loyerMensuelHC), 0) + " €", color: VERT },
    ...
    pieChart(45, y + 22, 18, [
      ...
      { label: "Travaux", value: pf(inputs.provisionTravauxAn), color: [249, 115, 22] },
      { label: "Comptable", value: pf(inputs.expertComptableAn), color: [236, 72, 153] },
    ...
    if (donneesCommune && donneesCommune.scores) {
      ...
      y = kpiRow([
        ...
        { label: "Prix moyen appart.", value: pri.appartement_m2 ? fmt(Number(pri.appartement_m2), 0) + " €/m²" : "—", color: BLEU },
    ...
    doc.addPage();
    hdr("Analyse patrimoniale", cleanTitle(nomProjet));
    y = 28;
    y = kpiRow([
    ...
    y += 4;
    y = section("📈 Projection cash-flow annuel", y);
    lineChart(45, y + 10, 120, 50, cashFlowData.slice(0, 10));
    doc.setFontSize(8); doc.setTextColor(100, 116, 139);
    doc.text("Années", 105, y + 65, { align: "center" });
    doc.text("Cash-flow €/an", 20, y + 25, { align: "center", angle: 90 });
    y += 75;
    footer(4);

    doc.save("dossier-" + cleanTitle(nomProjet).replace(/\\s+/g, "-") + ".pdf");
    } catch(e) { 
