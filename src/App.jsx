...
function CalculateurPlusValue() {
  ...
  const calc = useMemo(function() {
    ...
    if (pvBrute <= 0) return { pvBrute, prixRevient, pvNette: 0, impotIR: 0, impotPS: 0, impotTotal: 0, abattIR: 0, abattPS: 0, exonere: false, ans, surtaxe: 0, baseIR: 0, basePS: 0, creditRestant, creditRembourse, produitNetAvantImpot, moisEcoules, mensualite };
    if (typeBien === "resid-principale") return { pvBrute, prixRevient, pvNette: produitNetAvantImpot, impotIR: 0, impotPS: 0, impotTotal: 0, abattIR: 100, abattPS: 100, exonere: true, ans, surtaxe: 0, baseIR: 0, basePS: 0, creditRestant, creditRembourse, produitNetAvantImpot, moisEcoules, mensualite };
    ...
    const pvNette = produitNetAvantImpot - impotTotal;
    const exonere = abattIR >= 100 && abattPS >= 100;
    return { pvBrute, prixRevient, pvNette, impotIR, impotPS, impotTotal, abattIR, abattPS, exonere, ans, surtaxe, baseIR, basePS, creditRestant, creditRembourse, produitNetAvantImpot, moisEcoules, mensualite };
  }, [vals, duree, typeBien]);
  ...
  return (
    <div style={{ display: "flex", flexDirection: "column", gap: 14 }}>
        ...
        <div style={SECTION}>
          ...
          <div style={{ display: "flex", flexDirection: "column", gap: 10 }}>
            ...
            {[
              ...
              { field: "fraisAchat",   label: "Frais de notaire + agence", unit: "€", step: "500"  },
        ...
        <div style={{ display: "flex", flexDirection: "column", gap: 14 }}>
          ...
          {pf(vals.montantCredit) > 0 && (
            <div style={SECTION}>
              ...
              <StatRow label="Capital restant dû (à rembourser)" value={"– " + fmtEur(calc.creditRestant)} color="#dc2626" bold border={false} />
              <div style={{ marginTop: 10, background: "rgba(254,243,199,0.7)", borderRadius: 10, padding: "8px 12px", fontSize: 11, color: "#92400e", border: "1px solid rgba(251,191,36,0.3)" }}>
                ⚠ Les indemnités de remboursement anticipé (IRA) ne sont pas incluses (max 3% du capital restant ou 6 mois d'intérêts).
              </div>
            </div>
          )}

          {/* Abattements */}
          {typeBien !== "resid-principale" && calc.pvBrute > 0 && (
            <div style={SECTION}>
          ...
          {/* Fiscalité + bilan final */}
          {typeBien !== "resid-principale" && calc.pvBrute > 0 && (
            <div style={SECTION}>
              <SectionHeader icon="🧾" title="Fiscalité & bilan net" />
              {calc.exonere ? (
                ...
                <>
                  <StatRow label={"Base imposable IR (abatt. " + calc.abattIR + "%)"} value={fmtEur(calc.baseIR)} />
                  <StatRow label="Impôt IR à 19%" value={"– " + fmtEur(calc.impotIR - calc.surtaxe)} color="#dc2626" />
                  ...
                  <div style={{ margin: "10px 0", height: 1, background: "rgba(148,163,184,0.2)" }} />
                  <StatRow label="Impôt total sur PV" value={"– " + fmtEur(calc.impotTotal)} color="#dc2626" bold border={false} />
                </>
              )}
              {/* Bilan final */}
              <div style={{ marginTop: 12, background: "linear-gradient(135deg,rgba(99,102,241,0.07),rgba(56,189,248,0.07))", borderRadius: 14, padding: "14px", border: "1.5px solid rgba(99,102,241,0.2)" }}>
                <div style={{ fontSize: 12, fontWeight: 700, color: "#4338ca", marginBottom: 10 }}>💰 Bilan net encaissé</div>
                ...
                {!calc.exonere && <StatRow label="– Impôt plus-value" value={"– " + fmtEur(calc.impotTotal)} color="#dc2626" />}
                <div style={{ marginTop: 8, background: calc.pvNette > 0 ? "rgba(220,252,231,0.8)" : "rgba(254,226,226,0.8)", borderRadius: 10, padding: "10px 12px", border: "1px solid " + (calc.pvNette > 0 ? "rgba(22,163,74,0.3)" : "rgba(220,38,38,0.3)") }}>
                  <div style={{ fontSize: 11, color: "#64748b" }}>= Net encaissé après crédit + impôt</div>
                  <div style={{ fontSize: 26, fontWeight: 900, color: calc.pvNette > 0 ? "#15803d" : "#dc2626" }}>
          ...
          {/* Cas résidence principale : juste le bilan crédit */}
          {typeBien === "resid-principale" && calc.pvBrute > 0 && (
            <div style={SECTION}>
              <SectionHeader icon="💰" title="Bilan net encaissé" />
              <StatRow label="Prix de vente net" value={fmtEur(pf(vals.prixVente) - pf(vals.fraisVente))} color="#16a34a" />
              {pf(vals.montantCredit) > 0 && <StatRow label="– Capital restant dû" value={"– " + fmtEur(calc.creditRestant)} color="#dc2626" />}
              <StatRow label="– Impôt plus-value" value="0 € (exonéré)" color="#16a34a" />
              <div style={{ marginTop: 10, background: "rgba(220,252,231,0.8)", borderRadius: 12, padding: "12px 14px", border: "1px solid rgba(22,163,74,0.3)" }}>
                <div style={{ fontSize: 12, color: "#64748b" }}>Net encaissé</div>
                <div style={{ fontSize: 26, fontWeight: 900, color: "#15803d" }}>+ {fmtEur(calc.pvNette)}</div>
              </div>
            </div>
          )}
      ...
      <div style={{ background: "rgba(241,245,249,0.8)", borderRadius: 12, padding: "10px 14px", fontSize: 11, color: "#64748b", border: "1px solid rgba(148,163,184,0.2)" }}>
        ℹ️ IR 19% + PS 17.2% — Abattements à partir de 6 ans — Exo IR à 22 ans, totale à 30 ans — Surtaxe si PV nette IR &gt; 50 000 € — IRA crédit non incluses.
      </div>
    </div>
  );
}
