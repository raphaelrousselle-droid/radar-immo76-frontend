...
function SimulateurExploitation() {
  ...
  var _view = React.useState("lots"); var view = _view[0]; var setView = _view[1]; // "lots" | "compare"

  var handleChange = function(e) { setInputs(function(p) { return Object.assign({}, p, { [e.target.name]: e.target.value }); }); };
  ...
  function calcLot(lot) {
    ...
    var loyerMois = lot.mode === "coloc" ? Math.round(ch * loyerNu * 0.55) : lot.mode === "lcd" ? Math.round(s * 0.8 * 22) : Math.round(loyerNu * m.major);
  ...
  function calcAllModesForLots(modeId) {
    ...
    lots.forEach(function(lot) {
      var s = pf(lot.surface); var ch = Math.max(1, Math.round(pf(lot.nbChambres)));
      var loyerNu = pf(lot.loyerNuMarche);
      var loyerMois = modeId === "coloc" ? Math.round(ch * loyerNu * 0.55) : modeId === "lcd" ? Math.round(s * 0.8 * 22) : Math.round(loyerNu * m.major);
      var loyerAn = loyerMois * m.occup;
              ...
              <table style={{ width: "100%", borderCollapse: "collapse", fontSize: 11 }}>
                ...
                <tbody>
                  ...
                  <tr style={{ borderTop: "2px solid rgba(148,163,184,0.2)", background: "rgba(99,102,241,0.04)" }}>
                    ...
                    <td style={{ textAlign: "center", fontWeight: 700, color: "#16a34a", fontSize: 10 }}>{fmtEur(Math.round(totalLoyerAn / 12))}</td>
