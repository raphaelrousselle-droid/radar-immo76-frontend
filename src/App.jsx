...
function SimulateurExploitation() {
  var _i = React.useState({
    prixAchat: "175000", surface: "80", nbPieces: "4", nbChambres: "3",
    loyerNuMarche: "600", ville: "Dieppe",
  ...
  var surface = pf(inputs.surface);
  var nbChambres = Math.max(1, Math.round(pf(inputs.nbChambres)));
  var loyerNu = pf(inputs.loyerNuMarche);
  ...
  var nM = Math.max(1, pf(inputs.dureeCredit)) * 12;
  var mensualite = tM === 0 ? emprunt / nM : (emprunt * tM) / (1 - Math.pow(1 + tM, -nM));
  ...
  var modes = [
    {
      id: "nu", nom: "Location nue", icon: "🏠", color: "#6366f1", badge: "Classique",
      loyerMois: loyerNu,
      occupationMois: 11.5,
      chargesSpecAn: 0,
      mobilierInit: 0,
      gestionPct: 0,
      gliPct: 3,
      desc: "Bail 3 ans · Gestion simple · Turnover faible",
      risque: 2, gestion: 1,
    },
    {
      id: "meuble", nom: "Meublé (LMNP)", icon: "🛋️", color: "#0ea5e9", badge: "Optimisé",
      loyerMois: Math.round(loyerNu * 1.2),
      occupationMois: 11,
      chargesSpecAn: 300, // entretien mobilier
      mobilierInit: Math.round(surface * 80),
      gestionPct: 0,
      gliPct: 3.5,
      desc: "Bail 1 an · Loyer +20% · Amortissement mobilier · Régime BIC",
      risque: 3, gestion: 2,
    },
    {
      id: "coloc", nom: "Colocation", icon: "👥", color: "#22c55e", badge: "Rendement +",
      loyerMois: Math.round(nbChambres * loyerNu * 0.55),
      occupationMois: 10.5,
      chargesSpecAn: 1200, // charges incluses (eau, elec, internet)
      mobilierInit: Math.round(nbChambres * 2000),
      gestionPct: 0,
      gliPct: 4,
      desc: nbChambres + " chambres · Loyer/chambre · Charges incluses · Turnover moyen",
      risque: 4, gestion: 3,
    },
    {
      id: "lcd", nom: "Courte durée (LCD)", icon: "✈️", color: "#f59e0b", badge: "Max rendement",
      loyerMois: Math.round(surface * 0.8 * 22),
      occupationMois: 8,
      chargesSpecAn: Math.round(loyerNu * 12 * 0.35), // ménage, linge, conso, plateforme
      mobilierInit: Math.round(surface * 120),
      gestionPct: 0,
      gliPct: 0,
      desc: "Nuitée ~" + fmt(Math.round(surface * 0.8), 0) + " € · Taux occup. ~67% · Charges élevées",
      risque: 5, gestion: 5,
    },
  ];

  // Calculer les résultats pour chaque mode
  var resultats = modes.map(function(mode) {
    var loyerAn = mode.loyerMois * mode.occupationMois;
    var gliAn = loyerAn * mode.gliPct / 100;
    var gestionAn = loyerAn * mode.gestionPct / 100;
    var totalChargesAn = chargesBaseAn + mode.chargesSpecAn + gliAn + gestionAn;
    var cashflowAn = loyerAn - totalChargesAn - creditAn;
    var cashflowMois = cashflowAn / 12;
    var rendBrut = depenseNette > 0 ? (loyerAn / depenseNette * 100) : 0;
    var rendNet = depenseNette > 0 ? (cashflowAn / depenseNette * 100) : 0;
    return Object.assign({}, mode, {
      loyerAn: loyerAn, totalChargesAn: totalChargesAn, cashflowAn: cashflowAn,
      cashflowMois: cashflowMois, rendBrut: rendBrut, rendNet: rendNet,
    });
  });

  var best = resultats.reduce(function(a, b) { return b.cashflowAn > a.cashflowAn ? b : a; });

  // Radar chart SVG
  function RadarChart() {
    var axes = [
      { label: "Rendement", key: "rendBrut" },
      { label: "Cash-flow", key: "cashflowMois" },
      { label: "Simplicité", key: "simplicite" },
      { label: "Sécurité", key: "securite" },
      { label: "Fiscalité", key: "fiscalite" },
    ];
    var cx = 130, cy = 110, R = 80;
    var angleStep = (2 * Math.PI) / axes.length;
    function pt(axIdx, val) {
      var a = -Math.PI / 2 + axIdx * angleStep;
      var r = (val / 10) * R;
      return { x: cx + Math.cos(a) * r, y: cy + Math.sin(a) * r };
    }
    // Normalize values to 0-10 scale
    var maxRend = Math.max.apply(null, resultats.map(function(r2) { return r2.rendBrut; })) || 1;
    var maxCF = Math.max.apply(null, resultats.map(function(r2) { return Math.abs(r2.cashflowMois); })) || 1;

    return (
      <svg viewBox="0 0 260 220" style={{ width: "100%", maxWidth: 400, fontFamily: "system-ui" }}>
        {/* Grille */}
        {[2, 4, 6, 8, 10].map(function(lev) {
          var pts = axes.map(function(_, i) { var p = pt(i, lev); return p.x + "," + p.y; }).join(" ");
          return <polygon key={lev} points={pts} fill="none" stroke="rgba(148,163,184,0.2)" strokeWidth={lev === 10 ? 1.5 : 0.5} />;
        })}
        {axes.map(function(ax, i) {
          var p = pt(i, 10);
          var pl = pt(i, 11.5);
          return (<g key={ax.label}>
            <line x1={cx} y1={cy} x2={p.x} y2={p.y} stroke="rgba(148,163,184,0.25)" strokeWidth={0.5} />
            <text x={pl.x} y={pl.y + 3} textAnchor="middle" fontSize={9} fill="#64748b" fontWeight="600">{ax.label}</text>
          </g>);
        })}
        {/* Données par mode */}
        {resultats.map(function(r2) {
          var vals = [
            Math.min(10, r2.rendBrut / maxRend * 10),
            Math.min(10, Math.max(0, r2.cashflowMois) / maxCF * 10),
            (6 - r2.gestion) * 2,
            (6 - r2.risque) * 2,
            r2.id === "meuble" ? 9 : r2.id === "nu" ? 4 : r2.id === "coloc" ? 7 : 5,
          ];
          var pts = vals.map(function(v, i) { var p = pt(i, v); return p.x + "," + p.y; }).join(" ");
          return (<polygon key={r2.id} points={pts} fill={r2.color + "20"} stroke={r2.color} strokeWidth={2} />);
        })}
      </svg>
    );
  }
  ...
  return (
    <div style={{ display: "flex", flexDirection: "column", gap: 14 }}>
      ...
      {/* Paramètres du bien */}
      <div style={SECTION}>
        <SectionHeader icon="🏠" title="Caractéristiques du bien" />
        <div style={{ display: "grid", gridTemplateColumns: "repeat(auto-fit, minmax(140px, 1fr))", gap: 8 }}>
          {[
            { label: "Prix d'achat FAI", name: "prixAchat", unit: "€" },
            { label: "Surface", name: "surface", unit: "m²", step: "1" },
            { label: "Nb pièces", name: "nbPieces", unit: "", step: "1" },
            { label: "Nb chambres", name: "nbChambres", unit: "", step: "1" },
            { label: "Loyer nu marché", name: "loyerNuMarche", unit: "€/mois", step: "50" },
            { label: "Ville", name: "ville", unit: "" },
          ].map(function(f) {
            return (<div key={f.name}>
              <label style={labelS}>{f.label}</label>
              <div style={{ display: "flex", alignItems: "center", gap: 4 }}>
                <input type={f.name === "ville" ? "text" : "number"} name={f.name} value={inputs[f.name]} onChange={handleChange} step={f.step || "1000"} style={fieldS} />
                {f.unit && <span style={{ fontSize: 10, color: "#94a3b8", minWidth: 30 }}>{f.unit}</span>}
              </div>
            </div>);
          })}
        </div>
        <div style={{ display: "grid", gridTemplateColumns: "repeat(auto-fit, minmax(130px, 1fr))", gap: 8, marginTop: 8 }}>
          {[
            { label: "Frais notaire", name: "fraisNotaire" }, { label: "Travaux", name: "travaux" },
            { label: "Frais bancaires", name: "fraisBancaires" }, { label: "Apport", name: "apport" },
            { label: "Taux crédit", name: "tauxCredit", unit: "%", step: "0.05" }, { label: "Durée crédit", name: "dureeCredit", unit: "ans", step: "1" },
            { label: "Taxe foncière", name: "taxeFonciere", unit: "€/an" }, { label: "Charges/an", name: "chargesAn", unit: "€/an" },
            { label: "Assurance PNO", name: "assurancePNO", unit: "€/an" },
          ].map(function(f) {
            return (<div key={f.name}>
              <label style={labelS}>{f.label}</label>
              <div style={{ display: "flex", alignItems: "center", gap: 4 }}>
                <input type="number" name={f.name} value={inputs[f.name]} onChange={handleChange} step={f.step || "100"} style={fieldS} />
                {f.unit && <span style={{ fontSize: 10, color: "#94a3b8", minWidth: 28 }}>{f.unit}</span>}
              </div>
            </div>);
          })}
        </div>
      ...
      <div style={{ display: "grid", gridTemplateColumns: "repeat(auto-fit, minmax(220px, 1fr))", gap: 12 }}>
        {resultats.map(function(r2) {
          var isBest = r2.id === best.id;
          return (
            <div key={r2.id} style={{ background: "rgba(255,255,255,0.85)", borderRadius: 18, overflow: "hidden", border: isBest ? "2px solid " + r2.color : "1px solid rgba(148,163,184,0.2)", boxShadow: isBest ? "0 4px 20px " + r2.color + "25" : "0 2px 8px rgba(0,0,0,0.04)", position: "relative" }}>
              {isBest && <div style={{ background: r2.color, color: "#fff", fontSize: 10, fontWeight: 700, textAlign: "center", padding: "3px 0" }}>⭐ MEILLEUR CASH-FLOW</div>}
              <div style={{ padding: "14px 16px" }}>
                {/* Header */}
                <div style={{ display: "flex", alignItems: "center", gap: 8, marginBottom: 10 }}>
                  <span style={{ fontSize: 28 }}>{r2.icon}</span>
                  <div>
                    <div style={{ fontSize: 15, fontWeight: 800, color: "#0f172a" }}>{r2.nom}</div>
                    <span style={{ fontSize: 10, fontWeight: 600, color: r2.color, background: r2.color + "15", padding: "2px 8px", borderRadius: 6 }}>{r2.badge}</span>
                  </div>
                </div>
                <div style={{ fontSize: 11, color: "#64748b", marginBottom: 10, lineHeight: 1.4 }}>{r2.desc}</div>

                {/* KPIs */}
                <div style={{ display: "flex", flexDirection: "column", gap: 6 }}>
                  {[
                    { label: "Loyer mensuel", value: fmtEur(r2.loyerMois), color: "#16a34a" },
                    { label: "Loyer annuel", value: fmtEur(Math.round(r2.loyerAn)), color: "#16a34a" },
                    { label: "Charges totales/an", value: fmtEur(Math.round(r2.totalChargesAn)), color: "#dc2626" },
                    { label: "Crédit/an", value: fmtEur(Math.round(creditAn)), color: "#d97706" },
                  ].map(function(k) {
                    return (<div key={k.label} style={{ display: "flex", justifyContent: "space-between", fontSize: 12 }}>
                      <span style={{ color: "#64748b" }}>{k.label}</span>
                      <span style={{ fontWeight: 600, color: k.color }}>{k.value}</span>
                    </div>);
                  })}
                  <div style={{ borderTop: "1px solid rgba(148,163,184,0.2)", paddingTop: 6, marginTop: 2 }}>
                    {[
                      { label: "Cash-flow/mois", value: (r2.cashflowMois >= 0 ? "+" : "") + fmtEur(Math.round(r2.cashflowMois)), color: r2.cashflowMois >= 0 ? "#16a34a" : "#dc2626", big: true },
                      { label: "Rendement brut", value: fmtPct(r2.rendBrut), color: r2.color },
                      { label: "Rendement net", value: fmtPct(r2.rendNet), color: r2.rendNet >= 0 ? "#16a34a" : "#dc2626" },
                    ].map(function(k) {
                      return (<div key={k.label} style={{ display: "flex", justifyContent: "space-between", fontSize: k.big ? 14 : 12, marginBottom: k.big ? 4 : 0 }}>
                        <span style={{ color: "#64748b", fontWeight: k.big ? 600 : 400 }}>{k.label}</span>
                        <span style={{ fontWeight: 700, color: k.color, fontSize: k.big ? 16 : 12 }}>{k.value}</span>
                      </div>);
                    })}
                  </div>
                </div>
                ...
                {r2.mobilierInit > 0 && (
                  <div style={{ marginTop: 8, background: "rgba(248,250,252,0.8)", borderRadius: 8, padding: "6px 8px", fontSize: 11, color: "#64748b" }}>
                    🛒 Investissement mobilier : <b style={{ color: "#0f172a" }}>{fmtEur(r2.mobilierInit)}</b>
                  </div>
                )}

                {/* Barres risque / gestion */}
                <div style={{ display: "grid", gridTemplateColumns: "1fr 1fr", gap: 8, marginTop: 8 }}>
                  {[
                    { label: "Risque", value: r2.risque, max: 5, color: r2.risque <= 2 ? "#16a34a" : r2.risque <= 3 ? "#f59e0b" : "#dc2626" },
                    { label: "Gestion", value: r2.gestion, max: 5, color: r2.gestion <= 2 ? "#16a34a" : r2.gestion <= 3 ? "#f59e0b" : "#dc2626" },
                  ].map(function(b) {
                    return (<div key={b.label}>
                      <div style={{ fontSize: 10, color: "#94a3b8", marginBottom: 2 }}>{b.label} ({b.value}/{b.max})</div>
                      <div style={{ height: 5, background: "rgba(148,163,184,0.15)", borderRadius: 999 }}>
                        <div style={{ width: (b.value / b.max * 100) + "%", height: "100%", background: b.color, borderRadius: 999 }} />
                      </div>
                    </div>);
                  })}
                </div>
      ...
      {/* Radar chart */}
      <div style={SECTION}>
        <SectionHeader icon="📊" title="Comparaison radar" />
        <div style={{ display: "flex", alignItems: "center", gap: 20, flexWrap: "wrap" }}>
          <RadarChart />
          <div style={{ display: "flex", flexDirection: "column", gap: 6 }}>
            {resultats.map(function(r2) {
              return (<div key={r2.id} style={{ display: "flex", alignItems: "center", gap: 8, fontSize: 12 }}>
                <div style={{ width: 14, height: 14, borderRadius: 4, background: r2.color }} />
                <span style={{ fontWeight: 600, color: "#0f172a" }}>{r2.nom}</span>
                <span style={{ color: "#64748b" }}>— CF : {(r2.cashflowMois >= 0 ? "+" : "") + fmt(Math.round(r2.cashflowMois), 0)} €/mois</span>
              </div>);
            })}
          </div>
        </div>
      ...
      {/* Tableau comparatif */}
      <div style={SECTION}>
        <SectionHeader icon="📋" title="Tableau comparatif détaillé" />
        <div style={{ overflowX: "auto" }}>
          <table style={{ width: "100%", borderCollapse: "collapse", fontSize: 12 }}>
            <thead>
              <tr style={{ borderBottom: "2px solid rgba(148,163,184,0.2)" }}>
                {["", "Location nue", "Meublé LMNP", "Colocation", "Courte durée"].map(function(h) {
                  return <th key={h} style={{ padding: "8px 10px", textAlign: h === "" ? "left" : "center", fontSize: 11, fontWeight: 600, color: "#64748b" }}>{h}</th>;
                })}
              </tr>
            </thead>
            <tbody>
              {[
                { label: "Loyer/mois", key: "loyerMois", fmt: function(v) { return fmtEur(v); } },
                { label: "Occupation", key: "occupationMois", fmt: function(v) { return fmt(v, 1) + " mois/an"; } },
                { label: "Loyer annuel", key: "loyerAn", fmt: function(v) { return fmtEur(Math.round(v)); } },
                { label: "Charges/an", key: "totalChargesAn", fmt: function(v) { return fmtEur(Math.round(v)); }, color: "#dc2626" },
                { label: "Cash-flow/mois", key: "cashflowMois", fmt: function(v) { return (v >= 0 ? "+" : "") + fmtEur(Math.round(v)); }, bold: true },
                { label: "Cash-flow/an", key: "cashflowAn", fmt: function(v) { return (v >= 0 ? "+" : "") + fmtEur(Math.round(v)); }, bold: true },
                { label: "Rendement brut", key: "rendBrut", fmt: function(v) { return fmtPct(v); } },
                { label: "Rendement net", key: "rendNet", fmt: function(v) { return fmtPct(v); } },
                { label: "Mobilier initial", key: "mobilierInit", fmt: function(v) { return v > 0 ? fmtEur(v) : "—"; } },
                { label: "Risque", key: "risque", fmt: function(v) { return v + "/5"; } },
                { label: "Gestion", key: "gestion", fmt: function(v) { return v + "/5"; } },
              ].map(function(row, ridx) {
                return (<tr key={row.label} style={{ borderBottom: "1px solid rgba(148,163,184,0.1)", background: ridx % 2 === 0 ? "rgba(248,250,252,0.5)" : "transparent" }}>
                  <td style={{ padding: "7px 10px", fontWeight: row.bold ? 600 : 400, color: "#334155", fontSize: 11 }}>{row.label}</td>
                  {resultats.map(function(r2) {
                    var val = r2[row.key];
                    var isBestRow = row.bold && r2.id === best.id;
                    var cellColor = row.color || (row.bold ? (val >= 0 ? "#16a34a" : "#dc2626") : "#0f172a");
                    return (<td key={r2.id} style={{ padding: "7px 10px", textAlign: "center", fontWeight: row.bold ? 700 : 500, color: cellColor, fontSize: row.bold ? 13 : 12, background: isBestRow ? "rgba(22,163,74,0.06)" : "transparent" }}>
                      {row.fmt(val)}
                    </td>);
                  })}
                </tr>);
