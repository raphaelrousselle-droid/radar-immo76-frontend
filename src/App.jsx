...
function SimulationProjet({ photos, setPhotos, projets, setProjets, projetACharger, onProjetCharge }) {
  ...
  const exportPDF = async function() {
    ...
    if (photos && photos[0] && photos[0].url) {
      try {
        // Calculer le ratio pour ne pas écraser la photo
        var img = new Image();
        img.src = photos[0].url;
        var imgW = img.naturalWidth || img.width || CW;
        var imgH = img.naturalHeight || img.height || 60;
        var ratio = imgW / imgH;
        var pdfImgW = CW;
        var pdfImgH = Math.min(70, CW / ratio);
        doc.addImage(photos[0].url, "JPEG", ML, y, pdfImgW, pdfImgH, "", "MEDIUM");
        y += pdfImgH + 4;
      } catch(e) { y += 2; }
    }

    var kw = (CW - 8) / 3;
    kpiBox(ML, y, kw, 22, "Prix d'achat FAI", fmtP(pv, 0) + " €", C.green);
    kpiBox(ML + kw + 4, y, kw, 22, "Superficie", surf > 0 ? fmtP(surf, 0) + " m²" : "—", C.red);
    kpiBox(ML + kw * 2 + 8, y, kw, 22, "Rendement brut", fmtPctP(r.rendBrut), C.orange);
    y += 26;
    ...
    if (lots.length > 0) {
      ...
      lots.slice(0, 8).forEach(function(l) {
        ...
        doc.text(l.loyer ? fmtP(pf(l.loyer), 0) + " €" : "—", ML + 85, y);
        doc.text(pf(l.travaux) > 0 ? fmtP(pf(l.travaux), 0) + " €" : "—", ML + 120, y);
    ...
    kpiBox(ML + kw2 + 4, y, kw2, 20, "Rendement net", fmtPctP(regime.rendNet), C.primary);
    y += 24;
    ...
    var blocks = [
      { title: "Achat", x: ML, rows: [
        ...
        ["Autre frais", "0 €"], ["Meubles", "0 €"], ["Travaux", fmtP(pf(inputs.travaux), 0) + " €"],
        ["Frais bancaire et cautionnement", fmtP(pf(inputs.fraisBancairesAn), 0) + " €"],
        ["Total achat", fmtP(Math.round(r.depenseNette), 0) + " €"],
      ]},
      { title: "Crédit", x: ML + tw + 4, rows: [
        ["Apport", fmtP(pf(inputs.apport), 0) + " €"], ["Montant emprunté", fmtP(Math.round(r.sommeEmpruntee), 0) + " €"],
        ["Frais bancaire et cautionnement", fmtP(pf(inputs.fraisBancairesAn), 0) + " €"],
