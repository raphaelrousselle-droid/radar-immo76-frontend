    ...
    if (profilData && profilData.emprunteurs && profilData.emprunteurs[0] && profilData.emprunteurs[0].nom) {
      ...
      function pdfTable(headers, rows, colWidths, startY) {
        ...
        rows.forEach(function(row, ridx) {
          if (ridx % 2 === 0) { setF(C.bg); doc.rect(ML, startY - 3.5, CW, 6, "F"); }
          tx = ML;
          doc.setFontSize(7.5); doc.setFont("helvetica", "normal"); setC(C.dark);
          row.forEach(function(cell, ci) {
            var isLast = ridx === rows.length - 1;
            if (isLast) { doc.setFont("helvetica", "bold"); setC(C.primary); }
            doc.text(String(cell || "—"), tx + 2, startY);
            tx += colWidths[ci];
          });
          startY += 6;
        });
      ...
      y = pdfTable(["Type et adresse", "Propriété", "Année", "Val. achat", "Val. estimée", "Capital dû"], immoRows, [38, 25, 18, 28, 28, 28], y);
      y += 6;
