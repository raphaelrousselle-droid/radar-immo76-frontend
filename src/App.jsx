...
function DVFPanel({ commune }) {
  ...
  async function lancerRecherche(codeInsee, typeFiltre) {
    ...
    try {
      var params = new URLSearchParams({ page_size: 500 });
      if (typeFiltre) params.set("type_local", typeFiltre);
      var url = "https://apidf-preprod.cerema.fr/dvf_opendata/communes/" + codeInsee + "/mutations/?" + params.toString();
      var resp = await fetch(url);
      if (!resp.ok) throw new Error("Erreur API (" + resp.status + ")");
      var data = await resp.json();
      var results = data.results || [];
      var next = data.next; var page = 2;
      while (next && page <= 5) {
        var r2 = await fetch(next);
        if (!r2.ok) break;
