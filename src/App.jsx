...
function AmortChartSCI({ sciVals, calc }) {
  const [hovered, setHovered] = React.useState(null);

  const data = React.useMemo(function() {
    const pa = pf(sciVals.prixAchat);
    const fa = pf(sciVals.fraisAchat);
    const tv = pf(sciVals.travaux);
    const M = pf(sciVals.montantCredit);
    const duree = Math.max(1, Math.round(pf(sciVals.dureeCredit)));
    const tMensuel = pf(sciVals.tauxCredit) / 100 / 12;
    const nMois = duree * 12;
    const mensualite = tMensuel === 0 ? M / nMois : (M * tMensuel) / (1 - Math.pow(1 + tMensuel, -nMois));
    const loyerBase = pf(sciVals.loyerMensuel) * pf(sciVals.tauxOccupation);
    const gestionPct = pf(sciVals.gestionPct) / 100;
    const chargesBase = pf(sciVals.chargesAn) + pf(sciVals.taxeFonciereAn)
      + (pf(sciVals.assurancePNOAn) > 0 ? pf(sciVals.assurancePNOAn) : (pa + tv) * 0.0012)
      + pf(sciVals.provisionTravauxAn) + pf(sciVals.expertComptableAn) + pf(sciVals.fraisBancairesAn);

    // Amortissements fixes par an
    const amortBat = pa * (1 - pf(sciVals.partTerrain) / 100) * pf(sciVals.coefAmortBatiment) / 100;
    const amortTrav = tv * pf(sciVals.coefAmortTravaux) / 100;
    const amortFraisAcquis = sciVals.fraisNotaireAmort === "1" ? fa / 5 : 0;
    const amortFraisDossier = pf(sciVals.fraisDossierCredit) / Math.max(1, pf(sciVals.dureeFraisDossier));
    const provGros = pf(sciVals.provisionGrosTravauxAn);
    const tIS = pf(sciVals.tauxIS) / 100;
    const totalYears = Math.min(duree + 5, 30);

    var solde = M;
    var result = [];
    for (var y = 1; y <= totalYears; y++) {
      const loyersAn = loyerBase * Math.pow(1.01, y - 1);
      const gestionAn = loyersAn * gestionPct;
      const chargesAn = chargesBase + gestionAn;
      ...
      if (y <= duree && tMensuel > 0) {
        ...
        capitalAn = M / duree;
        solde = Math.max(0, solde - capitalAn);
      }

      // Amortissements : frais notaire amortis seulement sur 5 ans
      const amortFraisY = (y <= 5) ? amortFraisAcquis : 0;
      // Frais notaire déduits an 1 si option 0
      const fraisAn1 = (y === 1 && sciVals.fraisNotaireAmort === "0") ? fa : 0;
      // Amort frais dossier sur durée crédit
      const amortDossierY = (y <= duree) ? amortFraisDossier : 0;
      const totalAmort = amortBat + amortTrav + amortFraisY + amortDossierY;

      const baseFiscale = Math.max(0, loyersAn - chargesAn - interetsAn - totalAmort - fraisAn1 - provGros);
      const isAn = baseFiscale * tIS;
      ...
      result.push({
        ...
        amortBat: amortBat,
        amortTrav: amortTrav,
        amortFrais: amortFraisY + amortDossierY,
        baseFiscale: baseFiscale,
        isAn: isAn,
  ...
  const maxV = Math.max(...data.map(function(d) { return Math.max(d.amortBat + d.amortTrav + d.amortFrais + d.interets, d.isAn); })) * 1.15 || 1;
  const toY = function(v) { return padT + chartH - (v / maxV) * chartH; };
  const barW = Math.max(4, chartW / data.length - 2);
  const barCenterX = function(i) { return padL + i * (chartW / data.length) + (chartW / data.length) / 2; };

  const h = hovered !== null ? data[hovered] : null;
  const ttW = 160;
  const ttH = 116;
  var ttX = hovered !== null ? barCenterX(hovered) - ttW / 2 : 0;
  if (ttX < padL) ttX = padL;
  if (ttX + ttW > W - padR + 10) ttX = W - padR + 10 - ttW;

  const legend = [
    { color: "#818cf8", label: "Amort. bâtiment" },
    { color: "#a78bfa", label: "Amort. travaux/frais" },
    { color: "#38bdf8", label: "Intérêts crédit" },
    { color: "#fbbf24", label: "Base fiscale IS" },
    { color: "#f87171", label: "IS payé" },
  ];

  return (
    <div style={Object.assign({}, SECTION, { marginTop: 0 })}>
      <SectionHeader icon="📉" title="Évolution fiscale sur la durée du crédit" />
      <div style={{ fontSize: 11, color: "#64748b", marginBottom: 10 }}>
        Déductions fiscales et IS dû chaque année. Les intérêts baissent progressivement → la base IS augmente.
      </div>
      <div style={{ overflowX: "auto" }}>
        <svg viewBox={"0 0 " + W + " " + H} style={{ width: "100%", minWidth: 480, fontFamily: "system-ui,sans-serif" }}
          ...
          {[0, 0.25, 0.5, 0.75, 1].map(function(t) {
            ...
            return (
              <g key={t}>
                <line x1={padL} y1={y} x2={padL + chartW} y2={y} stroke="rgba(148,163,184,0.25)" strokeWidth={1} strokeDasharray="4 3" />
                <text x={padL - 5} y={y + 4} textAnchor="end" fontSize={9} fill="#94a3b8">{fmtK(v)}</text>
              </g>
            );
          })}
          {hovered !== null && (
            <rect x={padL + hovered * (chartW / data.length)} y={padT} width={chartW / data.length} height={chartH} fill="rgba(99,102,241,0.06)" rx={3} />
          )}
          {data.map(function(d, i) {
            const x = padL + i * (chartW / data.length) + (chartW / data.length - barW) / 2;
            const op = hovered !== null && hovered !== i ? 0.5 : 1;
            const hBat = Math.max(0, (d.amortBat / maxV) * chartH);
            const hTrav = Math.max(0, (d.amortTrav / maxV) * chartH);
            const hFrais = Math.max(0, (d.amortFrais / maxV) * chartH);
            const hInt = Math.max(0, (d.interets / maxV) * chartH);
            const yBase = toY(d.baseFiscale);
            const yIS = toY(d.isAn);
            const basePileTop = padT + chartH - hBat - hTrav - hFrais - hInt;
            return (
              <g key={i} opacity={op} onMouseEnter={function() { setHovered(i); }} style={{ cursor: "pointer" }}>
                {hBat > 0 && <rect x={x} y={padT + chartH - hBat} width={barW} height={hBat} fill="#818cf8" rx={2} />}
                {hTrav > 0 && <rect x={x} y={padT + chartH - hBat - hTrav} width={barW} height={hTrav} fill="#a78bfa" rx={2} />}
                {hFrais > 0 && <rect x={x} y={padT + chartH - hBat - hTrav - hFrais} width={barW} height={hFrais} fill="#c4b5fd" rx={2} />}
                {hInt > 0 && <rect x={x} y={basePileTop - hInt} width={barW} height={hInt} fill="#38bdf8" rx={2} />}
                {d.baseFiscale > 0 && (
                  <circle cx={barCenterX(i)} cy={yBase} r={hovered === i ? 5 : 3} fill="#fbbf24" stroke="white" strokeWidth={1.5} />
                )}
                {d.isAn > 0 && (
                  <circle cx={barCenterX(i)} cy={yIS} r={hovered === i ? 5 : 3} fill="#f87171" stroke="white" strokeWidth={1.5} />
                )}
              </g>
            );
          })}
          {/* Lignes courbes base fiscale et IS */}
          {data.some(function(d) { return d.baseFiscale > 0; }) && (
            <polyline
              points={data.filter(function(d) { return d.baseFiscale > 0; }).map(function(d, i) {
                return barCenterX(data.indexOf(d)) + "," + toY(d.baseFiscale);
              }).join(" ")}
              fill="none" stroke="#fbbf24" strokeWidth={2} strokeDasharray="5 3" strokeLinejoin="round" opacity={0.8}
            />
          )}
          {data.some(function(d) { return d.isAn > 0; }) && (
            <polyline
              points={data.filter(function(d) { return d.isAn > 0; }).map(function(d) {
                return barCenterX(data.indexOf(d)) + "," + toY(d.isAn);
              }).join(" ")}
              fill="none" stroke="#f87171" strokeWidth={2} strokeDasharray="5 3" strokeLinejoin="round" opacity={0.8}
            />
          )}
          {/* Axe X labels */}
          {data.map(function(d, i) {
            const showLabel = data.length <= 20 ? true : (i === 0 || (i + 1) % 5 === 0);
            if (!showLabel) return null;
            return (
              <text key={i} x={barCenterX(i)} y={H - padB + 14} textAnchor="middle" fontSize={9} fill="#94a3b8">
                {d.year}
              </text>
            );
          })}
          ...
          {legend.map(function(l, i) {
            return (
              <g key={l.label} transform={"translate(" + (W - padR + 12) + "," + (padT + 4 + i * 18) + ")"}>
          ...
          {h !== null && (
            <g>
              ...
              <text x={ttX + 8} y={17} fontSize={11} fontWeight="700" fill="#0f172a">Année {h.year}</text>
              {[
                { label: "Amort. total", value: h.totalAmort, color: "#818cf8" },
                { label: "dont intérêts", value: h.interets, color: "#38bdf8" },
                { label: "Base fiscale", value: h.baseFiscale, color: "#fbbf24" },
                { label: "IS dû", value: h.isAn, color: "#f87171" },
              ].map(function(row, idx) {
                return (
                  <g key={row.label} transform={"translate(" + ttX + "," + (24 + idx * 20) + ")"}>
                    <circle cx={8} cy={5} r={4} fill={row.color} />
                    <text x={18} y={9} fontSize={9} fill="#475569">{row.label}</text>
                    <text x={ttW - 6} y={9} fontSize={9} fontWeight="600" fill={row.color} textAnchor="end">{fmtK(row.value)}</text>
                  </g>
                );
              })}
            </g>
...
function SimulateurSCI() {
  const [sciVals, setSciVals] = useState({
    prixAchat: "200000",
    fraisAchat: "16000",
    travaux: "0",
    apport: "20000",
  ...
  const handleSciChange = useCallback(function(field, value) {
    setSciVals(function(prev) { return Object.assign({}, prev, { [field]: value }); });
  }, []);
  ...
  const calc = useMemo(function() {
    ...
    const M = pf(sciVals.montantCredit);
    const tMensuel = pf(sciVals.tauxCredit) / 100 / 12;
    const nMois = Math.max(1, pf(sciVals.dureeCredit)) * 12;
    const mensualite = tMensuel === 0 ? M / nMois : (M * tMensuel) / (1 - Math.pow(1 + tMensuel, -nMois));
    const remboursementAn = mensualite * 12;

    const loyersAn = pf(sciVals.loyerMensuel) * pf(sciVals.tauxOccupation);
    const gestionAn = loyersAn * pf(sciVals.gestionPct) / 100;
    const assurancePNO = pf(sciVals.assurancePNOAn) > 0 ? pf(sciVals.assurancePNOAn) : (pa + tv) * 0.0012;
    const chargesTotal = pf(sciVals.chargesAn) + pf(sciVals.taxeFonciereAn) + assurancePNO + gestionAn
      + pf(sciVals.provisionTravauxAn) + pf(sciVals.expertComptableAn) + pf(sciVals.fraisBancairesAn);

    // Intérêts année 1
    const interetsAn1 = tMensuel > 0 ? (function() {
      var solde = M; var interets = 0;
      for (var m = 0; m < 12; m++) {
        var intM = solde * tMensuel;
        interets += intM;
    ...
    // Amortissements bâtiment + travaux
    const baseAmortBatiment = pa * (1 - pf(sciVals.partTerrain) / 100);
    const amortBatimentAn = baseAmortBatiment * pf(sciVals.coefAmortBatiment) / 100;
    const amortTravauxAn = tv * pf(sciVals.coefAmortTravaux) / 100;

    // Frais notaire : amortis sur 5 ans OU déduits année 1
    const amortFraisAcquis = sciVals.fraisNotaireAmort === "1" ? fa / 5 : 0;
    const fraisAcquisAn1 = sciVals.fraisNotaireAmort === "0" ? fa : 0;

    // Frais de dossier crédit amortis sur durée du crédit
    const amortFraisDossier = pf(sciVals.fraisDossierCredit) / Math.max(1, pf(sciVals.dureeFraisDossier));

    // Total amortissements
    const amortTotalAn = amortBatimentAn + amortTravauxAn;
    const amortGrandTotal = amortTotalAn + amortFraisAcquis + amortFraisDossier;

    // Provision gros travaux (charge déductible)
    const provisionGrosAn = pf(sciVals.provisionGrosTravauxAn);

    // Résultat fiscal IS
    const resultatFiscal = Math.max(0,
      loyersAn - chargesTotal - interetsAn1 - amortGrandTotal - fraisAcquisAn1 - provisionGrosAn
    );
    const tIS = pf(sciVals.tauxIS) / 100;
    const isAn = resultatFiscal * tIS;

    // Trésorerie
    const tresoAn = loyersAn - chargesTotal - remboursementAn - isAn - provisionGrosAn;
    const tresoMois = tresoAn / 12;

    // Résultat après IS
    const resultatApresIS = Math.max(0, loyersAn - chargesTotal - interetsAn1 - amortGrandTotal - fraisAcquisAn1 - provisionGrosAn) - isAn;

    // Dividendes
    const tDiv = pf(sciVals.tauxDividendes) / 100;
    const dividendesMax = Math.max(0, resultatApresIS);
    const pfuSurDividendes = dividendesMax * tDiv;
    const dividendesNets = dividendesMax - pfuSurDividendes;
    const dividendesParPart = pf(sciVals.nbParts) > 0 ? dividendesNets / pf(sciVals.nbParts) : 0;

    // Rentabilité
    const investTotal = pa + fa + tv;
    const rendBrut = investTotal > 0 ? loyersAn / investTotal * 100 : 0;
    const rendNet = investTotal > 0 ? tresoAn / investTotal * 100 : 0;
    const regle70 = loyersAn > 0 ? remboursementAn / loyersAn : null;

    return {
      mensualite, remboursementAn, loyersAn, chargesTotal, interetsAn1,
      amortBatimentAn, amortTravauxAn, amortTotalAn,
      amortFraisAcquis, amortFraisDossier, amortGrandTotal,
      fraisAcquisAn1, provisionGrosAn,
      resultatFiscal, isAn, tresoAn, tresoMois,
      resultatApresIS, dividendesMax, pfuSurDividendes, dividendesNets, dividendesParPart,
      rendBrut, rendNet, regle70, investTotal, assurancePNO, gestionAn,
    };
  }, [sciVals]);
  ...
  return (
    <div style={{ display: "flex", flexDirection: "column", gap: 14 }}>
      ...
      <div style={{ background: "linear-gradient(135deg,rgba(99,102,241,0.08),rgba(56,189,248,0.08))", borderRadius: 14, padding: "12px 16px", border: "1px solid rgba(99,102,241,0.2)", fontSize: 12, color: "#334155" }}>
        <strong>SCI à l'IS</strong> — Les loyers sont imposés après déduction des amortissements (bâtiment, travaux, frais d'acquisition, frais de dossier) et des charges. Les bénéfices nets sont distribués sous forme de dividendes soumis au PFU 30%.
      </div>
      ...
      <div style={{ display: "grid", gridTemplateColumns: "repeat(auto-fit, minmax(280px, 1fr))", gap: 14 }}>
        ...
        <div style={{ display: "flex", flexDirection: "column", gap: 14 }}>
          ...
          <div style={SECTION}>
            ...
            <div style={{ display: "flex", flexDirection: "column", gap: 10 }}>
              ...
              <SCIField sciVals={sciVals} onChange={handleSciChange} field="prixAchat" label="Prix d'achat" unit="€" step="5000" />
              <SCIField sciVals={sciVals} onChange={handleSciChange} field="fraisAchat" label="Frais notaire + agence" unit="€" step="500" />
              <SCIField sciVals={sciVals} onChange={handleSciChange} field="travaux" label="Travaux" unit="€" step="1000" />
              <div style={sepS}>Financement</div>
              <SCIField sciVals={sciVals} onChange={handleSciChange} field="apport" label="Apport" unit="€" step="5000" />
              <SCIField sciVals={sciVals} onChange={handleSciChange} field="montantCredit" label="Montant emprunté" unit="€" step="5000" />
              <SCIField sciVals={sciVals} onChange={handleSciChange} field="tauxCredit" label="Taux hors assurance" unit="%" step="0.05" />
              <SCIField sciVals={sciVals} onChange={handleSciChange} field="dureeCredit" label="Durée du crédit" unit="ans" step="1" />
            </div>
          </div>
          ...
          <div style={SECTION}>
            ...
            <div style={{ display: "flex", flexDirection: "column", gap: 10 }}>
              <SCIField sciVals={sciVals} onChange={handleSciChange} field="loyerMensuel" label="Loyer mensuel HC" unit="€" step="50" />
              <div>
                <label style={labelS}>Taux d'occupation</label>
                <div style={{ display: "flex", alignItems: "center", gap: 5 }}>
                  <input type="number" value={sciVals.tauxOccupation} step="0.5" min="0" max="12"
                    onChange={function(e) { handleSciChange("tauxOccupation", e.target.value); }} style={inputS} />
                  <span style={{ fontSize: 11, color: "#94a3b8", minWidth: 65 }}>mois/an</span>
                </div>
              </div>
              <div style={sepS}>Charges annuelles</div>
              <SCIField sciVals={sciVals} onChange={handleSciChange} field="chargesAn" label="Charges immeuble" unit="€/an" />
              <SCIField sciVals={sciVals} onChange={handleSciChange} field="taxeFonciereAn" label="Taxe foncière" unit="€/an" />
              <SCIField sciVals={sciVals} onChange={handleSciChange} field="assurancePNOAn" label="Assurance PNO" unit="€/an" hint="0 = auto (0.12% du prix + travaux)" />
              <SCIField sciVals={sciVals} onChange={handleSciChange} field="gestionPct" label="Gestion locative" unit="%/an" step="0.5" />
              <SCIField sciVals={sciVals} onChange={handleSciChange} field="provisionTravauxAn" label="Provision entretien courant" unit="€/an" />
              <SCIField sciVals={sciVals} onChange={handleSciChange} field="expertComptableAn" label="Expert-comptable" unit="€/an" hint="Obligatoire en SCI IS" />
              <SCIField sciVals={sciVals} onChange={handleSciChange} field="fraisBancairesAn" label="Frais bancaires" unit="€/an" step="50" />
            </div>
          </div>
          ...
          <div style={SECTION}>
            ...
            <div style={{ display: "flex", flexDirection: "column", gap: 10 }}>
              <div style={sepS}>Amortissements immobiliers</div>
              <SCIField sciVals={sciVals} onChange={handleSciChange} field="coefAmortBatiment" label="Taux amort. bâtiment" unit="%/an" step="0.25" hint="Généralement 2.5% (hors terrain)" />
              <SCIField sciVals={sciVals} onChange={handleSciChange} field="coefAmortTravaux" label="Taux amort. travaux" unit="%/an" step="1" hint="Généralement 10% (10 ans)" />
              <SCIField sciVals={sciVals} onChange={handleSciChange} field="partTerrain" label="Part terrain (non amortissable)" unit="%" step="1" hint="Généralement 15-20% du prix" />

              <div style={sepS}>Frais & provisions</div>
              <div>
                <label style={labelS}>Frais notaire + agence ({fmtEur(pf(sciVals.fraisAchat))})</label>
                <div style={{ display: "flex", gap: 6 }}>
                  {[{ val: "1", label: "Amortis sur 5 ans" }, { val: "0", label: "Déduits année 1" }].map(function(t) {
                    const isA = sciVals.fraisNotaireAmort === t.val;
                    return (
                      <button key={t.val} onClick={function() { handleSciChange("fraisNotaireAmort", t.val); }}
                        style={{ flex: 1, padding: "6px 8px", borderRadius: 9, border: "none", cursor: "pointer", fontSize: 11, fontWeight: 600, background: isA ? "rgba(99,102,241,0.9)" : "rgba(148,163,184,0.12)", color: isA ? "#fff" : "#64748b" }}>
                        {t.label}
                      </button>
                    );
              ...
              <SCIField sciVals={sciVals} onChange={handleSciChange} field="fraisDossierCredit" label="Frais de dossier crédit" unit="€" step="100" />
              <SCIField sciVals={sciVals} onChange={handleSciChange} field="dureeFraisDossier" label="Amortis sur (durée crédit)" unit="ans" step="1" />
              <SCIField sciVals={sciVals} onChange={handleSciChange} field="provisionGrosTravauxAn" label="Provision gros travaux futurs" unit="€/an" step="100" hint="Charge déductible chaque année" />
              ...
              <div>
                ...
                <div style={{ display: "flex", gap: 6 }}>
                  {[{ val: "15", label: "15% (≤ 42 500 €)" }, { val: "25", label: "25% (normal)" }].map(function(t) {
                    const isA = sciVals.tauxIS === t.val;
                    return (
                      <button key={t.val} onClick={function() { handleSciChange("tauxIS", t.val); }}
                        style={{ flex: 1, padding: "7px 8px", borderRadius: 9, border: "none", cursor: "pointer", fontSize: 11, fontWeight: 600, background: isA ? "rgba(99,102,241,0.9)" : "rgba(148,163,184,0.12)", color: isA ? "#fff" : "#64748b" }}>
                        {t.label}
                      </button>
                    );
                  })}
              ...
              <div style={sepS}>Dividendes</div>
              <div>
                <label style={labelS}>Taux dividendes</label>
                <div style={{ display: "flex", gap: 6 }}>
                  {[{ val: "30", label: "30% (PFU flat tax)" }, { val: "17.2", label: "17.2% (PS seuls)" }].map(function(t) {
                    const isA = sciVals.tauxDividendes === t.val;
                    return (
                      <button key={t.val} onClick={function() { handleSciChange("tauxDividendes", t.val); }}
                        style={{ flex: 1, padding: "7px 8px", borderRadius: 9, border: "none", cursor: "pointer", fontSize: 11, fontWeight: 600, background: isA ? "rgba(99,102,241,0.9)" : "rgba(148,163,184,0.12)", color: isA ? "#fff" : "#64748b" }}>
                        {t.label}
                      </button>
                    );
              ...
              <SCIField sciVals={sciVals} onChange={handleSciChange} field="nbParts" label="Nombre de parts" unit="parts" step="1" hint="Pour calcul dividende par associé" />
              <SCIField sciVals={sciVals} onChange={handleSciChange} field="capitalSocial" label="Capital social" unit="€" step="100" />
            </div>
          </div>
        </div>
        ...
        <div style={{ display: "flex", flexDirection: "column", gap: 14 }}>
          ...
          <div style={{ display: "grid", gridTemplateColumns: "1fr 1fr", gap: 10 }}>
            {[
              { label: "Trésorerie / mois", value: fmt(calc.tresoMois, 0) + " €", color: couleurTreso, icon: "💰" },
              { label: "Rendement brut", value: calc.rendBrut.toFixed(2) + " %", color: calc.rendBrut >= 7 ? "#16a34a" : calc.rendBrut >= 5 ? "#d97706" : "#dc2626", icon: "📈" },
              { label: "Rendement net", value: calc.rendNet.toFixed(2) + " %", color: couleurRendNet, icon: "🎯" },
              { label: "Règle des 70%", value: calc.regle70 !== null ? (calc.regle70 * 100).toFixed(0) + "%" : "—", color: calc.regle70 !== null && calc.regle70 <= 0.70 ? "#16a34a" : "#dc2626", icon: "📏" },
            ].map(function(kpi) {
              return (
                <div key={kpi.label} style={{ background: "rgba(255,255,255,0.8)", borderRadius: 14, padding: "14px", border: "1px solid rgba(148,163,184,0.2)", textAlign: "center", backdropFilter: "blur(12px)" }}>
          ...
          {/* Compte de résultat */}
          <div style={SECTION}>
            <SectionHeader icon="📋" title="Compte de résultat annuel (année 1)" />
            <StatRow label="Loyers encaissés" value={fmtEur(calc.loyersAn)} color="#16a34a" bold />
            <div style={{ margin: "6px 0", height: 1, background: "rgba(148,163,184,0.15)" }} />
            <StatRow label="Charges + TF + assurance" value={"– " + fmtEur(pf(sciVals.chargesAn) + pf(sciVals.taxeFonciereAn) + calc.assurancePNO)} color="#64748b" />
            <StatRow label="Gestion locative" value={"– " + fmtEur(calc.gestionAn)} color="#64748b" />
            <StatRow label="Provision entretien courant" value={"– " + fmtEur(pf(sciVals.provisionTravauxAn))} color="#64748b" />
            <StatRow label="Expert-comptable + frais bancaires" value={"– " + fmtEur(pf(sciVals.expertComptableAn) + pf(sciVals.fraisBancairesAn))} color="#64748b" />
            <StatRow label="Provision gros travaux futurs" value={"– " + fmtEur(calc.provisionGrosAn)} color="#64748b" />
            <StatRow label="Intérêts du crédit (an 1)" value={"– " + fmtEur(calc.interetsAn1)} color="#d97706" />
            <StatRow label="Amort. bâtiment" value={"– " + fmtEur(calc.amortBatimentAn)} color="#6366f1" />
            <StatRow label="Amort. travaux" value={"– " + fmtEur(calc.amortTravauxAn)} color="#6366f1" />
            <StatRow label={sciVals.fraisNotaireAmort === "1" ? "Amort. frais notaire/agence (÷5 ans)" : "Frais notaire/agence (déduits an 1)"} value={"– " + fmtEur(sciVals.fraisNotaireAmort === "1" ? calc.amortFraisAcquis : calc.fraisAcquisAn1)} color="#6366f1" />
            <StatRow label={"Amort. frais dossier crédit (÷" + sciVals.dureeFraisDossier + " ans)"} value={"– " + fmtEur(calc.amortFraisDossier)} color="#6366f1" />
            <div style={{ margin: "6px 0", height: 1, background: "rgba(148,163,184,0.15)" }} />
            <StatRow label="Résultat fiscal imposable" value={fmtEur(calc.resultatFiscal)} bold color={calc.resultatFiscal === 0 ? "#16a34a" : "#334155"} />
            <StatRow label={"IS à " + sciVals.tauxIS + "%"} value={"– " + fmtEur(calc.isAn)} color="#dc2626" />
            <div style={{ marginTop: 10, background: "rgba(220,252,231,0.7)", borderRadius: 10, padding: "10px 12px", border: "1px solid rgba(22,163,74,0.3)" }}>
              <div style={{ fontSize: 11, color: "#64748b" }}>Résultat net après IS</div>
              <div style={{ fontSize: 20, fontWeight: 800, color: calc.resultatApresIS >= 0 ? "#15803d" : "#dc2626" }}>{fmtEur(calc.resultatApresIS)}</div>
            </div>
          </div>

          {/* Trésorerie */}
          <div style={SECTION}>
            <SectionHeader icon="💳" title="Trésorerie annuelle" />
            <StatRow label="Loyers" value={fmtEur(calc.loyersAn)} color="#16a34a" />
            <StatRow label="– Charges totales" value={"– " + fmtEur(calc.chargesTotal)} color="#64748b" />
            <StatRow label="– Provision gros travaux" value={"– " + fmtEur(calc.provisionGrosAn)} color="#64748b" />
            <StatRow label="– Remboursement crédit" value={"– " + fmtEur(calc.remboursementAn)} color="#d97706" />
            <StatRow label="– IS" value={"– " + fmtEur(calc.isAn)} color="#dc2626" />
            <div style={{ marginTop: 10, background: calc.tresoAn >= 0 ? "rgba(220,252,231,0.8)" : "rgba(254,226,226,0.8)", borderRadius: 12, padding: "12px 14px", border: "1px solid " + (calc.tresoAn >= 0 ? "rgba(22,163,74,0.3)" : "rgba(220,38,38,0.3)") }}>
              <div style={{ fontSize: 12, color: "#64748b" }}>Trésorerie annuelle</div>
              <div style={{ fontSize: 26, fontWeight: 900, color: couleurTreso }}>{calc.tresoAn >= 0 ? "+" : ""}{fmtEur(calc.tresoAn)}</div>
              <div style={{ fontSize: 11, color: "#94a3b8", marginTop: 2 }}>soit {fmt(calc.tresoMois, 0)} €/mois</div>
            </div>
          </div>

          {/* Dividendes */}
          <div style={SECTION}>
            <SectionHeader icon="💸" title="Distribution de dividendes" />
            <StatRow label="Résultat net distribuable" value={fmtEur(calc.dividendesMax)} color="#16a34a" />
            <StatRow label={"PFU / PS (" + sciVals.tauxDividendes + "%)"} value={"– " + fmtEur(calc.pfuSurDividendes)} color="#dc2626" />
            <StatRow label="Dividendes nets" value={fmtEur(calc.dividendesNets)} bold color="#4338ca" border={false} />
            {pf(sciVals.nbParts) > 1 && (
              <div style={{ marginTop: 8, background: "rgba(99,102,241,0.07)", borderRadius: 10, padding: "8px 12px", border: "1px solid rgba(99,102,241,0.2)" }}>
                <div style={{ fontSize: 11, color: "#64748b" }}>Par associé ({sciVals.nbParts} parts égales)</div>
                <div style={{ fontSize: 18, fontWeight: 800, color: "#4338ca" }}>{fmtEur(calc.dividendesParPart)}</div>
              </div>
            )}
            <div style={{ marginTop: 8, background: "rgba(254,243,199,0.7)", borderRadius: 8, padding: "7px 10px", fontSize: 10, color: "#92400e", border: "1px solid rgba(251,191,36,0.3)" }}>
              ⚠ La réserve légale (10% du capital social) doit être constituée avant toute distribution.
            </div>
          </div>

          {/* Amortissements détail */}
          <div style={SECTION}>
            <SectionHeader icon="🏗️" title="Détail des amortissements & déductions fiscales" />
            <StatRow label={"Base bâtiment (prix × " + (100 - pf(sciVals.partTerrain)) + "%)"} value={fmtEur(pf(sciVals.prixAchat) * (1 - pf(sciVals.partTerrain) / 100))} />
            <StatRow label={"Amort. bâtiment (" + sciVals.coefAmortBatiment + "%/an)"} value={"– " + fmtEur(calc.amortBatimentAn)} color="#6366f1" />
            <StatRow label={"Amort. travaux (" + sciVals.coefAmortTravaux + "%/an)"} value={"– " + fmtEur(calc.amortTravauxAn)} color="#6366f1" />
            <StatRow label={sciVals.fraisNotaireAmort === "1" ? "Amort. frais notaire/agence (÷5 ans)" : "Frais notaire/agence (déduits an 1)"} value={"– " + fmtEur(sciVals.fraisNotaireAmort === "1" ? calc.amortFraisAcquis : calc.fraisAcquisAn1)} color="#6366f1" />
            <StatRow label={"Amort. frais dossier (÷" + sciVals.dureeFraisDossier + " ans)"} value={"– " + fmtEur(calc.amortFraisDossier)} color="#6366f1" />
            <StatRow label="Provision gros travaux (charge)" value={"– " + fmtEur(calc.provisionGrosAn)} color="#d97706" />
            <StatRow label="Total déductions fiscales/an" value={fmtEur(calc.amortGrandTotal + calc.provisionGrosAn)} bold color="#4338ca" border={false} />
            <div style={{ marginTop: 8, background: "rgba(99,102,241,0.07)", borderRadius: 8, padding: "7px 10px", fontSize: 10, color: "#4338ca" }}>
              💡 Sans ces déductions, la base IS serait <strong>{fmtEur(Math.max(0, calc.loyersAn - calc.chargesTotal - calc.interetsAn1))}</strong> au lieu de <strong>{fmtEur(calc.resultatFiscal)}</strong>
            </div>
          </div>
      ...
      <AmortChartSCI sciVals={sciVals} calc={calc} />

      <div style={{ background: "rgba(241,245,249,0.8)", borderRadius: 12, padding: "10px 14px", fontSize: 11, color: "#64748b", border: "1px solid rgba(148,163,184,0.2)" }}>
        ℹ️ Calculs basés sur l'année 1 (intérêts maximum). Amortissements : bâtiment hors terrain (2.5%/an), travaux (10%/an), frais notaire/agence (÷5 ans ou an 1), frais dossier crédit (÷ durée). Provisions gros travaux déductibles chaque année. Expert-comptable obligatoire en SCI IS. Attention au piège de la plus-value à la revente (amortissements réintégrés).
      </div>
    </div>
  );
}
