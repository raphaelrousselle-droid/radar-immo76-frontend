...
function SCIConsolideChart({ rows }) {
  ...
  rows.forEach(function(r) {
    if (r.loyersTotal > posMax) posMax = r.loyersTotal;
    var neg = -(r.chargesTotal + r.provisionGrosTotal) + (-r.remboursementTotal) + (-r.isTotal);
  ...
  var toY = function(v) { return padT + chartH - ((v - negMin) / range) * chartH; };
  var zeroY = toY(0);
  var colW = chartW / n;
  ...
  for (var gn = -step; gn >= negMin; gn -= step) gridVals.push(gn);
...
async function cloudLoad(key) {
  ...
  var user = sess.data.session && sess.data.session.user;
  if (!user) return null;
  var res = await supabase.from("user_data").select("data_value").eq("user_id", user.id).eq("data_key", key).single();
  ...
  var submit = async function() {
    ...
    try {
      if (mode === "signup") {
        ...
        var res2 = await supabase.auth.signInWithPassword({ email: email, password: pass });
...
export default function App() {
  ...
  return <AppMain user={user} onLogout={function() { supabase.auth.signOut(); setUser(null); }} />;
}
