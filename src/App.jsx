...
function ProfilInvestisseur() {
  ...
  var defaultProfil = {
    emprunteurs: [
      { nom: "", prenom: "", nationalite: "Française", dateNaissance: "", lieuNaissance: "", telephone: "", employeur: "", ancienneteEmployeur: "", profession: "" },
      { nom: "", prenom: "", nationalite: "Française", dateNaissance: "", lieuNaissance: "", telephone: "", employeur: "", ancienneteEmployeur: "", profession: "" },
    ],
    adresse: "",
    situationLogement: "Propriétaire",
    ancienneteLogement: "",
  ...
  React.useEffect(function() {
    ...
    debouncedCloudSave("profil_investisseur", profil, 2000);
  }, [profil]);
  ...
  var fieldStyle = { width: "100%", background: "rgba(248,250,252,0.9)", border: "1px solid rgba(148,163,184,0.4)", borderRadius: 10, padding: "7px 10px", color: "#0f172a", fontSize: 13, outline: "none" };
  ...
  function Field(props) {
    return (<div style={{ marginBottom: 6 }}>
      ...
      <input type={props.type || "text"} value={props.value || ""} onChange={function(e) { update(props.path, e.target.value); }} style={fieldStyle} placeholder={props.placeholder || ""} />
    </div>);
  }
    ...
    <div style={{ display: "flex", flexDirection: "column", gap: 14 }}>
          ...
          {profil.emprunteurs.map(function(emp, idx) {
            ...
            return (<div key={idx} style={{ background: "rgba(248,250,252,0.7)", borderRadius: 12, padding: 12, border: "1px solid rgba(148,163,184,0.15)" }}>
              ...
              <div style={{ display: "grid", gridTemplateColumns: "1fr 1fr", gap: 6 }}>
                <Field label="Nom" path={prefix + "nom"} placeholder="NOM" />
                <Field label="Prénom" path={prefix + "prenom"} placeholder="Prénom" />
                <Field label="Nationalité" path={prefix + "nationalite"} />
                <Field label="Date de naissance" path={prefix + "dateNaissance"} type="date" />
                <Field label="Lieu de naissance" path={prefix + "lieuNaissance"} placeholder="Ville (Dept)" />
                <Field label="Téléphone" path={prefix + "telephone"} placeholder="06..." />
                <Field label="Employeur" path={prefix + "employeur"} />
                <Field label="Ancienneté" path={prefix + "ancienneteEmployeur"} placeholder="Depuis le ..." />
              </div>
            </div>);
          })}
        </div>
        <div style={{ display: "grid", gridTemplateColumns: "1fr 1fr", gap: 8, marginTop: 10 }}>
          <Field label="Adresse commune" path="adresse" placeholder="Adresse complète" />
          <div style={{ display: "grid", gridTemplateColumns: "1fr 1fr", gap: 6 }}>
            <div><label style={labelStyle}>Situation logement</label>
              <select value={profil.situationLogement} onChange={function(e) { update("situationLogement", e.target.value); }} style={fieldStyle}>
                <option>Propriétaire</option><option>Locataire</option><option>Hébergé</option>
              </select>
            </div>
            <Field label="Depuis le" path="ancienneteLogement" placeholder="JJ/MM/AAAA" />
          </div>
        </div>
      </div>
      ...
      <div style={SECTION}>
        ...
        <div style={{ display: "grid", gridTemplateColumns: "repeat(auto-fit, minmax(160px, 1fr))", gap: 10 }}>
          {(function() {
            var revMensuel = totalRevenus / 12;
            ...
            // Estimation capacité d'emprunt à 4% sur 25 ans
            ...
            return [
              { label: "Revenus mensuels", value: fmtEur(Math.round(revMensuel)), color: "#16a34a" },
              { label: "Charges mensuelles (crédits)", value: fmtEur(Math.round(chargesMensuelles)), color: "#dc2626" },
              { label: "Taux d'endettement actuel", value: fmt(tauxEndettement, 1) + " %", color: tauxEndettement > 35 ? "#dc2626" : "#16a34a" },
              { label: "Mensualité disponible (35%)", value: fmtEur(Math.round(capaciteMensuelleRestante)), color: "#6366f1" },
              { label: "Capacité d'emprunt (4%, 25 ans)", value: fmtEur(Math.round(capaciteEmprunt)), color: "#4338ca" },
            ];
          })().map(function(k) {
            return (<div key={k.label} style={{ background: "rgba(255,255,255,0.8)", borderRadius: 12, padding: "10px 14px", border: "1px solid rgba(148,163,184,0.2)" }}>
              <div style={{ fontSize: 10, color: "#94a3b8" }}>{k.label}</div>
