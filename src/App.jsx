...
function SimulationProjet({ photos, setPhotos, projets, setProjets, projetACharger, onProjetCharge }) {
  const [inputs, setInputs] = useState(function() {
    ...
    // La persistence se fait via les projets sauvegardÃ©s
  ...
  // projets et photos reÃ§us en props depuis App (persistent lors des changements d'onglet)
  // Sync photos dans localStorage Ã  chaque modif
  useEffect(function() {
  ...
  // Charger les donnÃ©es de la commune sÃ©lectionnÃ©e
    ...
    <div style={{ display: "flex", flexDirection: "column", gap: 14 }}>
      ...
      <div style={Object.assign({}, SECTION, { padding: "14px 20px" })}>
        <div style={{ display: "flex", justifyContent: "space-between", alignItems: "center", flexWrap: "wrap", gap: 10 }}>
          ...
          <div style={{ display: "flex", gap: 8, alignItems: "center" }}>
            ...
            <button onClick={exportJSON} style={{ background: "rgba(99,102,241,0.1)", border: "1px solid rgba(99,102,241,0.3)", borderRadius: 10, padding: "7px 12px", color: "#4338ca", cursor: "pointer", fontSize: 12, fontWeight: 500 }}>â¬‡ï¸ JSON</button>
            <label style={{ background: "rgba(99,102,241,0.1)", border: "1px solid rgba(99,102,241,0.3)", borderRadius: 10, padding: "7px 12px", color: "#4338ca", cursor: "pointer", fontSize: 12, fontWeight: 500 }}>
              â¬†ï¸ Import<input type="file" accept=".json" onChange={importJSON} style={{ display: "none" }} />
            </label>
            <button onClick={exportPDF} style={{ background: "linear-gradient(135deg,#dc2626,#f97316)", border: "none", borderRadius: 10, padding: "7px 12px", color: "#fff", cursor: "pointer", fontSize: 12, fontWeight: 600 }}>ðŸ“„ Export PDF</button>
          </div>
        </div>
        {projets.length > 0 && (
      ...
      <div style={Object.assign({}, SECTION, { padding: "14px 20px" })}>
        <div style={{ display: "flex", justifyContent: "space-between", alignItems: "center", marginBottom: photos.length > 0 ? 12 : 0 }}>
          <div style={{ fontSize: 12, fontWeight: 700, color: "#334155" }}>ðŸ“· Photos du projet <span style={{ fontSize: 10, color: "#94a3b8", fontWeight: 400 }}>({photos.length} photo{photos.length > 1 ? "s" : ""})</span></div>
              ...
              return (
                <div key={ph.id} style={{ position: "relative", borderRadius: 10, overflow: "hidden", border: idx === 0 ? "2px solid #6366f1" : "1px solid rgba(148,163,184,0.3)" }}>
                  ...
                  <button onClick={function() { removePhoto(ph.id); }}
                    style={{ position: "absolute", top: 4, right: 4, background: "rgba(0,0,0,0.55)", border: "none", borderRadius: "50%", width: 20, height: 20, color: "#fff", cursor: "pointer", fontSize: 12, lineHeight: "20px", padding: 0 }}>âœ•</button>
                </div>
              );
            })}
          </div>
        ...
        {photos.length === 0 && (
          <div style={{ fontSize: 11, color: "#94a3b8", marginTop: 4 }}>Ajoute des photos pour les retrouver facilement dans tes projets sauvegardÃ©s</div>
        )}
      </div>
      ...
      {activeTab === "params" && (
        <div style={{ display: "flex", flexDirection: "column", gap: 14 }}>
          ...
          <div style={Object.assign({}, SECTION, { padding: "14px 20px", isolation: "isolate", zIndex: 10, position: "relative" })}>
            <SectionHeader icon="ðŸ“" title="Commune du bien" />
                ...
                <div style={{ position: "absolute", top: "100%", left: 0, right: 0, zIndex: 1000, background: "#fff", border: "1px solid rgba(148,163,184,0.3)", borderRadius: 10, boxShadow: "0 8px 24px rgba(0,0,0,0.18)", marginTop: 4, overflow: "hidden" }}>
                  {communeSuggestions.map(function(c) {
                    return <div key={c.nom}
                      ...
                      <span>{c.nom}</span>
                      {c.scores && <span style={{ fontSize: 11, color: "#94a3b8" }}>Score {c.scores.global != null ? c.scores.global.toFixed(1) : "â€”"}/10</span>}
                    </div>;
                  })}
                </div>
              )}
            </div>
            {loadingCommune && <div style={{ marginTop: 8, fontSize: 12, color: "#94a3b8" }}>â³ Chargement des donnÃ©es marchÃ©...</div>}
            {donneesCommune && donneesCommune.prix && (function() {
              ...
              var diffLabel = function(d) { return d != null ? (d > 0 ? "â¬†ï¸ +" : "â¬‡ï¸ ") + Math.abs(d).toFixed(1) + "% vs votre projet" : surf > 0 ? "â€”" : "Renseigne la surface"; };
              return (
                <div style={{ marginTop: 14, display: "grid", gridTemplateColumns: "repeat(auto-fit, minmax(160px, 1fr))", gap: 10 }}>
                  {prixM2Appart && <div style={{ background: gb(diffA), borderRadius: 12, padding: "10px 14px", border: "1px solid rgba(148,163,184,0.15)" }}>
                    <div style={{ fontSize: 10, color: "#64748b", fontWeight: 600, textTransform: "uppercase" }}>Prix marchÃ© appart.</div>
                    <div style={{ fontSize: 18, fontWeight: 800, color: gc(diffA), margin: "3px 0 2px" }}>{fmt(prixM2Appart, 0)} â‚¬/mÂ²</div>
                  ...
                  {prixM2Maison && <div style={{ background: gb(diffM), borderRadius: 12, padding: "10px 14px", border: "1px solid rgba(148,163,184,0.15)" }}>
                    <div style={{ fontSize: 10, color: "#64748b", fontWeight: 600, textTransform: "uppercase" }}>Prix marchÃ© maison</div>
                    <div style={{ fontSize: 18, fontWeight: 800, color: gc(diffM), margin: "3px 0 2px" }}>{fmt(prixM2Maison, 0)} â‚¬/mÂ²</div>
                  ...
                  {loyerM2 && <div style={{ background: "rgba(224,242,254,0.5)", borderRadius: 12, padding: "10px 14px", border: "1px solid rgba(148,163,184,0.15)" }}>
                    <div style={{ fontSize: 10, color: "#64748b", fontWeight: 600, textTransform: "uppercase" }}>Loyer marchÃ© /mÂ²</div>
                    <div style={{ fontSize: 18, fontWeight: 800, color: "#0ea5e9", margin: "3px 0 2px" }}>{loyerM2.toFixed(1)} â‚¬/mÂ²/mois</div>
                    <div style={{ fontSize: 11, color: "#64748b" }}>{surf > 0 ? "â†’ Loyer estimÃ© : " + fmt(loyerM2 * surf, 0) + " â‚¬/mois" : "Renseigne la surface"}</div>
                  </div>}
                  {prixM2Projet && <div style={{ background: "rgba(238,242,255,0.7)", borderRadius: 12, padding: "10px 14px", border: "1px solid rgba(148,163,184,0.15)" }}>
                    <div style={{ fontSize: 10, color: "#64748b", fontWeight: 600, textTransform: "uppercase" }}>Votre prix /mÂ²</div>
                    <div style={{ fontSize: 18, fontWeight: 800, color: "#6366f1", margin: "3px 0 2px" }}>{fmt(prixM2Projet, 0)} â‚¬/mÂ²</div>
                    <div style={{ fontSize: 11, color: "#64748b" }}>{fmt(pf(inputs.prixVente), 0)} â‚¬ Ã· {fmt(surf, 0)} mÂ²</div>
                  </div>}
                </div>
              );
            })()}
          ...
          <div style={SECTION}>
            ...
            <div style={{ display: "grid", gridTemplateColumns: "1fr 1fr", gap: 8 }}>
              ...
              <InputField label="GLI (loyers impayÃ©s)" name="gliPct" value={inputs.gliPct} onChange={handleChange} unit="%" step="0.1" />
              ...
              {top10.map(function(c) {
                ...
                return (
                  <div key={c.nom} onClick={function() { setSelected(isSelected ? null : c); }} onContextMenu={function(e) { e.preventDefault(); toggleCompare(c); }}
                    ...
                    {/* Indicateurs rapides : prix mÂ² + tension locative */}
                    <div style={{ display: "flex", gap: 5, flexWrap: "wrap", marginBottom: 8 }}>
                      {c.prix && c.prix.appartement_m2 && (
                        <span style={{ fontSize: 10, fontWeight: 600, background: "rgba(99,102,241,0.08)", color: "#4338ca", borderRadius: 6, padding: "2px 7px" }}>
                          {Number(c.prix.appartement_m2).toLocaleString("fr-FR")} â‚¬/mÂ²
                        </span>
                      )}
                      {c.demographie && c.demographie.tension_locative_pct != null && (
                        <span style={{ fontSize: 10, fontWeight: 600,
                          ...
                          ðŸ”¥ {c.demographie.tension_locative_pct} % tension
                        </span>
                      )}
                      {c.loyer && c.loyer.appartement_m2 != null && (
                        <span style={{ fontSize: 10, fontWeight: 600, background: "rgba(14,165,233,0.08)", color: "#0369a1", borderRadius: 6, padding: "2px 7px" }}>
                          {Number(c.loyer.appartement_m2).toFixed(1)} â‚¬/mÂ²/mois
                        </span>
                      )}
                    </div>
            ...
            {detail && !detail.error && (
              <div>
                ...
                {detail.demographie && detail.demographie.tension_locative_pct != null && (function() {
                  ...
                  var label = t > 15 ? "MarchÃ© trÃ¨s tendu" : t > 8 ? "MarchÃ© tendu" : "MarchÃ© dÃ©tendu";
                  return (
                    <div style={{ marginBottom: 10, background: bg, borderRadius: 10, padding: "10px 12px", border: "1px solid " + border }}>
                      <div style={{ display: "flex", justifyContent: "space-between", alignItems: "center" }}>
                        ...
                        <div style={{ fontSize: 32 }}>{t > 15 ? "ðŸ”¥" : t > 8 ? "âš¡" : "ðŸŸ¢"}</div>
                      </div>
  ...
  return (
    <div style={{ display: "grid", gridTemplateColumns: "repeat(auto-fit, minmax(260px, 1fr))", gap: 14 }}>
      ...
      <div style={{ display: "flex", flexDirection: "column", gap: 10 }}>
          ...
          <div style={{ display: "flex", gap: 6 }}>
            <select
              ...
              {["01","02","03","04","05","06","07","08","09","10","11","12"].map(function(m) {
                var labels = ["Jan","FÃ©v","Mar","Avr","Mai","Jun","Jul","AoÃ»","Sep","Oct","Nov","DÃ©c"];
        ...
        <BienField vals={bien} onChange={onChange} field="gliAn" label="GLI loyers impayÃ©s" unit="â‚¬/an" step="50" hint="â‰ˆ 2-3% des loyers â€” dÃ©ductible IS" />
...
function SimulateurSCI() {
  ...
  // â”€â”€ Sauvegarde SCI â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€
  ...
  function sauvegarderSCI(silent) {
    if (!nomProjet.trim()) {
      if (!silent) alert("Donne un nom Ã  ta simulation SCI");
      return;
    }
    var entry = {
      id: Date.now(),
  ...
  return (
    <div style={{ display: "flex", flexDirection: "column", gap: 14 }}>
      ...
      {/* â”€â”€ Barre de sauvegarde SCI â”€â”€ */}
        ...
        <div style={{ display: "flex", justifyContent: "space-between", alignItems: "center", flexWrap: "wrap", gap: 10 }}>
          <div style={{ display: "flex", gap: 8, alignItems: "center", flexWrap: "wrap" }}>
            ...
            <span style={{ fontSize: 11, color: saveStatus === "saved" ? "#16a34a" : saveStatus === "pending" ? "#d97706" : "transparent" }}>
              {saveStatus === "saved" ? "âœ… SauvegardÃ©" : "ðŸ’¾ Modifications en cours..."}
            </span>
          </div>
        </div>
        {projets.length > 0 && (
              ...
              return (
                <div key={p.id} style={{ display: "flex", alignItems: "center", gap: 5, background: "rgba(99,102,241,0.08)", border: "1px solid rgba(99,102,241,0.25)", borderRadius: 8, padding: "3px 10px" }}>
                  ...
                  <button onClick={function() { supprimerProjetSCI(p.id); }}
                    style={{ background: "none", border: "none", color: "#94a3b8", cursor: "pointer", fontSize: 11 }}>âœ•</button>
                </div>
              );
            })}
          </div>
...
export default function App() {
  ...
  // Photos et projets remontÃ©s ici pour survivre aux changements d'onglet
  ...
  const navItems = [
    ...
    { id: "favoris",   label: "Mes projets",                  icon: "â­" },
  ];
  ...
  const titres = {
    ...
    favoris:   { h: "Mes projets sauvegardÃ©s", sub: "Retrouve et charge toutes tes simulations d'investissement." },
  };
          ...
          <div style={{ display: "flex", flexDirection: "column", gap: 16 }}>
            {simProjets.length === 0 ? (
              <div style={{ textAlign: "center", padding: "60px 20px", color: "#94a3b8" }}>
                ...
                <div style={{ fontSize: 16, fontWeight: 600, color: "#64748b" }}>Aucun projet sauvegardÃ©</div>
                <div style={{ fontSize: 13, marginTop: 6 }}>CrÃ©e une simulation et clique sur "ðŸ’¾ Sauver" pour la retrouver ici.</div>
              </div>
            ) : (
              <div style={{ display: "grid", gridTemplateColumns: "repeat(auto-fill, minmax(280px, 1fr))", gap: 16 }}>
                {simProjets.map(function(p) {
                  ...
                  var noteLabel = note != null ? getNoteLabel(note) : "â€”";
                  return (
                    <div key={p.id} style={{ background: "rgba(255,255,255,0.85)", borderRadius: 20, overflow: "hidden", boxShadow: "0 4px 24px rgba(99,102,241,0.10)", border: "1px solid rgba(148,163,184,0.2)", backdropFilter: "blur(18px)", cursor: "pointer", transition: "transform 0.15s, box-shadow 0.15s" }}
                      ...
                      <div style={{ height: 160, background: p.coverPhoto ? "transparent" : "linear-gradient(135deg,rgba(99,102,241,0.15),rgba(56,189,248,0.15))", position: "relative", overflow: "hidden" }}>
                        {p.coverPhoto
                          ...
                          : <div style={{ display: "flex", alignItems: "center", justifyContent: "center", height: "100%", fontSize: 48 }}>ðŸ </div>
                        }
                        {/* Badge note */}
                        {note != null && (
                      ...
                      <div style={{ padding: "14px 16px" }}>
                        ...
                        <div style={{ display: "flex", justifyContent: "space-between", alignItems: "center", marginBottom: 10 }}>
                          <span style={{ fontSize: 11, color: "#94a3b8" }}>ðŸ“… {p.savedAt}</span>
                            ...
                            return (
                              <div style={{ display: "grid", gridTemplateColumns: "1fr 1fr 1fr", gap: 6, marginBottom: 12 }}>
                                {[
                                  ...
                                  { label: "CF/mois", value: cfMois != null ? (cfMois >= 0 ? "+" : "") + fmt(cfMois, 0) + " â‚¬" : "â€”", color: cfMois != null && cfMois >= 0 ? "#16a34a" : "#dc2626" },
