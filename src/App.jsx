...
const LOT_DEFAULT = { id: 1, nom: "Lot 1", surface: "", loyer: "", travaux: "", charges: "", debutLoyerMois: "0" };
...
function GestionLots({ lots, onChange, surfaceGlobale, loyerGlobal }) {
  ...
  const addLot = function() {
    ...
    onChange(lots.concat([{ id: newId, nom: "Lot " + newId, surface: "", loyer: "", travaux: "", charges: "", debutLoyerMois: "0" }]));
  };
...
function SimulationProjet({ photos, setPhotos, projets, setProjets, projetACharger, onProjetCharge }) {
  ...
  const exportPDF = async function() {
    ...
    // ═══════════════ PAGE 1 — Présentation ═══════════════
    var y = 16;
    doc.setFontSize(22); doc.setFont("helvetica", "bold"); setC(C.primary);
    doc.text(nomProjet || "Projet d'investissement immobilier", W / 2, y, { align: "center" });
    y += 4;
    if (inputs.commune) { doc.setFontSize(12); doc.setFont("helvetica", "normal"); setC(C.accent); doc.text(inputs.commune, W / 2, y + 5, { align: "center" }); y += 8; }
    y += 6;

    if (photos && photos[0] && photos[0].url) {
      try { doc.addImage(photos[0].url, "JPEG", ML, y, CW, 55, "", "MEDIUM"); y += 58; } catch(e) { y += 2; }
    }

    var kw = (CW - 8) / 3;
    kpiBox(ML, y, kw, 22, "Prix d'achat FAI", fmt(pv, 0) + " €", C.green);
    kpiBox(ML + kw + 4, y, kw, 22, "Superficie", surf > 0 ? fmt(surf, 0) + " m²" : "—", C.red);
    kpiBox(ML + kw * 2 + 8, y, kw, 22, "Rendement brut", fmtPct(r.rendBrut), C.orange);
    y += 26;

    y = banner("Descriptif du projet", y);
    doc.setFontSize(9); doc.setFont("helvetica", "normal"); setC(C.dark);
    var desc = (nomProjet || "Projet") + " — " + (inputs.commune || "") + " — " + lots.length + " lot(s)";
    if (surf > 0) desc += " — " + fmt(surf, 0) + " m²";
    doc.text(desc, ML + 3, y); y += 6;

    if (lots.length > 0) {
      doc.setFontSize(8); doc.setFont("helvetica", "bold"); setC(C.grey);
      doc.text("Lot", ML + 3, y); doc.text("Surface", ML + 55, y); doc.text("Loyer/mois", ML + 85, y); doc.text("Travaux", ML + 120, y); doc.text("Début loyer", ML + 150, y);
      y += 2; setD(C.lightGrey); doc.line(ML, y, W - MR, y); y += 4;
      lots.slice(0, 8).forEach(function(l) {
        doc.setFontSize(8.5); doc.setFont("helvetica", "normal"); setC(C.dark);
        doc.text((l.nom || "Lot").substring(0, 20), ML + 3, y);
        doc.text(l.surface ? l.surface + " m²" : "—", ML + 55, y);
        doc.text(l.loyer ? fmt(pf(l.loyer), 0) + " €" : "—", ML + 85, y);
        doc.text(pf(l.travaux) > 0 ? fmt(pf(l.travaux), 0) + " €" : "—", ML + 120, y);
        var deb = pf(l.debutLoyerMois); doc.text(deb > 0 ? deb + " mois" : "Immédiat", ML + 150, y);
        y += 5.5;
      });
      y += 4;
    }

    setF([255, 248, 220]); doc.roundedRect(ML, y, CW, 22, 3, 3, "F");
    doc.setFontSize(10); doc.setFont("helvetica", "bold"); setC(C.red);
    doc.text("Vos notes", ML + 5, y + 7);
    y += 26;
    footer(1);
    ...
    kpiBox(ML, y, kw, 22, "Loyer mensuel HC", fmt(pf(inputs.loyerMensuelHC), 0) + " €", C.green);
    kpiBox(ML + kw + 4, y, kw, 22, "Charges mensuelles (avec crédit)", fmt(Math.round((r.totalFraisAnnuels + r.remboursementAnnuel) / 12), 0) + " €", C.orange);
    kpiBox(ML + kw * 2 + 8, y, kw, 22, "Cash-Flow", (tresoPMois >= 0 ? "+" : "") + fmt(Math.round(tresoPMois), 0) + " €", tresoPMois >= 0 ? C.green : C.red);
    y += 26;
    var kw2 = (CW - 4) / 2;
    kpiBox(ML, y, kw2, 20, "Rendement brut", fmtPct(r.rendBrut), C.orange);
    kpiBox(ML + kw2 + 4, y, kw2, 20, "Rendement net", fmtPct(regime.rendNet), C.primary);
    y += 24;
    ...
    var chargesList = [
      ...
      ["Assurance PNO", fmt(pf(inputs.assurancePNOAn) || Math.round(r.depenseNette * 0.0012), 0) + " €/an"],
      ["Gestion agence (" + pf(inputs.gestionLocativePct) + " %)", fmt(Math.round(r.loyersAnnuels * pf(inputs.gestionLocativePct) / 100), 0) + " €/an"],
      ["Garantie loyers impayés (" + pf(inputs.gliPct) + " %)", fmt(Math.round(r.loyersAnnuels * pf(inputs.gliPct) / 100), 1) + " €/an"],
      ["Charges de copropriété", fmt(pf(inputs.chargesImmeubleAn), 0) + " €/an"],
      ["Vacance locative (" + (12 - pf(inputs.tauxOccupation)).toFixed(1) + " mois/an)", fmt(Math.round(pf(inputs.loyerMensuelHC) * (12 - pf(inputs.tauxOccupation))), 0) + " €/an"],
      ["Provision travaux", fmt(pf(inputs.provisionTravauxAn), 0) + " €/an"],
      ["Comptabilité / CGA / CFE", fmt(pf(inputs.expertComptableAn), 0) + " €/an"],
    ...
    doc.text(fmt(Math.round(r.totalFraisAnnuels), 0) + " €/an", W - MR - 3, y + 1, { align: "right" });
    y += 10;
    ...
    var blocks = [
      { title: "Achat", x: ML, rows: [
        ["Prix d'achat FAI", fmt(pv, 0) + " €"], ["Frais de notaire", fmt(pf(inputs.fraisNotaire), 0) + " €"],
        ["Autre frais", "0 €"], ["Meubles", "0 €"], ["Travaux", fmt(pf(inputs.travaux), 0) + " €"],
        ["Frais bancaire et cautionnement", fmt(pf(inputs.fraisBancairesAn), 0) + " €"],
        ["Total achat", fmt(Math.round(r.depenseNette), 0) + " €"],
      ]},
      { title: "Crédit", x: ML + tw + 4, rows: [
        ["Apport", fmt(pf(inputs.apport), 0) + " €"], ["Montant emprunté", fmt(Math.round(r.sommeEmpruntee), 0) + " €"],
        ["Frais bancaire et cautionnement", fmt(pf(inputs.fraisBancairesAn), 0) + " €"],
        ["Taux crédit", pf(inputs.tauxCredit) + " %"], ["Taux assurance", "0,5 %"],
        ["Durée", pf(inputs.dureeAnnees) + " ans"], ["Mensualité", fmt(Math.round(r.mensualite), 0) + " €"],
      ]},
      { title: "Indicateurs clés", x: ML + tw * 2 + 8, rows: [
        ["Prix au m² à l'achat", prixM2 ? fmt(Math.round(prixM2), 0) + " €/m²" : "—"],
        ["Prix au m² final", prixM2 ? fmt(Math.round(r.depenseNette / surf), 0) + " €/m²" : "—"],
        ["Rendement brut", fmtPct(r.rendBrut)], ["Rendement net", fmtPct(regime.rendNet)],
        ["Cash-flow annuel", fmt(Math.round(regime.tresorerie), 0) + " €"],
        ["Cash-flow mensuel", fmt(Math.round(tresoPMois), 0) + " €"],
        ["Taux de couverture", regime.regle70 != null ? fmtPct(1 / regime.regle70) : "—"],
      ]},
    ];
    ...
    if (donneesCommune && donneesCommune.scores) {
      ...
      kpiBox(ML, y, kw, 22, "Nombre d'habitants", dc.population ? dc.population.toLocaleString("fr-FR") : "—", C.green);
      kpiBox(ML + kw + 4, y, kw, 22, "Tension locative", dem.tension_locative_pct != null ? dem.tension_locative_pct + " %" : "—", C.red);
      kpiBox(ML + kw * 2 + 8, y, kw, 22, "Prix au m² moyen", pri.appartement_m2 ? fmt(pri.appartement_m2, 0) + " €/m²" : "—", C.accent);
      y += 26;

      y = sectionH("Informations sur la ville", y);
      var infoItems = [
        ["Niveau de vie moyen par habitant", se.revenu_median ? se.revenu_median.toLocaleString("fr-FR") + " €" : "—"],
    ...
    kpiBox(ML, y, kw, 22, "Loyers annuels HC", fmt(Math.round(r.loyersAnnuels), 0) + " €", C.green);
    kpiBox(ML + kw + 4, y, kw, 22, "Amortissement annuel moyen", fmt(Math.round(r.amortissement), 0) + " €", C.orange);
    ...
    Object.entries(r.regimes).forEach(function(entry) {
      ...
      doc.text(nom, rX[0], y);
      doc.text(fmt(Math.round(reg.tresorerie), 0) + " €", rX[1], y);
      var cfm = reg.tresorerie / 12; setC(cfm >= 0 ? C.green : C.red);
      doc.text(fmt(Math.round(cfm), 0) + " €", rX[2], y);
      setC(C.red); doc.text(fmt(Math.round(reg.impot), 0) + " €", rX[3], y);
      setC(C.dark); doc.text(fmtPct(reg.rendBrut), rX[4], y); doc.text(fmtPct(reg.rendNet), rX[5], y);
      y += 7;
    });
    y += 4;
    ...
    cfData.slice(0, maxRows).forEach(function(d, idx) {
      ...
      doc.text(fmt(Math.round(d.loyers), 0) + " €", tX[1], y);
      doc.text("-" + fmt(Math.round(d.credit), 0) + " €", tX[2], y);
      doc.text("-" + fmt(Math.round(d.frais), 0) + " €", tX[3], y);
      setC(C.red); doc.text("-" + fmt(Math.round(d.impots), 0) + " €", tX[4], y);
      setC(d.cashflow >= 0 ? C.green : C.red); doc.setFont("helvetica", "bold");
      doc.text((d.cashflow >= 0 ? "+" : "") + fmt(Math.round(d.cashflow), 0) + " €", tX[5], y);
